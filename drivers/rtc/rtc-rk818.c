 ...
 */
static int
rk818_rtc_readtime(struct device *dev, struct rtc_time *tm)
{
	...
	do {
		...
		pr_info("RTC date/time %4d-%02d-%02d(%d) %02d:%02d:%02d\n",
 ...
 */
static int
rk818_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(dev);
	struct rk818 *rk818 = rk818_rtc->rk818;
	int ret;
 ...
 */
static int
rk818_rtc_readalarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(dev);
	int ret;
...
static int
rk818_rtc_stop_alarm(struct rk818_rtc *rk818_rtc)
{
...
static int
rk818_rtc_start_alarm(struct rk818_rtc *rk818_rtc)
{
...
static int
rk818_rtc_setalarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(dev);
	int ret;
...
static int
rk818_rtc_alarm_irq_enable(struct device *dev, unsigned int enabled)
{
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(dev);
...
static int
rk818_rtc_update_irq_enable(struct device *dev, unsigned int enabled)
{
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(dev);
 ...
 */
static int
rk818_rtc_irq_set_freq(struct device *dev, int freq)
{
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(dev);
	int ret;
	u8 rtc_ctl;
...
static irqreturn_t
rk818_alm_irq(int irq, void *data)
{
...
static irqreturn_t
rk818_per_irq(int irq, void *data)
{
	struct rk818_rtc *rk818_rtc = data;
...
static int
rk818_rtc_suspend(struct device *dev)
{
 ...
 */
static int
rk818_rtc_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(&pdev->dev);
	int ret;
...
static int
rk818_rtc_freeze(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct rk818_rtc *rk818_rtc = dev_get_drvdata(&pdev->dev);
	int ret;
...
static int
rk818_rtc_probe(struct platform_device *pdev)
{
...
static int
rk818_rtc_remove(struct platform_device *pdev)
{
	struct rk818_rtc *rk818_rtc = platform_get_drvdata(pdev);
	int per_irq = rk818_rtc->rk818->chip_irq + RK818_IRQ_RTC_PERIOD;
	int alm_irq = rk818_rtc->rk818->chip_irq + RK818_IRQ_RTC_ALARM;
...
static ssize_t
rtc_rk818_test_write(struct file *file,
		     const char __user *buf, size_t count, loff_t *offset)
{
	...
	if (nr == 3) {
		...
		ret =
		    rk818_reg_write(rk818_rtc->rk818, RK818_RTC_STATUS_REG,
				    ret & 0xff);
		if (ret < 0) {
			pr_err("%s:Failed to read RTC status: %d\n", __func__,
				ret);
