...
static int xgold_spi_init_ctrl_from_platdata(struct platform_device *pdev)
{
	...
	if (IS_ERR(platdata->pins_reset))
        ...
        platdata->pins_off = pinctrl_lookup_state(platdata->pinctrl,
                                 "off");
        if (IS_ERR(platdata->pins_off))
                dev_err(dev, "could not get off pinstate\n");
skip_pinctrl:
	return 0;
...
#ifdef ctrl_cs_change
	enable_cs(ctl_drv, ctl_drv->current_msg->spi);
#endif
			...
			iowrite32(mis & USIF_SPI_TXFIFO_MASK,
					USIF_ICR(ctl_drv->base));
#ifdef ctrl_cs_change
	disable_cs(ctl_drv, ctl_drv->current_msg->spi);
#endif
		}
	}
...
static void xgold_spi_off(struct spi_device *spi, bool off)
{
        struct xgold_spi_ctl_drv *ctl_drv = spi_master_get_devdata(spi->master);
        ...
        if (off) {
            printk("%s: set pinstate pins_off \n", __func__);
            pinstate = pdata->pins_off;
        } else {
            printk("%s: set pinstate pins_default \n", __func__);
...
static int xgold_spi_transfer(struct spi_device *spi, struct spi_message *msg)
{
	...
	{
		struct spi_transfer *xfer;
		pr_info("SPI transfer, DMA mapped: %s\n",
...
static int xgold_spi_resume(struct device *dev)
{
	...
	xgold_spi_set_pinctrl_state(dev,  pdata->pins_off);
