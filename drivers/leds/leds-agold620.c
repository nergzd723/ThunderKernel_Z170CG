...
#define LED_DRIVER_ON	0
#define LED_DRIVER_OFF	1

static int led_ctrl = SCU_LED_UP_CMP_100mv;

static int ledoff=LED_DRIVER_OFF;
struct xgold_led_data *led_g;
...
static inline void agold620_led_on(struct device *dev)
{
	...
	pr_debug("%s -->\n", __func__);
	if (freerun_mode) {
		int32_t val = (TOTAL_CLK_20K_IN_26M * intensity)/DRIVER_MAX_INTENSITY;
		mdelay(10);
		led_write32(led, LED_K2_CONTROL, TOTAL_CLK_20K_IN_26M - val);
		led_write32(led, LED_K1_CONTROL, val);
	} else {
		int32_t val = (SCU_K2_VAL * DRIVER_MAX_INTENSITY)/intensity;
		mdelay(10);
...
static inline void agold620_led_off(struct device *dev)
{
	...
	pr_debug("%s -->\n", __func__);
	led_write32(led, LED_CTRL, SCU_LED_DOWN);
	ledoff=LED_DRIVER_OFF;
	led->set_gpio(dev, false);
}
