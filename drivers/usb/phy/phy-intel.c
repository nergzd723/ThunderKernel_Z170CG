...
int hardware_id = 0;
static int usb_suspend_tag = 0;
struct intel_usbphy *usb_audio_cover;
extern void cover_detect_register(char *, char *, void(*)(bool));
static int usb_audio_cover_detected = 0;
static int pre_usb_audio_cover_detected = 0;
...
#define USB_VBUS_SW 82
#define USB_DPDN_SEL 79
...
static irqreturn_t intel_usb2phy_id(int irq, void *dev)
{
	...
	if (!(hardware_id == 1)) {
		if(usb_audio_cover_detected) {
			gpio_set_value(iphy->usbvbussw_gpio,0);
			mdelay(250);
		}
	}
#endif
	...
	if (id_status) {
		...
		printk(KERN_INFO "ID_Status = %d, USB OTG Cable Disconnected\n", id_status);
		...
		if (!(hardware_id == 1)) {
			if(usb_audio_cover_detected) {
				printk(KERN_INFO "USB-Audio : PAD-OUTPUT + OTG\n");
				gpio_set_value(iphy->usbdpdnsel_gpio,0);
			}
		}
#endif
		dev_dbg(iphy->phy.dev, "ID clear\n");
		printk(KERN_INFO "ID_Status = %d, USB OTG Cable Connected\n", id_status);
		usb_cable_status = POWER_SUPPLY_CHARGER_TYPE_USB_OTG;
		cable_status_notifier_call_chain(usb_cable_status, dev);
		set_bit(A_BUS_REQ, &iphy->inputs);
...
static void audio_cover_attached(bool attached)
{
	usb_audio_cover_detected = attached;
	printk(KERN_INFO "+++ Audio Cover IRQ +++\n");

	if (usb_audio_cover_detected  != pre_usb_audio_cover_detected) {
		printk(KERN_INFO "Receive difference IRQ for Audio-Cover\n");
		pre_usb_audio_cover_detected = usb_audio_cover_detected;
		schedule_delayed_work(&usb_audio_cover->usb_coverdet_work, 1*HZ);
	}
	else {
		printk(KERN_INFO "Receive same IRQ for Audio-Cover\n");
		pre_usb_audio_cover_detected = usb_audio_cover_detected;
	}
}

unsigned int audio_cover_attached_query(void)
{
	printk(KERN_INFO "%s : Current audio cover status = %d\n", __func__, usb_audio_cover_detected);
	return usb_audio_cover_detected;
}
EXPORT_SYMBOL(audio_cover_attached_query);
...
#endif
			dev_info(iphy->dev, "BSV set\n");
			set_bit(B_SESS_VLD, &iphy->inputs);
		} else {
			dev_info(iphy->dev, "BSV clear\n");
			clear_bit(B_SESS_VLD, &iphy->inputs);
			clear_bit(A_BUS_SUSPEND, &iphy->inputs);
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
			if (!(hardware_id == 1)) {
				if (usb_audio_cover_detected && (query_cable_status() == POWER_SUPPLY_CHARGER_TYPE_USB_DCP)) {
					dev_info(iphy->dev, "Audio Cover : Unplug AC Cable\n");
					gpio_set_value(iphy->usbvbussw_gpio,1);
					gpio_set_value(iphy->usbdpdnsel_gpio,1);
				...
				if(usb_audio_cover_detected){
					printk(KERN_INFO "Audio Cover : Set usbvbussw to low\n");
					gpio_set_value(iphy->usbvbussw_gpio,0);
				}
			}
#endif
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
			if (!(hardware_id == 1)) {
				if(usb_audio_cover_detected && (query_cable_status() == POWER_SUPPLY_CHARGER_TYPE_NONE))
...
static void intel_chg_detect_work(struct work_struct *w)
{
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (usb_fsvplus_status(iphy)) {
			...
			usb_cable_status = iphy->chg_type;
			iphy->chg_state =
			...
			if (iphy->tmout)
				...
				/* Add workaround to support floating charger */
				iphy->chg_type =
	...
	case USB_CHG_STATE_DETECTED:
		...
		cable_status_notifier_call_chain(usb_cable_status, phy);
		schedule_work(&iphy->sm_work);
		return;
	default:
		return;
...
static void asus_cover_detect_work(struct work_struct *w)
{
	struct intel_usbphy *iphy
			= container_of(w, struct intel_usbphy, usb_coverdet_work.work);

	printk(KERN_INFO "ASUS Cover Detect Work and Attached Value is %d\n", usb_audio_cover_detected);

	if(usb_audio_cover_detected) {
		switch(query_cable_status())
		{
			case POWER_SUPPLY_CHARGER_TYPE_NONE:
				printk(KERN_INFO "USB-Audio : COVER-OUTPUT + NONE\n");
				gpio_set_value(iphy->usbdpdnsel_gpio,1);
				gpio_set_value(iphy->usbvbussw_gpio,0);
				clear_bit(ID, &iphy->inputs);
				pm_runtime_resume(iphy->dev);
				schedule_work(&iphy->sm_work);
				break;
			case POWER_SUPPLY_CHARGER_TYPE_USB_SDP:
				printk(KERN_INFO "USB-Audio : PAD-OUTPUT + SDP\n");
				gpio_set_value(iphy->usbvbussw_gpio,0);
				break;
			case POWER_SUPPLY_CHARGER_TYPE_USB_DCP:
				printk(KERN_INFO "USB-Audio : COVER-OUTPUT + DCP\n");
				gpio_set_value(iphy->usbdpdnsel_gpio,1);
				gpio_set_value(iphy->usbvbussw_gpio,1);
				clear_bit(ID, &iphy->inputs);
				pm_runtime_resume(iphy->dev);
			...
			case POWER_SUPPLY_CHARGER_TYPE_USB_OTG:
				...
				break;
			default:
				printk(KERN_INFO "POWER_SUPPLY_CHARGER_TYPE_UNDEFINED\n");
				break;
		}
	}
	else {
		...
		{
			case POWER_SUPPLY_CHARGER_TYPE_NONE:
				printk(KERN_INFO "USB-Audio : PAD-OUTPUT + NONE\n");
				gpio_set_value(iphy->usbdpdnsel_gpio,0);
				gpio_set_value(iphy->usbvbussw_gpio,0);
				set_bit(ID, &iphy->inputs);
				clear_bit(B_SESS_VLD, &iphy->inputs);
			...
			case POWER_SUPPLY_CHARGER_TYPE_USB_SDP:
				...
				break;
			case POWER_SUPPLY_CHARGER_TYPE_USB_DCP:
				printk(KERN_INFO "USB-Audio : PAD-OUTPUT + DCP\n");
				gpio_set_value(iphy->usbdpdnsel_gpio,0);
				set_bit(ID, &iphy->inputs);
				clear_bit(B_SESS_VLD, &iphy->inputs);
//				pm_runtime_resume(iphy->dev);
...
static void intel_otg_init_sm(struct intel_usbphy *iphy)
{
	...
	case USB_DR_MODE_OTG:
		if (usbid_status(iphy)) {
			...
			printk(KERN_INFO "ID_Status = %d, USB OTG Cable Connected\n", usbid_status(iphy));
			usb_cable_status = POWER_SUPPLY_CHARGER_TYPE_USB_OTG;
			cable_status_notifier_call_chain(usb_cable_status, iphy);
			set_bit(A_BUS_REQ, &iphy->inputs);
			clear_bit(ID, &iphy->inputs);
			irq_set_irq_type(iphy->id_irq, IRQ_TYPE_EDGE_RISING);
		}
		...
		if (!test_bit(ID, &iphy->inputs) && otg->host) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case POWER_SUPPLY_CHARGER_TYPE_USB_DCP:
					...
					if (!(hardware_id == 1)) {
						if(usb_audio_cover_detected)
							schedule_delayed_work(&iphy->usb_coverdet_work, 0*HZ);
					}
#endif
					break;
				case POWER_SUPPLY_CHARGER_TYPE_USB_CDP:
			...
			if (!(hardware_id == 1)) {
				if(usb_audio_cover_detected)
					schedule_delayed_work(&iphy->usb_coverdet_work, 1*HZ);
			}
#endif
		}
		break;
...
static int intel_otg_usbdpdnsel_show(struct seq_file *s, void *unused)
{

	return 0;
}

static int intel_otg_usbdpdnsel_open(struct inode *inode, struct file *file)
{
	return single_open(file, intel_otg_usbdpdnsel_show, inode->i_private);
}


static ssize_t intel_otg_usbdpdnsel_write(struct file *file, const char __user *ubuf,
...
const struct file_operations intel_otg_usbdpdnsel_fops = {
	.open = intel_otg_usbdpdnsel_open,
	.read = seq_read,
	.write = intel_otg_usbdpdnsel_write,
	.llseek = seq_lseek,
	.release = single_release,
};

static int intel_otg_usbvbussw_show(struct seq_file *s, void *unused)
{

	return 0;
}

static int intel_otg_usbvbussw_open(struct inode *inode, struct file *file)
{
	return single_open(file, intel_otg_usbvbussw_show, inode->i_private);
}


static ssize_t intel_otg_usbvbussw_write(struct file *file, const char __user *ubuf,
				size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct intel_usbphy *iphy = s->private;
...
const struct file_operations intel_otg_usbvbussw_fops = {
	.open = intel_otg_usbvbussw_open,
	.read = seq_read,
	.write = intel_otg_usbvbussw_write,
	.llseek = seq_lseek,
	.release = single_release,
};
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
	if (!(hardware_id == 1)) {
		intel_otg_dentry = debugfs_create_file("usbvbussw", S_IRUGO | S_IWUSR,
			intel_otg_dbg_root, iphy,
			&intel_otg_usbvbussw_fops);
		...
		intel_otg_dentry = debugfs_create_file("usbdpdnsel", S_IRUGO | S_IWUSR,
			intel_otg_dbg_root, iphy,
			&intel_otg_usbdpdnsel_fops);
...
static int usb_lcd_power_on_notification_handler(struct notifier_block *nb, unsigned long event, void *data)
{
	struct fb_event *evdata = data;
	struct usb_otg *otg = usb_audio_cover->phy.otg;
	int *blank;

	blank = evdata->data;
	printk(KERN_INFO "USB : Receive LCD Event +++\n");

	if ((usb_cable_status == POWER_SUPPLY_CHARGER_TYPE_USB_OTG) ||
				(usb_audio_cover_detected && !(hardware_id == 1) &&
				(usb_cable_status == POWER_SUPPLY_CHARGER_TYPE_NONE || usb_cable_status == POWER_SUPPLY_CHARGER_TYPE_USB_DCP))) {
		if(*blank == FB_BLANK_UNBLANK && event == FB_EVENT_BLANK && usb_suspend_tag == 1){
			usb_suspend_tag == 0;
			printk(KERN_INFO "USB-Devices Enumerated\n");
			set_bit(A_BUS_REQ, &usb_audio_cover->inputs);
			clear_bit(ID, &usb_audio_cover->inputs);
			usb_audio_cover->sm_work_pending = true;
		}

		atomic_set(&usb_audio_cover->pm_suspended, 0);

		if (usb_audio_cover->async_int || usb_audio_cover->sm_work_pending) {
			if (usb_audio_cover->async_int)
				usb_audio_cover->async_int = 0;
			pm_runtime_get_noresume(usb_audio_cover->dev);
			intel_otg_resume(usb_audio_cover);

			/* Update runtime PM status */
			pm_runtime_disable(usb_audio_cover->dev);
			pm_runtime_set_active(usb_audio_cover->dev);
			pm_runtime_enable(usb_audio_cover->dev);

			if (usb_audio_cover->sm_work_pending) {
				usb_audio_cover->sm_work_pending = false;
				schedule_work(&usb_audio_cover->sm_work);
			}
		}
	}
	return NOTIFY_OK;
...
static struct notifier_block usb_lcd_power_on_notification = {
	.notifier_call = usb_lcd_power_on_notification_handler,
};
...
static int intel_usb2phy_probe(struct platform_device *pdev)
{
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
	if (!(hardware_id == 1)) {
		iphy->usbdpdnsel_gpio = of_get_named_gpio(np, "intel,usbdpdnsel-gpio", 0);
		if (!gpio_is_valid(iphy->usbdpdnsel_gpio)) {
			dev_info(dev, "can't find usbdpdnsel_gpio gpio, using phy by default\n");
			iphy->usbdpdnsel_gpio = -1;
		}

		ret = gpio_request(iphy->usbdpdnsel_gpio, "usbdpdnsel_gpio");
		gpio_set_value(iphy->usbdpdnsel_gpio,0);
		if (ret) {
			pr_err("Unable to configure GPIO%d usbdpdnsel_gpio\n", iphy->usbdpdnsel_gpio);
		}
		printk(KERN_INFO"phy->usbdpdnsel-gpio value = %d\n",gpio_get_value(iphy->usbdpdnsel_gpio));

		iphy->usbvbussw_gpio = of_get_named_gpio(np, "intel,usbvbussw-gpio", 0);
		if (!gpio_is_valid(iphy->usbvbussw_gpio)) {
			dev_info(dev, "can't find usbvbussw_gpio gpio, using phy by default\n");
			iphy->usbvbussw_gpio = -1;
		}

		ret = gpio_request(iphy->usbvbussw_gpio, "usbvbussw_gpio");
		gpio_set_value(iphy->usbvbussw_gpio,0);
		if (ret) {
			pr_err("Unable to configure GPIO%d usbvbussw_gpio\n", iphy->usbvbussw_gpio);
		}
		printk(KERN_INFO"phy->usbvbussw_gpio value = %d\n",gpio_get_value(iphy->usbvbussw_gpio));
	}
#endif
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
	if (!(hardware_id == 1)) {
		cover_detect_register("audio_sleeve", "audio_cover_attached", audio_cover_attached);
		INIT_DELAYED_WORK(&iphy->usb_coverdet_work, asus_cover_detect_work);
	}
#endif
	...
	usb_audio_cover = iphy;
	fb_register_client(&usb_lcd_power_on_notification);
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
	if (!(hardware_id == 1))
		cancel_delayed_work_sync(&iphy->usb_coverdet_work);
#endif
	fb_unregister_client(&usb_lcd_power_on_notification);
...
static int intel_otg_pm_suspend(struct device *dev)
{
	...
	usb_suspend_tag = 1;
	dev_dbg(iphy->dev, "%s\n", usb_otg_state_string(otg->phy->state));
	if (otg->phy->state == OTG_STATE_A_HOST || otg->phy->state == OTG_STATE_A_WAIT_BCON) {
		dev_info(dev, "force disconnect device\n");
		set_bit(ID, &iphy->inputs);
		clear_bit(B_CONN, &iphy->inputs);
		clear_bit(A_BUS_REQ, &iphy->inputs);
		clear_bit(A_VBUS_VLD, &iphy->inputs);
...
static int intel_otg_pm_resume(struct device *dev)
{
	...
	if (test_bit(ID, &iphy->inputs))
		...
		dev_dbg(dev, "ID clear\n");

	if (usb_audio_cover_detected)
		dev_dbg(dev, "audio cover detected\n");


	if (usbid_status(iphy) && usb_audio_cover_detected == 0) {
		printk(KERN_INFO "Sm_work_pending : true\n");
		iphy->sm_work_pending = true;
	}
