...
//WA_add
volatile unsigned int recv_1st_setup  = 0;
volatile unsigned int inTXFEMP  = 0;
volatile unsigned int inNAKEFF  = 0;
volatile unsigned int inNAKINTR  = 0;

EXPORT_SYMBOL_GPL(recv_1st_setup);
EXPORT_SYMBOL_GPL(inTXFEMP);
EXPORT_SYMBOL_GPL(inNAKEFF);
EXPORT_SYMBOL_GPL(inNAKINTR);
 ...
 */
void s3c_hsotg_start_req(struct dwc2_hsotg *hsotg,
				struct s3c_hsotg_ep *hs_ep,
				struct s3c_hsotg_req *hs_req,
				bool continuing)
{
...
EXPORT_SYMBOL_GPL(s3c_hsotg_start_req);
/**
	...
	if (dir_in && !hs_ep->isochronous) {
		...
		if (ints & DIEPMSK_INTKNTXFEMPMSK) {
			...
			dev_dbg(hsotg->dev, "%s: ep%d: INTknTXFEmpMsk\n",
                ...
                if (ints & DIEPMSK_INEPNAKEFFMSK) {
                        if(recv_1st_setup == 1) {
                                inNAKEFF++;
                        }
			printk(KERN_INFO "inTXFEMP: %d\n", inTXFEMP);
			printk(KERN_INFO "inNAKEFF: %d\n", inNAKEFF);
			printk(KERN_INFO "inNAKINTR: %d\n", inNAKINTR);
			if(inNAKEFF == 1) {
					u32 dsts = readl(hsotg->regs + DSTS);
					if(dsts & DSTS_SUSPSTS)
						printk(KERN_INFO "DSTS_SUSPSTS\n");
					else {
						printk(KERN_INFO "Bus keep alive\n");
						int epCTL0 = readl(hsotg->regs + 0x900);
						epCTL0 |= DXEPCTL_EPDIS;
						printk(KERN_INFO "epCTL0: 0x%x\n", epCTL0);
						writel(epCTL0, hsotg->regs + 0x900);
					}
			}

                }
                if (ints & DXEPINT_NAKINTR) {
                        if(recv_1st_setup == 1) {
                                inNAKINTR++;
                        }
                }
						...
						bool is_usb_reset)
{
	...
	writel(((hsotg->dedicated_fifos && !using_dma(hsotg)) ?
		...
		DIEPMSK_INTKNEPMISMSK | DIEPMSK_INEPNAKEFFMSK,
		hsotg->regs + DIEPMSK); // WA_add DIEPMSK_INEPNAKEFFMSK
...
static void s3c_hsotg_init(struct dwc2_hsotg *hsotg)
{
	...
	writel(DIEPMSK_TIMEOUTMSK | DIEPMSK_AHBERRMSK |
		DIEPMSK_EPDISBLDMSK | DIEPMSK_XFERCOMPLMSK | DIEPMSK_INEPNAKEFFMSK,
		hsotg->regs + DIEPMSK); // WA_add
