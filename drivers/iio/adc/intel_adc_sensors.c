				...
				int *p_val, int *p_val2, long mask)
{
	...
	case IIO_CHAN_INFO_PROCESSED:{
		...
		adc_ret =
			...
			//ASUS Rock print log for debug
			//printk(KERN_ERR "sensors:%s,adc_voltage_uv=%d,adc_current_na=%d, degreeC = %d\n",
				//adc_sensors_info[p_channel->adc_log_chan].adc_sensor_name,adc_voltage_uv,adc_current_na,*p_val);
			break;
		}
