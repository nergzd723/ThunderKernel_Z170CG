...
struct xgold_dma_driver_data {
	...
	int num_signals;
	int (*pl08x_suspend)(struct device *dev);
	int (*pl08x_resume)(struct device *dev);
	spinlock_t lock;
...
#ifdef CONFIG_PM
/*
 * xgold_dma_suspend: suspend function for xgold-wrapper
 * responsible for changing DMA pm state to disable
 * dev: xgold dma device pointer
 */
static int xgold_dma_suspend(struct device *dev)
{
	struct xgold_dma_driver_data *dma = dev_get_drvdata(dev);
	struct amba_device *adev = xgold_dma_to_amba(dma);
	struct pl08x_platform_data *pl08x_platdata = adev->dev.platform_data;
...
/*
 * xgold_dma_resume: resume function for xgold-wrapper
 * responsible for changing DMA pm state to enable or high perf
 * dev: xgold dma device pointer
 * */
static int xgold_dma_resume(struct device *dev)
{
	struct xgold_dma_driver_data *dma = dev_get_drvdata(dev);
	struct amba_device *adev = xgold_dma_to_amba(dma);
	struct pl08x_platform_data *pl08x_platdata = adev->dev.platform_data;
...
/*
 * adev_dma_runtime_suspend: main suspend function for the pl08x dma device
 * This callback is used to shut down the pl08x IP by first calling pl08x
 * suspend callback, then call xgold-wrapper suspend.
 *
 * dev: pl08x dma device pointer
 */
static int adev_dma_runtime_suspend(struct device *dev)
{
	struct amba_device *adev = to_amba_device(dev);
	struct xgold_dma_driver_data *dma = amba_to_xgold_dma(adev);
	int ret;

	dev_dbg(dev, "--> %s: suspended %d\n", __func__,
			atomic_read(&dma->is_suspended));

	if (atomic_cmpxchg(&dma->is_suspended, 0, 1))
		return 0;

	if (dma->pl08x_suspend) {
		ret = dma->pl08x_suspend(dev);

		if (ret) {
			dev_err(dev, "pl08x suspend error %d\n", ret);
			return ret;
		}
	}

	return xgold_dma_suspend(dma->dev);
}

/*
 * adev_dma_runtime_resume: main resume function for the pl08x dma device
 * This callback is used to properly power up the pl08x IP by first calling
 * the xgold-wrapper resume callback, then the pl08x resume function.
 *
 * dev: pl08x dma device pointer
 */
static int adev_dma_runtime_resume(struct device *dev)
{
	struct amba_device *adev = to_amba_device(dev);
	struct xgold_dma_driver_data *dma = amba_to_xgold_dma(adev);
	int ret;
	...
	if (!atomic_cmpxchg(&dma->is_suspended, 1, 0))
		return 0;

	ret = xgold_dma_resume(dma->dev);
	if (ret)
		return ret;

	if (dma->pl08x_resume)
		return dma->pl08x_resume(dev);

	if (ret)
		dev_err(dev, "pl08x resume error %d\n", ret);

	return 0;
}

/*
 * adev_dma_suspend: generic pm suspend function for the pl08x dma device
 * This callback is used to shut down the pl08x IP by first calling pl08x
 * suspend callback, then call xgold-wrapper suspend.
 *
 * dev: pl08x dma device pointer
 */
static int adev_dma_suspend(struct device *dev)
{
	struct amba_device *adev = to_amba_device(dev);
	struct xgold_dma_driver_data *dma = amba_to_xgold_dma(adev);
	...
	if (atomic_read(&dma->is_suspended) == 1)
		return 0;

	if (pm_runtime_enabled(dev) && pm_runtime_suspended(dev))
		return 0;

	return adev_dma_runtime_suspend(dev);
}

/*
 * adev_dma_resume: generic pm resume function for the pl08x dma device
 * This callback is used to properly power up the pl08x IP by first calling
 * the xgold-wrapper resume callback, then the pl08x resume function.
 *
 * dev: pl08x dma device pointer
 */
static int adev_dma_resume(struct device *dev)
{
	struct amba_device *adev = to_amba_device(dev);
	struct xgold_dma_driver_data *dma = amba_to_xgold_dma(adev);

	dev_dbg(dev, "--> %s: power %d, suspended %d\n", __func__,
			atomic_read(&adev->dev.power.usage_count),
			atomic_read(&dma->is_suspended));

	if (atomic_read(&dma->is_suspended) == 0)
		return 0;

	if (!pm_runtime_enabled(dev) || !pm_runtime_suspended(dev))
		return adev_dma_runtime_resume(dev);

	return 0;
}

static const struct dev_pm_ops adev_dma_pm_ops = {
	SET_LATE_SYSTEM_SLEEP_PM_OPS(adev_dma_suspend, adev_dma_resume)
	SET_RUNTIME_PM_OPS(adev_dma_runtime_suspend,
			adev_dma_runtime_resume, NULL)
};

#define ADEV_DMA_PM_OPS (&adev_dma_pm_ops)

#else

#define ADEV_DMA_PM_OPS NULL
...
#ifdef CONFIG_PM
	/* overwrite pl08x driver pm ops by xgold ones, to handle power
	 * state transitions (with pm_platdata) from xgold driver */
	dma->pl08x_suspend = adev->dev.driver->pm->suspend;
	dma->pl08x_resume = adev->dev.driver->pm->resume;
	adev->dev.driver->pm = ADEV_DMA_PM_OPS;
#endif

#ifdef CONFIG_PM_RUNTIME
	/* pm_runtime is already enabled, in amba bus probe */
	/* 3sec delay */
	pm_runtime_set_autosuspend_delay(&adev->dev, 3 * MSEC_PER_SEC);
...
static const struct of_device_id xgold_dma_of_match[] = {
