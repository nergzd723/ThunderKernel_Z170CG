			   ...
			   struct pl08x_txd *txd)
{
	...
	pm_runtime_mark_last_busy(&pl08x->adev->dev);
	pm_runtime_put_autosuspend(&pl08x->adev->dev);
#endif
}
...
#ifdef CONFIG_PM_RUNTIME
	ret = pm_runtime_get_sync(&pl08x->adev->dev);
...
#ifdef CONFIG_PM_RUNTIME
	SET_RUNTIME_PM_OPS(pl08x_suspend, pl08x_resume, NULL)
#endif
};
