...
void oct_offlog_config(struct device_node *np, char *cfg_path)
{
	if(strlen(cfg_path) == 0)
	{
		OCT_LOG("set oct_offlog_conf_path to %s", OFFLOG_CONFIG_FILE);
		sprintf(oct_offlog_conf_path, "%s", OFFLOG_CONFIG_FILE);
	}

	oct_detect_off_flag(cfg_path);
	OCT_DBG("oct_detect_off_flag oct_out_path: %d", oct_out_path);

	if (oct_out_path == OCT_PATH_FILE) {
		oct_offlog_get_config(cfg_path);
		if (!oct_offlog_onoff && !oct_offlog_run)
