...
static char oct_offlog_conf_path[256] = {0};
...
static int oct_thread(void *param)
{
	...
	oct_offlog_config(np, oct_offlog_conf_path);
	#endif
		...
		size_t count, loff_t *position)
{
	...
	if (!strncmp(buffer, "stop", 4)) {
		...
		data_mounted = 1;
		OCT_DBG("Data partition mounted");
	} else if (strstr(buffer, "conf") != NULL) {
		sprintf(oct_offlog_conf_path, "%s", buffer);
		OCT_DBG("config file path : %s", oct_offlog_conf_path);
	} else {
		int nb_of_bytes;
