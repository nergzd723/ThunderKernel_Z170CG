...
static void sdhci_do_set_ios(struct sdhci_host *host, struct mmc_ios *ios)
{
    ...
    if (host->vmmc){
        //pr_info("%s:regulator enabled:%d, vddbit:%d\n", mmc_hostname(host->mmc),
        //    regulator_is_enabled(host->vmmc), vdd_bit);
        if (ios->power_mode == MMC_POWER_OFF && regulator_is_enabled(host->vmmc)){
            pr_info("change the vdd bit: %d to 0", vdd_bit);
                vdd_bit = 0;
        }
    }
