				 ...
				 struct rockchip_screen *screen)
{
	...
	pr_err("=rockchip_prase_timing_dt=\n");
	pr_err("panel source = %d\n",panel_source);

	#if defined(CONFIG_Z370C) || defined(CONFIG_Z370CG)
	/*use panel source to load different panel config. CPT=0 BOE=1*/
	int asus_panel_id = asustek_boardinfo_get(FUN_LCM_ID);
	printk("asus_panel_id = %d\n",asus_panel_id);
	switch(asus_panel_id){
		case BOE:
			screen->index = 1;
			pr_err("In %s, use boe screen index = %d\n", __FUNCTION__,screen->index);
			break;
		case CPT:
			screen->index = 0;
			pr_err("In %s, use cpt screen index = %d\n", __FUNCTION__,screen->index);
                        break;
		default:
			screen->index = 0;
			pr_err("In %s, use default cpt screen index = %d\n", __FUNCTION__,screen->index);
	}
	#endif
