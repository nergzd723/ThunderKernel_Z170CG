...
static int rockchip_vop_early_suspend(struct rockchip_vop_driver *dev_drv)
{
	...
	msleep(100);
#else
        cadiz_trigger_brightness_work(0);
#endif 
...
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C) || defined(CONFIG_Z170CG) || defined(CONFIG_Z170C)
	if(entry_mode==4 || entry_mode==2){
		vop_dev->driver.iommu_enabled = 0; //disable iommu in charging mode and recovery mode
		printk("Disable IOMMU in charging mode\n");
	}
#endif
#else
	vop_dev->driver.iommu_enabled = 0;
