...
extern struct xgold_led_data *led_g;
...
#if defined(CONFIG_Z170C) || defined(CONFIG_Z170CG)
        if(entry_mode==4){
	iowrite32(0x104, (char *)led_g->mmio + 0x02C);
        if (gpio_is_valid(led_g->gpio)) {
            gpio_direction_output(led_g->gpio, true);
            iowrite32(true ? 0x3 : 0x0, led_g->cgu_mmio);
            printk("[LED] backlight on!!\n");
		}
	}
#endif
	return 0;
...
#if defined(CONFIG_Z170C) || defined(CONFIG_Z170CG)
	iowrite32(0x200, (char *)led_g->mmio + 0x02C);
    if (gpio_is_valid(led_g->gpio)) {
        gpio_direction_output(led_g->gpio, false);
        iowrite32(false ? 0x3 : 0x0, led_g->cgu_mmio);
        printk("[LED] backlight off!!\n");
    }
#endif
	return 0;
}
