...
//Author:Javid 20150305
//Z370C or Z370CG bit rate need to meet cadiz so didn't need to be overhead
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
...
static void dsi_panel_power_on(struct dsi_display *display)
{
        pr_info("%s -->\n", __func__);
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
        pr_info("[JAVID] %s ===== cadiz power on sequence 1 =====\n", __func__);
        /* +++ cadiz power on sequence 1 +++ */
        cadiz_power(1);
        lcm_vhigh_on_off(1);
        cadiz_sysclk_power(1);
        msleep(20);
        cadiz_reset(1);
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
        lcm_vhigh_on_off(0);
        cadiz_sysclk_power(0);
        cadiz_reset(0);
        cadiz_power(0);
#endif
}

static void dsi_panel_reset(struct dsi_display *display)
{
        pr_info("%s -->\n", __func__);
	if (display->gpios_reset)
		dsi_set_gpiolist(display, display->gpios_reset);
}
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
        pr_info("[JAVID] %s ===== cadiz power on sequence 2 =====\n", __func__);
	/* +++ cadiz power on sequence 2 +++ */
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
        pr_info("[JAVID] %s ===== cadiz power on sequence 3 =====\n", __func__);
        /* +++ cadiz power on sequence 3 +++ */
                        cadiz_setup_boot3();
                        cadiz_setup_boot4();
                        cadiz_wait_tx_init_done();
                        cadiz_setup_boot4();
...
static void dsi_dphy_calculation(struct dsi_display *display)
{
	 ...
	 * margin.
	 */
	ths_trail_ns = DIV_ROUND_UP(70000 + 4 * ui_ps, 1000);
