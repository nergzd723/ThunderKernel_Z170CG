...
#define PROP_DISPLAY_KD			"asus-kd,display"
#define PROP_DISPLAY_BOE		"asus-boe,display"
...
#define GPIO_LIST_RESET         "gpio-reset"
...
#define PROP_DISPLAY_CMDUDELAY   "intel,cmd-udelay"
#endif
...
static struct of_device_id sofia_display_of_match_kd[] = {
	{ .compatible = PROP_DISPLAY_KD, },
	{ },
};

static struct of_device_id sofia_display_of_match_boe[] = {
	{ .compatible = PROP_DISPLAY_BOE, },
	{ },
};
#endif
...
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
        ret = of_property_read_u32(n, PROP_DISPLAY_CMDUDELAY, &cmd->udelay);
        if (ret)
                cmd->udelay = 0;
#endif
			 ...
			 struct xgold_mipi_dsi_device *mipi_dsi)
{
	...
	printk("Lance asus_panel_id = %d\n",asus_panel_id);

	switch(asus_panel_id){

		case KD:
			display_dev_n = of_find_matching_node(NULL, sofia_display_of_match_kd);
			printk("Lance load kd sequence \n");
			break;
		case BOE_FOCAL:
			display_dev_n = of_find_matching_node(NULL, sofia_display_of_match_boe);
			printk("Lance load boe sequence \n");
			break;
		case HH_HIMAX:
                        display_dev_n = of_find_matching_node(NULL, sofia_display_of_match_boe);
                        printk("Lance load HH sequence \n");
                        break;
		default:
			display_dev_n = of_find_matching_node(NULL, sofia_display_of_match_boe);
			printk("Lance can not load LCD ID , load boe sequence for default \n");
	}
#else
	display_dev_n = of_find_matching_node(NULL, sofia_display_of_match);
#endif
	
	for_each_matching_node(display_dev_n, sofia_display_of_match) {
	...
	for_each_child_of_node(display_dev_n, child) {
		...
		} else if (!strcmp(child->name, GPIO_LIST_RESET)) {
			ret = dsi_of_parse_display_gpiolist(pdev, child,
				&display->gpios_reset);
