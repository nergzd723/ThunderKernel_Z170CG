...
#include "ASUS_Z170_3427_BOE_0x58_0xA7_20150728.h"
};
#endif

#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
static unsigned char CTPM_FW[] = {
#include "ASUS_Z370_3517_CPT_0x08_0xA2_20150705.h"
};
static unsigned char CTPM_FW_BOE[] = {
#include "ASUS_Z370_3517_BOE_0x02_0x22_20150625.h"
};
#endif
...
upgrade with *.i file
*/
#define FTS_RETRY_UPDATE_FW_WITH_I_FILE	5
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	if (ftxxxx_ts->vendor_id == FTXXXX_VAL_VENID_CPT){
	    printk("[Focal][Touch] %s: vendor ID:0x%02x, vendor ID is CTP\n", __func__, ftxxxx_ts->vendor_id);
            fw_len = sizeof(CTPM_FW);
	}else if(ftxxxx_ts->vendor_id == FTXXXX_VAL_VENID_BOE){
	    printk("[Focal][Touch] %s: vendor ID:0x%02x, vendor ID is BOE\n", __func__, ftxxxx_ts->vendor_id);
            fw_len = sizeof(CTPM_FW_BOE);
	}else{
	    ftxxxx_ts->update_check = 0;
	    fw_len = sizeof(CTPM_FW_BOE);
            printk("[Focal][Touch] %s: vendor ID error, fw_len\n", __func__);
	}
#endif
	...
	*/
	...
	if (ftxxxx_ts->vendor_id  == FTXXXX_VAL_VENID_CPT){
	    ...
	    ftxxxx_ts->update_check = 0;
	    pbt_buf = CTPM_FW_BOE;
            printk("[Focal][Touch] %s: vendor ID error, pbt_buf  \n", __func__);
	}
#endif
	/*call the upgrade function*/

RETRY_UPDATE_FW_WITH_I_FILE:
#if defined(CONFIG_Z170CG) || defined(CONFIG_Z170C)
...
#endif
	if (i_ret != 0) {
		dev_err(&client->dev, "[Focal][TOUCH_ERR] %s : Touch FW update Fail, err = %d, retry= %d \n", __func__, i_ret, retry);
		if (retry < FTS_RETRY_UPDATE_FW_WITH_I_FILE){
			retry++;
			goto RETRY_UPDATE_FW_WITH_I_FILE;
		}
		return i_ret;
	} else {
#ifdef AUTO_CLB
...
u8 fts_ctpm_get_i_file_ver(void)
{
	...
	if (ftxxxx_ts->vendor_id == FTXXXX_VAL_VENID_CPT){
	    ...
	    ui_sz = sizeof(CTPM_FW_BOE);
            ...
            ftxxxx_ts->update_check = 0;
	    ui_sz = sizeof(CTPM_FW_BOE);
            printk("[Focal][Touch] %s: vendor ID error, ui_sz \n", __func__);
	}
	...
	if (ui_sz > 2) {
		if (ftxxxx_ts->vendor_id == FTXXXX_VAL_VENID_CPT){
		    return CTPM_FW[ui_sz - 2];
                    ...
                    ftxxxx_ts->update_check = 0;
		    return CTPM_FW_BOE[ui_sz - 2];
                    printk("[Focal][Touch] %s: vendor ID error, ui_sz -2 \n", __func__);
		}
#endif
	} else {
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	u8 uc_tp_fm_ver;
	u8 uc_tp_upgrade_flg = 0; // 1 upgrade, 0 do not
	int i_ret;
	...
	printk("[Focal][Touch] uc_host_fm_ver: 0x%02x\n", uc_host_fm_ver);
...
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
        //confirm vendor ID ok 
        if (ftxxxx_ts->update_check == 0){
	    printk("[Focal][Touch] %s : vendor ID and lcm id mismatch, check again! \n", __func__);
	    unsigned char reg_addr;
	    unsigned char reg_value;
	    int i2c_err;
            reg_addr = FTXXXX_REG_VENDOR_ID;
	    msleep(50);
	    i2c_err = ftxxxx_i2c_Read(client, &reg_addr, 1, &reg_value, 1);
	    if (i2c_err < 0){
	        printk("[Focal][Touch] FW update but VENDOR ID read fail \n");
	    } else {
		ftxxxx_ts->vendor_id = reg_value;
	    }
        ...
        printk("[Focal][Touch] %s: VENDOR ID = 0x%x, LCM ID = %d \n", __func__ ,ftxxxx_ts->vendor_id, ftxxxx_ts->lcm_id);

	if (uc_tp_fm_ver != uc_host_fm_ver && (ftxxxx_ts->update_check) ) 
#endif
	...
	if (uc_tp_fm_ver < uc_host_fm_ver){
		uc_tp_upgrade_flg = 1;
	} else if (uc_tp_fm_ver > uc_host_fm_ver){
		printk("[Focal][Touch] %s: update Touch FW fail last time, need force update. \n", __func__);
		/*********Step 2:Enter StdI2c mode *****/
		i_ret = HidI2c_To_StdI2c(client);
            ...
            if (reg_val[0] == 0X54 && reg_val[1] == 0X2C){
                uc_tp_upgrade_flg = 1;
                printk("[Focal][Touch] %s: Start force update Touch FW. \n", __func__);
            }
			}

    }
  if (uc_tp_upgrade_flg == 1)
#endif

	{
		...
		if (i_ret == 0) {
			mdelay(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			printk("[Focal][Touch] %s : TPUPGRADESUCCESS upgrade to new version 0x%x \n", __func__, uc_host_fm_ver);
		} else {
			printk("[Focal][TOUCH_ERR] %s : TPUPGRADEFAIL ret=%d. \n", __func__, i_ret);
		}
...
int fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		i_ret = HidI2c_To_StdI2c(client);
		if (i_ret == 0) {
			printk("[TOUCH_ERR] %s : HidI2c change to StdI2c fail ! \n", __func__);
			FTS_DBG("[TOUCH_ERR] %s : HidI2c change to StdI2c fail ! \n", __func__);
			continue;
		}
		mdelay(5);
		...
		if (i_ret < 0) {
			printk("[TOUCH_ERR] %s : failed writing  0x55 and 0xaa ! \n", __func__);
		...
		if (reg_val[0] == upgradeinfo.upgrade_id_1 && reg_val[1] == upgradeinfo.upgrade_id_2) {	/*asus : relate on bootloader FW*/
			printk("[Touch] %s : Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x \n", __func__, reg_val[0], reg_val[1]);
			FTS_DBG("[Touch] %s : Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x \n", __func__, reg_val[0], reg_val[1]);
			break;
		} else {
			printk("[Focal][TOUCH_ERR] %s :  Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x \n", __func__, reg_val[0], reg_val[1]);
			dev_err(&client->dev, "[Focal][TOUCH_ERR] %s :  Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x \n", __func__, reg_val[0], reg_val[1]);
			continue;
		}
	}
	if (i >= FTS_UPGRADE_LOOP) {
		printk("[Focal][TOUCH_ERR] Update FW fail!\n");
		return -EIO;
	}
	if (i >= FTS_UPGRADE_LOOP)
		return -EIO;
...
*/
#define FTS_RETRY_UPDATE_FW_WITH_APP_FILE 5
...
RETRY_UPDATE_FW_WITH_APP_FILE:
	i_ret =  fts_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	if (i_ret != 0) {
		if (retry < FTS_RETRY_UPDATE_FW_WITH_APP_FILE){
			retry++;
			printk("%s() - ERROR:[FTS] upgrade failed i_ret = %d, retry = %d \n", __FUNCTION__, i_ret, retry);
			goto RETRY_UPDATE_FW_WITH_APP_FILE;
		}
		printk("%s() TPUPGRADEFAIL", __FUNCTION__);
		return i_ret;
	} else {
#ifdef AUTO_CLB
		fts_ctpm_auto_clb(client);  /*start auto CLB*/
	...
	printk("%s() TPUPGRADESUCCESS", __FUNCTION__);
	kfree(pbt_buf);
	return i_ret;
}
...
#define ASUS_ITR_STATUS_RETRY	5
static ssize_t asus_itr_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	int i_ret;
	int tmp = 0;
	int retry = 0;
retry_asus_itr_status:
	i_ret = ftxxxx_write_reg(client, 0x00, FT_INTERRUPT_40);
	...
	i_ret = ftxxxx_write_reg(client, 0x08, FT_INTERRUPT_01);
	...
	if (gpio_get_value(ftxxxx_ts->irq)) {
		if(retry < ASUS_ITR_STATUS_RETRY){
			printk("[TOUCH_ERR] Set interrupt low fail, ready to reset ic");
			ftxxxx_irq_switch(false);
			/* place holder for future use */
			ftxxxx_reset_tp(0);
			msleep(10);
		...
		printk("[TOUCH_ERR] Set interrupt low fail, retry = %d \n", retry);
		retry++;
		ftxxxx_ts->int_flag = false;
		goto retry_asus_itr_status;
	}
	else {
		printk("Set interrupt low successfully\n");
		ftxxxx_ts->int_flag = true;
	}

	return sprintf(buf, "%d\n", tmp);
}

static ssize_t asus_itr_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ftxxxx_ts_data *data = dev_get_drvdata(dev);
	return sprintf(buf, "%s\n", (data->int_flag == true) ? "0" : "1");
}
...
static ssize_t fw_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	printk("[Focal][Touch] FW vendor ID = %x , FW project code = %x , Bootloader PRJ code = %s \n", F_VenderID, F_projectcode, B_projectcode);

	return sprintf(buf, "%x-%x-%s\n", F_VenderID, F_projectcode, B_projectcode);
}
...
{
	struct ftxxxx_ts_data *data = dev_get_drvdata(dev);
	return sprintf(buf, "%s\n", (data->auto_update_fw_result == 0) ? "1" : "0");
}
...
*/
	...
	data->auto_update_fw_result = fts_ctpm_auto_upgrade(client);
	ftxxxx_irq_switch(true);
	mutex_unlock(&ftxxxx_ts->g_device_mutex);
	wake_unlock(&ftxxxx_ts->wake_lock);
	return count;
...
static ssize_t enable_print_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ftxxxx_ts_data *data = dev_get_drvdata(dev);
	size_t count = 0;
	static unsigned int value;
	value = cat_event_printable();
	return count += sprintf(buf, "%d\n", value);
}
...
static ssize_t touch_dclick_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ftxxxx_ts_data *data = dev_get_drvdata(dev);
	return sprintf(buf, "%s\n", (data->dclick == true) ? "1" : "0");
}

static ssize_t touch_dclick_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct ftxxxx_ts_data *data = dev_get_drvdata(dev);
	if (buf[0] == '0')
	{
        ...
        printk("[Focal][Touch] dclick: %d \n", data->dclick);

	return count;
}

static ssize_t touch_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ftxxxx_ts_data *data = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", data->gesture_all);
}

static ssize_t touch_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("[Focal][Touch] gesture all:%d, wec:0x%x, s:%d, z:%d, v:%d\n", data->gesture_all, data->gesture_wec, data->gesture_s, data->gesture_z, data->gesture_v);

	return count;
}

#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
static ssize_t lcm_match_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
static ssize_t lcm_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        if ( ftxxxx_ts->lcm_id == CPT){
	    printk("[Focal][Touch]get panel : CPT\n");
            return sprintf(buf, "CPT\n");
        }else if(ftxxxx_ts->lcm_id == BOE){
            printk("[Focal][Touch]get panel : BOE\n");
            return sprintf(buf, "BOE\n");
        }else{
            printk("[Focal][Touch]get panel : can't identify\n");
            return sprintf(buf, "panel can't identify\n");
        }
}
...
static ssize_t vendor_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	u8 vendorID = 0;
        ...
        ftxxxx_read_reg(ftxxxx_ts->client, FTXXXX_REG_VENDOR_ID, &vendorID);
        printk("[Focal][Touch]get vendor ID:%d\n",vendorID);
...
static DEVICE_ATTR(INR_STATUS, Focal_RW_ATTR, asus_itr_show, asus_itr_store);
...
static DEVICE_ATTR(ftprintevent, Focal_RW_ATTR, enable_print_show, enable_print_store);
static DEVICE_ATTR(touch_dclick, Focal_RW_ATTR, touch_dclick_show, touch_dclick_store);
static DEVICE_ATTR(touch_gesture, Focal_RW_ATTR, touch_gesture_show, touch_gesture_store);
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
static DEVICE_ATTR(lcm_id, Focal_RW_ATTR, lcm_id_show, NULL);
static DEVICE_ATTR(vendor_id, Focal_RW_ATTR, vendor_id_show, NULL);
static DEVICE_ATTR(ven_lcm_match, Focal_RW_ATTR, lcm_match_show, NULL);
#endif
