...
#define BMA_IRQ   "bosch,bma2x2-gpio-irq"
/* Calibration Constants */
#define GSENSOR_CALIBRATION_FILE_PATH	"/factory/accel_cal_data.ini"
static int asus_init_calibration_data(struct i2c_client *client);
extern unsigned int entry_mode;

#define SENSOR_NAME                 "bma2x2"
#if defined (PZ170C) || defined (PZ170CG)
#define OF_SENSOR_NAME              "bosch,bma2x2,sr"
#define OF_SENSOR_NAME_SR           "bosch,bma2x2,sr"
#define OF_SENSOR_NAME_ER2          "bosch,bma2x2,er2"
#define OF_PLACE                    "bmm2x2,place"
#define OF_GPIO_IRQ                 "bma2x2,bma2x2-gpio-irq"
#else
#define OF_SENSOR_NAME              "bosch,bmc156,sr"
#define OF_SENSOR_NAME_SR           "bosch,bmc156,sr"
#define OF_SENSOR_NAME_ER2          "bosch,bmc156,er2"
#define OF_PLACE                    "bmc156,place"
#define OF_GPIO_IRQ                 "bmc156,bmc156-gpio-irq"
#endif
...
#define HIGH_G_INTERRUPT2           REL_X
...
#define LOW_G_INTERRUPT_HAPPENED_FACE_UP            27
#define LOW_G_INTERRUPT_HAPPENED_FACE_DOWN          28
#define TAP_INTERRUPT_UNKNOW                        29
#define SENSORS_EVENT_INIT                          30
#define HIGH_G_FLICK_ONCE		                     31
#define HIGH_G_FLICK_TWICE     		                 32
#define FLAT_INTERRUPT_TURE_HAPPENED_AND_UP         33
#define FLAT_INTERRUPT_TURE_HAPPENED_AND_DOWN       34
#define HIGH_G_FLICK_UNKNOW    		                 35
#define HIGH_G_FLICKHV_ONCE		                     36
#define HIGH_G_FLICKHV_TWICE     		             37
#define HIGH_G_FLICKHV_UNKNOW    		             38
...
#ifdef CONFIG_HIGH_G
#define DEFAULT_HG_JUDGE_PERIOD 1000    /* default judge in 1 second */
#define DEFAULT_HG_DEBOUNCE_PERIOD 500    /* default judge in 0.5 second */
#endif
...
#define BMA_ENABLED_LOWG 5
#define BMA_ENABLED_HIGHG 6
#define BMA_ENABLED_ORIENTATION 7
#define BMA_ENABLED_FLAT 8
...
struct bma2x2_data {
	...
	struct bosch_sensor_specific *apdata;
	...
	struct timer_list	tap_timer2;
	int tap_time_period;
	atomic_t last_flat;
#endif
#ifdef CONFIG_HIGH_G
	atomic_t en_high_g;
	/* filter continues shack*/
	atomic_t slop_x;
	atomic_t slop_y;
	struct mutex		shack_mutex;
	struct timer_list	shack_timer;
	struct timer_list	shack_timer2;
	atomic_t shack_times;
	int shack_time_period;
	struct timeval start,middle,end;
#endif
#ifdef CONFIG_LOW_G
	atomic_t en_low_g;
#endif
#ifdef CONFIG_ORIENTATION
	atomic_t en_orientation;
	struct delayed_work flat_work;
	struct workqueue_struct *flat_wq;
#endif
};
		...
		for (i = 0; i < bma2x2_sensor_type_count; i++) {
			if (sensor_type_map[i].chip_id == chip_id) {
				...
				PINFO("Bosch Sensortec Device detected,"
					...
					data->apdata->name = sensor_type_map[i].sensor_name;
					return err;
			}
		}
						...
						unsigned char enabled_mode)
{
	...
	if((mode != BMA2X2_MODE_SUSPEND) && (mode != BMA2X2_MODE_DEEP_SUSPEND)){
		/*work around sometimes daemon set range failed*/
		bma2x2_set_range(bma2x2->bma2x2_client,
									BMA2X2_RANGE_4G);
	}
...
static void bma2x2_work_func(struct work_struct *work)
{
	...
	//update calibration data offset
	mutex_lock(&bma2x2->value_mutex);
	acc.x += bma2x2->offset_x;
	acc.y += bma2x2->offset_y;
	acc.z += bma2x2->offset_z;
...
static ssize_t asus_update_cali_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

	/* load the calibration data from factory partition
	 * force to load calibration data and set to offset sysfs
	 * call by calibration data
	 */
	int ret = 0;
	ret = asus_init_calibration_data(bma2x2->bma2x2_client);


	return sprintf(buf, "%d\n", ret);
}

static ssize_t asus_update_cali_store(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
	...
	/** load the calibration data from factory partition
	 *  call by HAL to do first time init
	 *  return 1 success of load and set
	 *        -1 some error with load/set value
	 *         0 already loaded
	 * */
	int ret = 0;
//	if(!atomic_read(&bma2x2->is_offset_inited)){
	ret = asus_init_calibration_data(bma2x2->bma2x2_client);
//		if(ret > 0){
//			mutex_lock(&bma2x2->value_mutex);
//			atomic_set(&bma2x2->is_offset_inited,1);
//			mutex_unlock(&bma2x2->value_mutex);
//		}
//	}
...
static ssize_t asus_chip_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	if (enable) {
		...
		/* update calibration only one time*/
		if(!atomic_read(&bma2x2->is_offset_inited)){
			if(asus_init_calibration_data(bma2x2->bma2x2_client) > 0){
				mutex_lock(&bma2x2->value_mutex);
				atomic_set(&bma2x2->is_offset_inited,1);
				mutex_unlock(&bma2x2->value_mutex);
			}
...
/* please give attation for the fifo output data format*/
	if (f_len == 6) {
		...
		for (i = 0; i < bma2x2->fifo_count; i++) {
			...
			mutex_lock(&bma2x2->value_mutex);
			acc_lsb.x += bma2x2->offset_x;
			acc_lsb.y += bma2x2->offset_y;
			acc_lsb.z += bma2x2->offset_z;
			mutex_unlock(&bma2x2->value_mutex);
...
//		bma2x2_set_tap_duration(client, 0xc0);
		err = bma2x2_set_tap_threshold(client, 0x01);
		...
		int en)
{
	...
	if (atomic_read(&bma2x2->en_double_tap) != en) {
		if (en) {
			...
			err = bma2x2_set_bandwidth(bma2x2->bma2x2_client, BMA2X2_BW_125HZ);
		} else {
			err = bma2x2_set_en_single_tap_int(bma2x2, en);
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
...
static void bma2x2_tap_timeout_handle2(unsigned long data)
{
	struct bma2x2_data *bma2x2 = (struct bma2x2_data *)data;

	PINFO("tap interrupt handle2, timeout2\n");
	atomic_set(&bma2x2->tap_times, 0);
...
static int bma2x2_set_en_high_g_int(struct bma2x2_data *bma2x2, int en)
{
	int err;
	struct i2c_client *client = bma2x2->bma2x2_client;

	if (en) {
		/* set high-g interruption parameter here if needed.*/
		/* not really know why it act differently on different platform*/

        /* 
         * from planner's info ,it should be set to 50(0.8g) ,but there is a HW mis judge 
         * for Z170 when set less than 70 when portrait and landscape the device
         * */
		err += bma2x2_set_high_g_threshold(bma2x2->bma2x2_client, (unsigned
				char)100);
		err += bma2x2_set_high_g_duration(bma2x2->bma2x2_client, (unsigned
				char)50);

		/*Enable the high g interrupts*/
		err = bma2x2_set_Int_Enable(client, 1, 1); //X
		err += bma2x2_set_Int_Enable(client, 2, 1); //Y
//		err += bma2x2_set_Int_Enable(client, 3, 1); //Z
		...
		err += bma2x2_set_Int_Enable(client, 1, 0); //X
		err += bma2x2_set_Int_Enable(client, 2, 0); //Y
//		err += bma2x2_set_Int_Enable(client, 3, 0); //Z
	}
	return err;
}

static ssize_t bma2x2_en_high_g_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

	return sprintf(buf, "%d\n", atomic_read(&bma2x2->en_high_g));
}

static int bma2x2_set_en_high_g(struct bma2x2_data *bma2x2,
		int en)
{
	int err = 0;
	...
	if (atomic_read(&bma2x2->en_high_g) != en) {
		if (en) {
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_NORMAL, BMA_ENABLED_HIGHG);
			err = bma2x2_set_en_high_g_int(bma2x2, en);
		} else {
			err = bma2x2_set_en_high_g_int(bma2x2, en);
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_SUSPEND, BMA_ENABLED_HIGHG);
		}
		atomic_set(&bma2x2->en_high_g, en);
	}
	return err;
}

static ssize_t bma2x2_en_high_g_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if ((data == 0) || (data == 1))
		bma2x2_set_en_high_g(bma2x2, data);
...
static void bma2x2_shack_timeout_handle(unsigned long data)
{
	struct bma2x2_data *bma2x2 = (struct bma2x2_data *)data;

	PINFO("shack interrupt handle, timeout\n");
//	mutex_lock(&bma2x2->shack_mutex);
//	bma2x2->shack_times = 0;
	atomic_set(&bma2x2->shack_times,0);
//	mutex_unlock(&bma2x2->shack_mutex);
...
static void bma2x2_shack_timeout_handle2(unsigned long data)
{
	struct bma2x2_data *bma2x2 = (struct bma2x2_data *)data;

	PINFO("shack interrupt handle2, timeout\n");
//	del_timer(&bma2x2->shack_timer);
//	mutex_lock(&bma2x2->shack_mutex);
//	bma2x2->shack_times = 0;
//	mutex_unlock(&bma2x2->shack_mutex);
	atomic_set(&bma2x2->shack_times,0);
...
#ifdef CONFIG_LOW_G
static int bma2x2_set_en_low_g_int(struct bma2x2_data *bma2x2, int en)
{
	int err;
	struct i2c_client *client = bma2x2->bma2x2_client;

	if (en) {
		/* set low-g interruption parameter here if needed.*/

		/*Enable the high g interrupts*/
		err = bma2x2_set_Int_Enable(client, 0, 1);
	#ifdef BMA2X2_ENABLE_INT1
		...
		err = bma2x2_set_Int_Enable(client, 0, 0);
	}
	return err;
}


static ssize_t bma2x2_en_low_g_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

	return sprintf(buf, "%d\n", atomic_read(&bma2x2->en_low_g));
}

static int bma2x2_set_en_low_g(struct bma2x2_data *bma2x2,
		int en)
{
	int err = 0;
	...
	if (atomic_read(&bma2x2->en_low_g) != en) {
		if (en) {
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_NORMAL, BMA_ENABLED_LOWG);
			err = bma2x2_set_en_low_g_int(bma2x2, en);
		} else {
			err = bma2x2_set_en_low_g_int(bma2x2, en);
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_SUSPEND, BMA_ENABLED_LOWG);
		}
		atomic_set(&bma2x2->en_low_g, en);
	}
	return err;
}

static ssize_t bma2x2_en_low_g_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if ((data == 0) || (data == 1))
		bma2x2_set_en_low_g(bma2x2, data);
...
#ifdef CONFIG_ORIENTATION
static int bma2x2_set_en_orientation_int(struct bma2x2_data *bma2x2, int en)
{
	int err;
	struct i2c_client *client = bma2x2->bma2x2_client;
	unsigned char flat_theta = 20;
	unsigned char flat_hold_time = 0;

	if (en) {
		/* set orientation interruption parameter here if needed.*/
//		err = bma2x2_set_orient_mode(client,2);
//		err += bma2x2_set_orient_blocking(client,3);
		err += bma2x2_set_bandwidth(bma2x2->bma2x2_client, BMA2X2_BW_125HZ);
//		err += bma2x2_set_flat_hold_time(bma2x2->bma2x2_client, (unsigned
//					char)data);
		err += bma2x2_set_theta_flat(bma2x2->bma2x2_client, (unsigned
					char)flat_theta);
		err += bma2x2_set_flat_hold_time(bma2x2->bma2x2_client, (unsigned
					char)flat_hold_time);
		err += bma2x2_set_Int_Mode(bma2x2->bma2x2_client,0);

		/*Enable the orientation interrupts*/
...
//		err = bma2x2_set_Int_Enable(client, 10, 0);
		...
		err += bma2x2_set_Int_Mode(bma2x2->bma2x2_client,1);
...
static ssize_t bma2x2_en_orientation_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

	return sprintf(buf, "%d\n", atomic_read(&bma2x2->en_orientation));
}

static int bma2x2_set_en_orientation(struct bma2x2_data *bma2x2,
		int en)
{
	int err = 0;
	...
	if (atomic_read(&bma2x2->en_orientation) != en) {
		if (en) {
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_NORMAL, BMA_ENABLED_ORIENTATION | BMA_ENABLED_FLAT);
			err = bma2x2_set_en_orientation_int(bma2x2, en);

			//filter case that face down and ring/music comes
		    atomic_set(&bma2x2->last_flat, (int)FLAT_INTERRUPT_TURE_HAPPENED_AND_DOWN);

		} else {
			err = bma2x2_set_en_orientation_int(bma2x2, en);
			err = bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_SUSPEND, BMA_ENABLED_ORIENTATION | BMA_ENABLED_FLAT);
		}
		atomic_set(&bma2x2->en_orientation, en);
	}
	return err;
}

static ssize_t bma2x2_en_orientation_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if ((data == 0) || (data == 1))
		bma2x2_set_en_orientation(bma2x2, data);
...
static void flat_work_handler(struct work_struct *w)
{
	struct bma2x2_data *bma2x2 = container_of(w, struct bma2x2_data, flat_work);
    int ret;
    int value;
	...
	if (sign_value == 1) {
		...
		bma2x2_read_accel_xyz(bma2x2->bma2x2_client, bma2x2->sensor_type,
									&acc_value);
		if(acc_value.z > 0){
			if(atomic_read(&bma2x2->last_flat) != (int)FLAT_INTERRUPT_TURE_HAPPENED_AND_UP){
				input_report_abs(bma2x2->dev_interrupt,
					FLAT_INTERRUPT,
					FLAT_INTERRUPT_TURE_HAPPENED_AND_UP);
					atomic_set(&bma2x2->last_flat,(int)FLAT_INTERRUPT_TURE_HAPPENED_AND_UP);
				}
		}else if(acc_value.z < 0){
			if(atomic_read(&bma2x2->last_flat) != (int)FLAT_INTERRUPT_TURE_HAPPENED_AND_DOWN){
				input_report_abs(bma2x2->dev_interrupt,
					FLAT_INTERRUPT,
					FLAT_INTERRUPT_TURE_HAPPENED_AND_DOWN);
					atomic_set(&bma2x2->last_flat,(int)FLAT_INTERRUPT_TURE_HAPPENED_AND_DOWN);
			}
		}else{
			if(atomic_read(&bma2x2->last_flat) != (int)FLAT_INTERRUPT_TURE_HAPPENED){
				input_report_abs(bma2x2->dev_interrupt,
					FLAT_INTERRUPT,
					FLAT_INTERRUPT_TURE_HAPPENED);
					atomic_set(&bma2x2->last_flat,(int)FLAT_INTERRUPT_TURE_HAPPENED);
				}
		}
		input_sync(bma2x2->dev_interrupt);

	} else {
		if(atomic_read(&bma2x2->last_flat) != (int)FLAT_INTERRUPT_FALSE_HAPPENED){
			input_report_abs(bma2x2->dev_interrupt,
				FLAT_INTERRUPT,
				FLAT_INTERRUPT_FALSE_HAPPENED);
			atomic_set(&bma2x2->last_flat,(int)FLAT_INTERRUPT_FALSE_HAPPENED);
		}
...
static int asus_init_calibration_data(struct i2c_client *client)
{
	...
	//let kernel bypass memory check
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	fp = filp_open(GSENSOR_CALIBRATION_FILE_PATH, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		PERR("BOSCH : filp_open calibration file error, error code:%ld!\n", IS_ERR(fp));
		return -1;
	}
	...
	if (len <= 0) {
		PERR("BOSCH : len=%d, cannot be set to cali!\n", len);
		return -1;
	}

	// Split string by " " from buf[] to buf2[][];
	buf2 = (char **)kzalloc(cali_count * sizeof(char *), GFP_KERNEL);
	if (asus_strtok(buf, (size_t)len, buf2, cali_count) < 0)
		PERR("BOSCH : asus_strtok error!\n");
	else {
		...
		for (i=0; i<cali_count; ++i) {
			...
			if (err < 0) {
				PERR("BOSCH : astrtoint returned err = %d!\n", err);
				goto exit;
			}
		}
		...
		mutex_lock(&bma2x2->value_mutex);
		bma2x2->offset_x = (int)calibration[0];
		bma2x2->offset_y = (int)calibration[1];
		bma2x2->offset_z = (int)calibration[2];
		mutex_unlock(&bma2x2->value_mutex);
	}
...
#endif
#ifdef CONFIG_HIGH_G
static DEVICE_ATTR(en_high_g, S_IRUGO|S_IWUSR|S_IWGRP,
		bma2x2_en_high_g_show, bma2x2_en_high_g_store);
#endif
#ifdef CONFIG_LOW_G
static DEVICE_ATTR(en_low_g, S_IRUGO|S_IWUSR|S_IWGRP,
		bma2x2_en_low_g_show, bma2x2_en_low_g_store);
#endif
#ifdef CONFIG_ORIENTATION
static DEVICE_ATTR(en_orientation, S_IRUGO|S_IWUSR|S_IWGRP,
		bma2x2_en_orientation_show, bma2x2_en_orientation_store);
#endif
static DEVICE_ATTR(update_cali, S_IRUGO|S_IWUSR|S_IWGRP,
		asus_update_cali_show, asus_update_cali_store);
static DEVICE_ATTR(gsensor_state, S_IRUGO,
		asus_chip_id_show, NULL);
...
#endif
#ifdef CONFIG_HIGH_G
	&dev_attr_en_high_g.attr,
#endif
#ifdef CONFIG_LOW_G
	&dev_attr_en_low_g.attr,
#endif
#ifdef CONFIG_ORIENTATION
	&dev_attr_en_orientation.attr,
#endif
	&dev_attr_update_cali.attr,
	&dev_attr_gsensor_state.attr,
...
static struct attribute *bma2x2_asus[] = {
	&dev_attr_gsensor_state.attr,
	&dev_attr_value.attr,
	&dev_attr_update_cali.attr,
	&dev_attr_place.attr,
...
static struct attribute_group bma2x2_asus_group = {
	.attrs = bma2x2_asus
};
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
	...
	if ((status&0x80) == 0x80) {
//		PINFO("New data interrupt happened status = 0x%x\n", status);
...
/* use single tap INT to simulate double tap */
#ifdef CONFIG_DOUBLE_TAP
	if (status & 0x20) {
		if (atomic_read(&bma2x2->en_double_tap) == 1) {
			PINFO("single tap interrupt happened\n");
			bma2x2_set_Int_Enable(client, 8, 0);
			if (atomic_read(&bma2x2->tap_times) == 0)	{
				PINFO("stap simulate dtap \%d",atomic_read(&bma2x2->tap_times));
				mod_timer(&bma2x2->tap_timer, jiffies +
				msecs_to_jiffies(bma2x2->tap_time_period));
				atomic_set(&bma2x2->tap_times, 1);
			} else if(atomic_read(&bma2x2->tap_times) == 1){
				/* only double tap is judged */
				PINFO("dtap simulate dtap \%d",atomic_read(&bma2x2->tap_times));
...
//					DOUBLE_TAP_INTERRUPT_HAPPENED);
//				input_sync(bma2x2->dev_interrupt);
				mod_timer(&bma2x2->tap_timer2, jiffies +
				msecs_to_jiffies(bma2x2->tap_time_period/2));
				atomic_set(&bma2x2->tap_times, 2);
			} else {
				/* outside of double tap */
				PINFO("dtap simulate dtap \%d",atomic_read(&bma2x2->tap_times));
				mutex_lock(&bma2x2->tap_mutex);
				atomic_set(&bma2x2->tap_times, 0);
				del_timer(&bma2x2->tap_timer2);
				del_timer(&bma2x2->tap_timer);
				mutex_unlock(&bma2x2->tap_mutex);
				input_report_rel(bma2x2->dev_interrupt,
					DOUBLE_TAP_INTERRUPT,
					TAP_INTERRUPT_UNKNOW);
				input_sync(bma2x2->dev_interrupt);
...
#ifdef CONFIG_HIGH_G
	if (status & 0x2) {
		if (atomic_read(&bma2x2->en_high_g) == 1 && atomic_read(&bma2x2->slop_x)==1 && atomic_read(&bma2x2->slop_y)==1 ){
			bma2x2_set_Int_Enable(client, 1, 0);
			bma2x2_set_Int_Enable(client, 2, 0);
			bma2x2_set_Int_Enable(client, 3, 0);
			if (atomic_read(&bma2x2->shack_times) == 0)	{
				PINFO("shack 1 happened\n");
				do_gettimeofday(&bma2x2->start);
				mod_timer(&bma2x2->shack_timer, jiffies +
								msecs_to_jiffies(bma2x2->shack_time_period));
				atomic_set(&bma2x2->shack_times,1);
			}else if (atomic_read(&bma2x2->shack_times) == 1){
				do_gettimeofday(&bma2x2->middle);
				/* avoid shack back miss-judged */
				if((1000*(bma2x2->middle.tv_sec-bma2x2->start.tv_sec) +
						(bma2x2->middle.tv_usec-bma2x2->start.tv_usec)/1000) > DEFAULT_HG_DEBOUNCE_PERIOD)
				{
					PINFO("shack 2 happened\n");
					del_timer(&bma2x2->shack_timer);
					mod_timer(&bma2x2->shack_timer2, jiffies +
					msecs_to_jiffies(bma2x2->shack_time_period/2));
					atomic_set(&bma2x2->shack_times,2);
				}
			}else{
				/* outside of double shack */
				do_gettimeofday(&bma2x2->end);
				/* avoid shack back miss-judged */
				if((1000*(bma2x2->end.tv_sec-bma2x2->middle.tv_sec) +
						(bma2x2->end.tv_usec-bma2x2->middle.tv_usec)/1000) > DEFAULT_HG_DEBOUNCE_PERIOD/2)
				{
					PINFO("shack unknow happened\n");
					atomic_set(&bma2x2->shack_times,0);
					del_timer(&bma2x2->shack_timer2);
//					input_report_rel(bma2x2->dev_interrupt,
//												HIGH_G_INTERRUPT,
//												HIGH_G_FLICK_UNKNOW);
					input_report_rel(bma2x2->dev_interrupt,
												HIGH_G_INTERRUPT2,
												HIGH_G_FLICKHV_UNKNOW);
					input_sync(bma2x2->dev_interrupt);
					
					/* Enable the slop no motion interrupts to filter continues flick*/

					bma2x2_set_Int_Enable(client, 15, 1); //set slop no motion to no-motion mode
					bma2x2_set_Int_Enable(client, 12, 1); //X
					bma2x2_set_Int_Enable(client, 13, 1); //Y
//					bma2x2_set_Int_Enable(client, 14, 1); //Z
                    atomic_set(&bma2x2->slop_x,0);
                    atomic_set(&bma2x2->slop_y,0);
				}
			}
			bma2x2_set_Int_Enable(client, 1, 1);
			bma2x2_set_Int_Enable(client, 2, 1);
			bma2x2_set_Int_Enable(client, 3, 1);
		}
	}
#endif
	...
	case 0x01:
		...
		//low g can be better than FLIP?
		if (atomic_read(&bma2x2->en_low_g) == 1) {
			struct bma2x2acc acc_value;
			...
			if(acc_value.z > 0){
				input_report_rel(bma2x2->dev_interrupt, LOW_G_INTERRUPT,
						LOW_G_INTERRUPT_HAPPENED_FACE_UP);
				input_sync(bma2x2->dev_interrupt);
				PINFO("Low G interrupt FACE UP\n");
			}
			else if(acc_value.z < 0){
				input_report_rel(bma2x2->dev_interrupt, LOW_G_INTERRUPT,
						LOW_G_INTERRUPT_HAPPENED_FACE_DOWN);
				input_sync(bma2x2->dev_interrupt);
				PINFO("Low G interrupt FACE DOWN\n");
			}
			else{
				input_report_rel(bma2x2->dev_interrupt, LOW_G_INTERRUPT,
						LOW_G_INTERRUPT_HAPPENED);
				input_sync(bma2x2->dev_interrupt);
				PINFO("Low G interrupt NONE\n");
			}
		}
		else
			break;
...
//		bma2x2_high_g_interrupt_handle(bma2x2);
	...
	case 0x08:
		...
		/* mark report data for further use */
//		input_report_rel(bma2x2->dev_interrupt,
//			SLOW_NO_MOTION_INTERRUPT,
//			SLOW_NO_MOTION_INTERRUPT_HAPPENED);
		atomic_set(&bma2x2->slop_x,1);
		atomic_set(&bma2x2->slop_y,1);
		bma2x2_set_Int_Enable(client, 12, 0); //X
		bma2x2_set_Int_Enable(client, 13, 0); //Y
//		bma2x2_set_Int_Enable(client, 14, 0); //Z
		break;
	...
	case 0x40:
		bma2x2_get_orient_status(bma2x2->bma2x2_client,
				    &first_value);
		PINFO("orient interrupt happened,%s value = %d\n",
				orient[first_value],first_value);
		if (first_value == 0){
			input_report_abs(bma2x2->dev_interrupt,
			ORIENT_INTERRUPT,
...
#if 0
		if(bma2x2->flat_wq)
		    	queue_delayed_work(bma2x2->flat_wq, &bma2x2->flat_work,msecs_to_jiffies(500));
#endif
#if 1
		bma2x2_get_orient_flat_status(bma2x2->bma2x2_client,
				    &sign_value);
			...
			struct bosch_sensor_specific *pdata)
{
	...
	rc = of_property_read_u32(np, OF_PLACE, &temp_val);
	if (rc && (rc != -EINVAL)) {
...
//	rc = of_property_read_u32(np, OF_GPIO_IRQ, &temp_val);
//	if (rc && (rc != -EINVAL)) {
//		dev_err(dev, "Unable to read sensor gpio paramater\n");
//	}
//
//	if (temp_val <= 0) {
//		dev_err(dev, "Invalid gpio num %d \n",temp_val);
//	} else {
//		pdata->gpio_num = temp_val;
//	}
	pdata->gpio_num = of_get_named_gpio_flags(dev->of_node,OF_GPIO_IRQ, 0, NULL);

	if (pdata->gpio_num <= 0)
		dev_err(dev, "Invalid gpio num %d \n",pdata->gpio_num);

	pdata->irq_num = irq_of_parse_and_map(np,0);

	printk(KERN_INFO "bma2x2_parse_dt Result  place = %d , gpio num = %d irq num = %d\n",pdata->place,pdata->gpio_num,pdata->irq_num);
		...
		const struct i2c_device_id *id)
{
...
#ifdef DUSRBUILD
	set_debug_log_level(LOG_LEVEL_E);
#else
	set_debug_log_level(LOG_LEVEL_D);
#endif

	PINFO("BMA2x2 driver probe start");
	...
	if (client->dev.of_node) {
		data->apdata = devm_kzalloc(&client->dev,
			sizeof(*data->apdata), GFP_KERNEL);
		if (!data->apdata) {
			PERR("Failed to allcated memory\n");
			err = -ENOMEM;
			goto kfree_exit;
		}
		err = bma2x2_parse_dt(&client->dev, data->apdata);
		if (err) {
			PERR("Failed to parse device tree\n");
			err = -EINVAL;
	...
	if (!data->apdata) {
		PERR("Cannot get device platform data\n");
		err = -EINVAL;
		goto kfree_exit;
	}
	...
	//clear the internel offset value
	atomic_set(&data->offset_x,0);
	atomic_set(&data->offset_y,0);
	atomic_set(&data->offset_z,0);
	atomic_set(&data->is_offset_inited,0);
...
//	bma2x2_set_int2_pad_sel(client, PAD_SLOW_NO_MOTION);
	...
	if(data->apdata->gpio_num > 0){
		err = gpio_request(data->apdata->gpio_num,"bma2x2");

		if(err)
			dev_err(&client->dev,  "could not request gpio %d,\n",data->apdata->gpio_num);
	}

	data->IRQ = data->apdata->irq_num;
...
#endif
	if (err)
		PERR("bma2x2 could not request irq\n");
	...
	if (NULL != data->apdata) {
		...
		if (NULL != data->bst_pd) {
			memcpy(data->bst_pd, data->apdata,
					sizeof(*data->bst_pd));
	...
	err = sysfs_create_link(&data->bma2x2_client->dev.kobj, &data->input->dev.kobj, "i2c");
	...
	setup_timer(&data->tap_timer, bma2x2_tap_timeout_handle,
			(unsigned long)data);
	setup_timer(&data->tap_timer2, bma2x2_tap_timeout_handle2,
			(unsigned long)data);
#endif

#ifdef CONFIG_HIGH_G
	atomic_set(&data->en_high_g, 0);
	atomic_set(&data->slop_x, 1);
	atomic_set(&data->slop_y, 1);
	data->shack_time_period = DEFAULT_HG_JUDGE_PERIOD;
	mutex_init(&data->shack_mutex);
	atomic_set(&data->shack_times,0);
	setup_timer(&data->shack_timer, bma2x2_shack_timeout_handle,
			(unsigned long)data);
	setup_timer(&data->shack_timer2, bma2x2_shack_timeout_handle2,
			(unsigned long)data);
#endif
#ifdef CONFIG_LOW_G
	atomic_set(&data->en_low_g, 0);
#endif
	atomic_set(&data->last_flat, 0);
#ifdef CONFIG_ORIENTATION
	atomic_set(&data->en_orientation, 0);
	data->flat_wq = create_singlethread_workqueue("flat_work");
	INIT_DELAYED_WORK(&data->flat_work, flat_work_handler);
#endif
...
pdata_free_exit:
	if (data->apdata && (client->dev.of_node))
		devm_kfree(&client->dev, data->apdata);
	data->apdata = NULL;
...
static struct of_device_id bma2x2_match_table[] = {
	{.compatible = OF_SENSOR_NAME },
	{},
};
static struct of_device_id bma2x2_match_table_sr[] = {
	{.compatible = OF_SENSOR_NAME_SR },
	{},
};
static struct of_device_id bma2x2_match_table_er2[] = {
	{.compatible = OF_SENSOR_NAME_ER2 },
	{},
};
#else
...
static int __init BMA2X2_init(void)
{
	...
	PINFO("system entry_mode = %d \n",entry_mode);
	if(entry_mode != 1 ){
		PINFO("Not in MOS, no need gsensor driver. \n");
        return ret;
	}

	PINFO("bma2x2: add i2c driver %s\n", __func__);
    PINFO("PCL_33 value is %d\n", gpio_get_value(33));


    int hardware_id = 0;
    hardware_id=asustek_boardinfo_get(FUN_HARDWARE_ID);
    PINFO("%s : hardware_id is %d \n", __func__, hardware_id);

    //SR:0 ER:1 ER2:2 PR:3
    if (hardware_id == 0x00 || hardware_id == 0x01) {
    	PINFO("%s : hardware_id is SR or ER1 !\n", __func__);
	    gpio_set_value(75,1); //VDD12_CADIZ_EN
    	bma2x2_driver.driver.of_match_table = bma2x2_match_table_sr;
    }
    /*else if  (hardware_id == 0x03) {
        PINFO("%s : hardware_id is ER2 !\n", __func__);
        bma2x2_driver.driver.of_match_table = bma2x2_match_table[2];
    }
    */
    else {
    	PINFO(" %s : hardware_id is ER2 or others !\n", __func__);
	    gpio_set_value(33,1); //SENSOR_1V8_EN
    	bma2x2_driver.driver.of_match_table = bma2x2_match_table_er2;
    }

    ret = i2c_add_driver(&bma2x2_driver);

	if(ret){
		PERR("bma2x2: add i2c driver faild %s\n", __func__);
		return ret;
	}
	return 0;
}
