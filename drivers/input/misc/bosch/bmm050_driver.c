		...
		struct device_attribute *attr, char *buf)
{
	...
	err = bmm_check_chip_id(bmm_client);
...
static DEVICE_ATTR(place, S_IRUGO,
		bmm_show_place, NULL);
static DEVICE_ATTR(compass_state, S_IRUGO,
		...
		struct bosch_sensor_specific *pdata)
{
	...
	rc = of_property_read_u32(np, "bmm050,place", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	printk(KERN_INFO "bmm050_parse_dt Result  place = %d irq num = %d \n",pdata->place,pdata->irq);
	return 0;
}
#else
static int bmm050_parse_dt(struct device *dev,
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	PINFO("bmm050 driver probe start");
	...
	if (client->dev.of_node) {
		...
		err = bmm050_parse_dt(&client->dev, pdata);
		if (err) {
	...
	pdata->name = SENSOR_NAME;
	client_data->apdata = pdata;
	...
	/* sysfs link to devices/i2c */
	err = sysfs_create_link(&client->dev.kobj, &client_data->input->dev.kobj, "i2c");

	if (err < 0)
		goto exit_err_sysfs;

#ifdef CONFIG_BMM_USE_PLATFORM_DATA
	if (NULL != client_data->apdata) {
		client_data->bst_pd = kzalloc(sizeof(*client_data->bst_pd),
				GFP_KERNEL);

		if (NULL != client_data->bst_pd) {
			memcpy(client_data->bst_pd, client_data->apdata,
...
//	err = request_irq(client_data->apdata->irq, bmm_work_func, IRQF_TRIGGER_RISING,
//			"bmm050", client_data);
//	if (err)
//			dev_err(&client->dev,  "could not request irq %d,\n",client_data->apdata->irq);
	...
	printk("bmm050 driver probe successfully");
...
static struct of_device_id bmm_match_table_sr[] = {
	{.compatible = "bosch,bmm050,sr" },
	{},
};
static struct of_device_id bmm_match_table_er2[] = {
	{.compatible = "bosch,bmm050,er2" },
	{},
};
#else
#define bmm_match_table NULL
#endif
...
static int __init BMM_init(void)
{
	...
	if(entry_mode != 1 ){
		PINFO("Not in MOS, no need compass driver. \n");
        return ret;
	}


	PINFO("bmm050: add i2c driver %s\n", __func__);
    ...
    if (hardware_id == 0x00 || hardware_id == 0x01) {
    	...
    	bmm_driver.driver.of_match_table = bmm_match_table_sr;
    }
    /*else if  (hardware_id == 0x02) {
    ...
    */
    	...
    	bmm_driver.driver.of_match_table = bmm_match_table_er2;
    }

    ret = i2c_add_driver(&bmm_driver);

	if(ret){
		PERR("bmm050: add i2c driver faild %s\n", __func__);
		return ret;
	}
	return 0;
}
