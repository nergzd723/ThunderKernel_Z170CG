...
#ifdef FACTORY_BUILD
		pr_info("%s[%d] Sees [0x%02X] [%c]", tag, link, c,
		             isprint(c) ? c : '-');
#else
...
#ifdef FACTORY_BUILD
		char buf[VUSB_SECMD_BUFFER_SIZE];

		memset(buf, 0, VUSB_SECMD_BUFFER_SIZE);
		strncpy(buf, read_buf, VUSB_SECMD_BUFFER_SIZE);

		pr_info("%s[%d] Read %d : %s", tag, link, have_read, buf);
#else
