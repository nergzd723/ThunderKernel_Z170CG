...
static unsigned int data_mounted;
static char vdump_config_name[64];
		...
		const char __user *user_buffer, size_t count, loff_t *position)
{
	...
	if (result)
		VD_DEBUG("Error copy_from_user\n");
	buffer[count] = '\0';
	VD_DEBUG("Buffer: %s Count: %d\n", (char *)buffer, (uint32_t)count);
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	{
	if (!strncmp(buffer, "data", 4)) {
		data_mounted = 1;
		VD_LOG("Data partition mounted\n");
       } else if (strstr(buffer, "vdump") != NULL) {
		sprintf(vdump_config_name, "%s", buffer);
		VD_LOG("vdump_config_name:%s\n", vdump_config_name);
        }
	if (count <= 0)
		VD_LOG("Vdump write Error\n");
	}
	set_fs(old_fs);
	kfree(buffer);
	return count;
...
static int vdump_init(void)
{
	...
	memset(vdump_config_name, 0, sizeof(vdump_config_name));
...
static int vdump_thread(void *param)
{
    ...
    while (data_mounted != 1) //wait for data partition mounted
        msleep(2000);

	vdump_set_linux_config(vdump_config_name);
