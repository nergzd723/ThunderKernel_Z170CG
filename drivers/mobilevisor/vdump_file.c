...
#if FACTORY_BUILD
#define VDUMP_COREDUMP_FILE "/data/logs/coredump.istp"
#else
#define VDUMP_COREDUMP_FILE "/sdcard/Logs/coredump.istp"
#endif
...
#if FACTORY_BUILD
#define DEFAULT_COREDUMP_PATH "/data/logs/"
#else
#define DEFAULT_COREDUMP_PATH "/sdcard/Logs/"
#endif
...
void vdump_set_linux_config(char *cfg_name)
{
	int i = 20;
	vdump_detect(VDUMP_CONFIG_FILE);
	while (i--) {
	    if (true == vdump_get_config(cfg_name)) {
			vdump_send_config();
			break;
		}
		msleep(2000);
...
void vdump_open_coredump(void)
{
	...
	for (i = 0; i <= 99; i++) {
        #if FACTORY_BUILD
        sprintf(filename, "/data/logs/coredump_%02d.istp", i);
        #else
        sprintf(filename, "/sdcard/Logs/coredump_%02d.istp", i);
        #endif
...
static bool vdump_get_config(char *cfg_path)
{
	...
	set_fs(KERNEL_DS);
       ...
       if(strstr(cfg_path, "vdump") != NULL)
       {
           VD_LOG("vdump_get_config from: %s\n", cfg_path);
           fp_cfg = filp_open(cfg_path, O_RDONLY, 0);
       } else {
           VD_LOG("vdump_get_config from default: %s\n", VDUMP_CONFIG_FILE);
           fp_cfg = filp_open(VDUMP_CONFIG_FILE, O_RDONLY, 0);
       }
	//VD_LOG("fp_cfg:%p\n", (void *)fp_cfg);
