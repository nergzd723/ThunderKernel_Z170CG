 ...
 *  Copyright (C) 2014 Intel Mobile Communications GmbHd
 *
...
#define mvpipe_info(fmt, arg...) \
	do { \
		if (DEBUG_LEVEL >= DEBUG_LEVEL_INFO) \
			pr_info(MODULE_NAME"[I][pid %i]: "fmt, current->pid, ##arg); \
	} while (0)

#define mvpipe_error(fmt, arg...) \
	do { \
		if (DEBUG_LEVEL >= DEBUG_LEVEL_ERROR) \
			pr_info(MODULE_NAME"[E][pid %i]: "fmt, current->pid, ##arg); \
	} while (0)
...
int mvpipe_dev_open(struct inode *inode, struct file *filp)
{
	...
	mvpipe_error("Open mvpipe: %s\n", dev->dev_name);

	if (down_interruptible(&dev->open_sem)) {
		mvpipe_error("down interruptible fail! %s\n", dev->dev_name);
		return -ERESTARTSYS;
	}
	...
	if (dev->open_count) {
		...
		if (dev->is_dbg) {
			...
			mvpipe_error("Multiple open, open_count = %d! %s\n", dev->open_count, dev->dev_name);
			return -ERESTARTSYS;
		}
	}
...
int mvpipe_dev_release(struct inode *inode, struct file *filp)
{
	...
	if (!time)
		WARN(1, "mvpipe %s close timeout elapsed\n", dev->dev_name);

	mvpipe_error("Close %s in %d ms\n", dev->dev_name, jiffies_to_msecs(time));

	/* set status, and inform peer */
	set_pipe_status(dev, MVPIPE_CLOSE);
	mv_ipc_mbox_post(dev->token, dev->pipe_event);

	mvpipe_error("Release mvpipe successful! %s\n", dev->dev_name);
