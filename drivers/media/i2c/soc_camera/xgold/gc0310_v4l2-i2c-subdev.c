...
static unsigned char *sensor_name = GC0310_DRIVER_NAME;
...
int ATD_gc0310_status = 0;  //Add for ATD read camera status
static ssize_t gc0310_show_status(struct device *dev,struct device_attribute *attr,char *buf)
{
        printk("%s: get gc0310 status (%d) !!\n", __func__, ATD_gc0310_status);
        //Check sensor connect status, just do it  in begining for ATD camera status

        return sprintf(buf,"%d\n", ATD_gc0310_status);
}

static DEVICE_ATTR(gc0310_status, S_IRUGO,gc0310_show_status,NULL);

static struct attribute *gc0310_attributes[] = {
        &dev_attr_gc0310_status.attr,
        NULL
};
static struct attribute_group gc0310_attribute_group = {
	.attrs = gc0310_attributes
};
static int gc0310_dbgfs_init(void)
{
	...
	debugfs_create_u32("camera_status",
		0644, debugfs_dir, &ATD_gc0310_status);
...
static int gc0310_start_streaming(struct gc_camera_module *cam_mod)
{
	...
	gc_camera_module_pr_debug(cam_mod, "\n");
        ...
        printk("[DEBUG] gc0310_start_streaming");
	...
	if (IS_ERR_VALUE(gc_camera_module_write_reg(cam_mod, 0xfe, 0x03)))
		goto err;
	//if (IS_ERR_VALUE(gc_camera_module_write_reg(cam_mod, 0x16, 0x09)))
	//	goto err;
...
static int gc0310_stop_streaming(struct gc_camera_module *cam_mod)
{
	...
	gc_camera_module_pr_debug(cam_mod, "\n");
        ...
        printk("[DEBUG] gc0310_stop_streaming");

	ret = gc_camera_module_write_reg(cam_mod, 0xfe, 0x03);
	if (IS_ERR_VALUE(ret))
		goto err;
	//ret = gc_camera_module_write_reg(cam_mod, 0x16, 0x05);
	//if (IS_ERR_VALUE(ret))
	//	goto err;
	//	msleep(200);
	...
	const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "probing...\n");
        ...
        ret = sysfs_create_group(&client->dev.kobj, &gc0310_attribute_group);
        if (ret) {
