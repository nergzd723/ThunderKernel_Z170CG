/*
 * Support for spca700xa_16mp Camera Sensor.
 *
...
//#include "i7002a.h"
#include "spca700xa_16mp.h"
#define SPCA700XA_16MP_DRIVER_NAME "spca700xa_16mp" //Clark add
#define SPCA700XA_16MP_5M_DRIVER_NAME "icatch5m"
#define SPCA700XA_16MP_8M_DRIVER_NAME "icatch8m"
#define to_spca700xa_16mp_sensor(sd) container_of(sd, struct spca700xa_16mp_device, sd)
static int spca700xa_16mp_set_mbus_fmt(struct v4l2_subdev *sd,
...
static int lastEV = -100;//change to a big value because -1 is in the EV range
static bool bFristTimeTriggerAF = true;
//We need trigger AF(0x7146) before set continous focus mode
...
static int mLowLightMode = 0;
static int mLowLightCapture = 0;
static int mZSL = 0;
...
static struct attribute *spca700xa_16mp_attributes[] = {
	&dev_attr_spca700xa_16mp_status.attr,
	NULL
};

static int spca700xa_16mp_dbgfs_init(void)
{
...
static unsigned char *sensor_name = SPCA700XA_16MP_DRIVER_NAME;
...
#define SUB_CAMERA_ID_03M	0
#define SUB_CAMERA_ID_2M	1
#else
#define MAIN_CAMERA_ID_5M	0
#define MAIN_CAMERA_ID_2M	1
#endif 
...
static int spca700xa_16mp_s_power(struct v4l2_subdev *sd, int power);
static int spca700xa_16mp_s_config(struct v4l2_subdev *sd);
//Move and modify from ov5693.c for firmware --- Wesley
...
int sensor_read_reg(struct i2c_client *client, u16 addr, u16 *val)
{
	...
	if (err != 2) {
		pr_info("spca700xa_16mp: %s err: %d\n", __func__, err);
		return -EINVAL;
	}
...
static void dumpReg(struct i2c_client *client, u16 addr) {
	u16 retvalue_l = 0;
	sensor_read_reg(client, addr, &retvalue_l);
...
int I2C_SPIInit(struct v4l2_subdev *sd)
{
	...
	printk("============ Wesley I2C_SPIInit ========== I2C_SPIInit ======== In =======\n");
...
BB_WrSPIFlash(struct v4l2_subdev *sd, char* binfile_path)
{
	...
	ret = spca700xa_16mp_s_power(sd, 1);
	goto end; //Wesley Test

	if (ret) {
		dev_err(&client->dev, "spca700xa_16mp power-up err");
	}
	...
	ret = spca700xa_16mp_s_power(sd, 0);
	if (ret) {
		dev_err(&client->dev, "spca700xa_16mp power down err");
	}
...
static int i7002a_proc_front_read(struct seq_file *m, void *start)
{
	...
	int ret=0;
	static unsigned int version_num_in_isp = 0xffffff;
   	ret = spca700xa_16mp_s_power(fw_sd, 1);
	version_num_in_isp = get_fw_version_in_isp(fw_sd);
	ret = spca700xa_16mp_s_power(fw_sd, 0);
...
//ASUS_BSP---- For DIT VCM Debug and ATD flash Interface---
...
int sensor_s_lowlight_cap(struct v4l2_subdev *sd, int hdr);

int sensor_s_hdr(struct v4l2_subdev *sd, int hdr)
{
	printk("%s HDR enable %d \n", __func__, hdr);
	mHDR = hdr;
...
int sensor_s_lowlight_cap(struct v4l2_subdev *sd, int enable)
{
	printk("%s low light capture enable %d \n", __func__, enable);
	mLowLightCapture = enable;
...
int sensor_s_scene_mode(struct v4l2_subdev *sd, int scene_mode)
{
	...
	if (mLowLightMode && mLowLightCapture)
		return 0;
	 ...
	 mNightMode = false;
		...
		sensor_write_reg(client, 0x7134, 0x00);//set to ultra night mode
...
int sensor_s_ae_metering_mode(struct v4l2_subdev *sd, int ae_mode)
{
	...
	case V4L2_EXPOSURE_METERING_CENTER_WEIGHTED:
		sensor_write_reg(client, 0x710E, 0x01);//center
		break;
	default:
...
int sensor_s_focus_mode(struct v4l2_subdev *sd, int focus_mode)
{
	...
	case SENSOR_FOCUS_MODE_INFINITY:
		...
		sensor_write_reg(client, 0x7146, 0x01);
		break;
	case SENSOR_FOCUS_MODE_CONTINUOUS_VIDEO:
	case SENSOR_FOCUS_MODE_CONTINUOUS_PICTURE:
	//Continuous focus special flow need to trigger focus first and than set mode
		if (bFristTimeTriggerAF){
			sensor_write_reg(client, 0x7146, 0x01);
		}
		sensor_write_reg(client, 0x7105, 0x03);//continous
		break;
	default:
		dev_err(&client->dev, "invalid focus mode: %d", focus_mode);
		return -ERANGE;
	}
	bFristTimeTriggerAF = false;
	return 0;
}

int sensor_s_focus_start(struct v4l2_subdev *sd, int focus_start)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	printk("%s mode = %d\n", __func__, focus_start);

	if (focus_start){
		sensor_write_reg(client, 0x7105, 0x00);
		sensor_write_reg(client, 0x7146, 0x01);
	}
	return 0;
}
...
int sensor_s_focus_window(struct v4l2_subdev *sd, char *window)
{
	...
	//sensor_write_reg(client, 0x7105, 0x00);//AF auto mode
...
int sensor_s_read_spi(struct v4l2_subdev *sd, int *val)
{
	...
	//int i,j;
        ...
        ret = sensor_write_reg(client,  0x40E1, 0x02);
	printk("%s, val = %x\n", __func__, *val);
	type = (*val)>>28;
...
//add for fix write LSC_DQ header probelm --
	...
	*val = 0;
	...
	kfree(ucStartAddr);
        ret = sensor_write_reg(client,  0x40E1, 0x00);
...
//For iCatch 3A information---
int sensor_g_vblanking(struct v4l2_subdev *sd, int *val)
{
        //struct i2c_client *client = v4l2_get_subdevdata(sd);
        *val = 2000;
...
static UINT8 EXISP_SPIDataWrite(UINT8 ucSPIMode, UINT8 *ucStartAddr, UINT32 ulTransByteCnt, UINT32 ulDmaAddr, struct v4l2_subdev *sd)
{
	...
	sensor_read_reg(client, 0x72F8, &retvalue_l);
	printk("spca700xa_16mp BEFORE Wesley SPi Test read 0x72F8 %x\n",retvalue_l&0xffff);

        retvalue_l = 0;
	sensor_read_reg(client, 0x4284, &retvalue_l);
	printk("spca700xa_16mp BEFORE Wesley SPi Test read 0x4284 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x4285, &retvalue_l);
	printk("spca700xa_16mp BEFORE Wesley SPi Test read 0x4285 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x4280, &retvalue_l);
	printk("spca700xa_16mp BEFORE Wesley SPi Test read 0x4280 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x0026, &retvalue_l);
	printk("spca700xa_16mp BEFORE Wesley SPi Test read 0x0026 (1) %x\n",retvalue_l&0xffff);
#if 0
	sensor_write_reg(client, 0x0026, 0xcf);
#endif
	retvalue_l = 0;
	sensor_read_reg(client, 0x0026, &retvalue_l);
	printk("spca700xa_16mp BEFORE Wesley SPi Test read 0x0026 (2) %x\n",retvalue_l&0xffff);

	retvalue_l = 0;
	sensor_read_reg(client, 0x0022, &retvalue_l);
	printk("spca700xa_16mp BEFORE Wesley SPi Test read 0x0022 %x\n",retvalue_l&0xffff);
	spca700xa_SPI_write(ucStartAddr, ulTransByteCnt);

        retvalue_l = 0;
	sensor_read_reg(client, 0x4178, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4178 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x4179, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4179 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x417A, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x417A %x\n",retvalue_l&0xffff);
	...
	printk("spca700xa_16mp Wesley SPi Test checksun\n");

	sensor_read_reg(client, 0x4284, &retvalue_l);
	printk("spca700xa_16mp Wesley SPi Test read 0x4284 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x4285, &retvalue_l);
	printk("spca700xa_16mp Wesley SPi Test read 0x4285 %x\n",retvalue_l&0xffff);
...
#if 1
{
	...
	for (i = 0; i < 16; i++) {
		...
		if (temp1 != reg10a6) {
			reg10a6 = temp1;
			sensor_write_reg(client,0x10a6,reg10a6);
		}
		if (temp2 != reg10a7) {
			reg10a7 = temp2;
			sensor_write_reg(client,0x10a7,reg10a7);
		}
		retvalue_l = 0;
		sensor_read_reg(client, regData, &retvalue_l);
		printk("====WESLEY===== READ SPI from reg %d=%x\n",i,retvalue_l&0xffff);
		//*(pDataBuf+i) = sensor_read_reg(client, regData);
		startAddr++;
	}
	
	sensor_write_reg(client,0x10a6, 0x00);
	sensor_write_reg(client,0x10a7, 0x00);
}
#endif
        ...
        retvalue_l = 0;
	sensor_read_reg(client, 0x4284, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4284 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x4285, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4285 %x\n",retvalue_l&0xffff);
	...
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x417A %x\n",retvalue_l&0xffff);
        ...
        retvalue_l = 0;
	sensor_read_reg(client, 0x4051, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4051 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x4164, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4164 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x4170, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4170 %x\n",retvalue_l&0xffff);
        retvalue_l = 0;
	sensor_read_reg(client, 0x4171, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4171 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x4172, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4172 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x108C, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x108C %x\n",retvalue_l&0xffff);
        retvalue_l = 0;
	sensor_read_reg(client, 0x1080, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x1080 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x1081, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x1081 %x\n",retvalue_l&0xffff);
	retvalue_l = 0;
	sensor_read_reg(client, 0x1082, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x1082 %x\n",retvalue_l&0xffff);
        retvalue_l = 0;
	sensor_read_reg(client, 0x1084, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x1084 %x\n",retvalue_l&0xffff);
        retvalue_l = 0;
	sensor_read_reg(client, 0x40E1, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x40E1 %x\n",retvalue_l&0xffff);
        retvalue_l = 0;
	sensor_read_reg(client, 0x4288, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x4288 %x\n",retvalue_l&0xffff);
        retvalue_l = 0;
	sensor_read_reg(client, 0x1011, &retvalue_l);
	printk("spca700xa_16mp AFTER Wesley SPi Test read 0x1011 %x\n",retvalue_l&0xffff);
#endif
	...
	struct v4l2_subdev *sd)
{
	...
	u16 retvalue=0;
        ...
        printk("In spca700xa_16mp EXISP_LoadCode\n");
	if (pIspFw == NULL) {
		printk("In spca700xa_16mp pIspFw is NULL\n");
	...
	printk("@@@spca700xa_16mp ISP FW: Get BOOT.BIN Bin File End\n");
...
_EXIT_:
	if (is_calibration == 1) { //calibration
		printk("********** spca700xa_16mp Load Golden Res **********\n");
	}
	else { //normal
		if (loadCodeRet.retCalibOpt == SUCCESS &&
			...
			printk("********** spca700xa_16mp Load Calibration Res **********\n");
		}
		else {
			if (loadCodeRet.retCalibOpt != SUCCESS) {
...
//	while (!(I2CDataRead(SP7K_RDREG_INT_STS_REG_0)&0x02)) {
	...
	printk("@@@In spca700xa_16mp ISP FW: Load DMEM/FICDMEM Bin File Start\n");
	...
	sensor_read_reg(client, 0x72f8, &retvalue_l);
	printk("Allen Test 0 spca700xa_16mp read 0x72f8 %x\n",retvalue_l&0xffff);
	...
	if(pFwInfo->DmemFicdmemSize <= 6*1024) { /* Data size <6K, load all bin file */
		...
		sensor_read_reg(client, 0x4284, &checksumRead);
	        ...
	        //u16 retvalue_l = 0;
		sensor_read_reg(client, 0x4284, &retvalue_l);
		printk("Allen Test 1 spca700xa_16mp read 0x4284 %x\n",retvalue_l&0xffff);

		retvalue_l = 0;
		sensor_read_reg(client, 0x72f8, &retvalue_l);
		printk("Allen Test 1_1 spca700xa_16mp read 0x72f8 %x\n",retvalue_l&0xffff);
		sensor_write_reg(client, 0x1011, 0); /* Set CPU to normal operation */
		...
		retvalue_l = 0;
		sensor_read_reg(client, 0x72f8, &retvalue_l);
		printk("Allen Test 1_2 spca700xa_16mp read 0x72f8 %x\n",retvalue_l&0xffff);

		printk("Allen Test 2 spca700xa_16mp read 0x4284 %x\n",retvalue_l&0xffff);

		/* Reset checksum value */
		sensor_write_reg(client, 0x4284, 0x00);
		ucRet = EXISP_SPIDataWrite(0/*SPI mode0*/, pIspFw+(6*1024), pFwInfo->DmemFicdmemSize-(6*1024), 0x0800+(6*1024), sd); //Ryant Modify cali
		sensor_read_reg(client, 0x4284, &retvalue_l);
		printk("Allen Test 3 spca700xa_16mp read 0x4284 %x\n",retvalue_l&0xffff);

		retvalue_l = 0;
		sensor_read_reg(client, 0x72f8, &retvalue_l);
		printk("Allen Test 3_1 spca700xa_16mp read 0x72f8 %x\n",retvalue_l&0xffff);
	}

	/* Checksum value check */
	sensor_read_reg(client, 0x4284, &checksumRead);
	if (checksumWrite == checksumRead) {
		printk("@@@In spca700xa_16mp ISP FW: Checksum DMEM/FICDMEM test: OK, %x, %x\n", checksumRead, checksumWrite);
	}
	else {
		printk("@@@In spca700xa_16mp ISP FW: Checksum DMEM/FICDMEM test: FAIL, %x, %x\n", checksumRead, checksumWrite);
		ucRet = FAIL;
		return ucRet;
	}
	...
	sensor_read_reg(client, 0x4284, &checksumRead);
	if (checksumWrite == checksumRead) {
		printk("@@@In spca700xa_16mp ISP FW: Checksum IMEM test: OK, %x, %x\n", checksumRead, checksumWrite);
	}
	else {
		printk("@@@In spca700xa_16mp ISP FW: Checksum IMEM test: FAIL, %x, %x\n", checksumRead, checksumWrite);
		ucRet = FAIL;
		return ucRet;
	}
	printk("@@@In spca700xa_16mp ISP FW: Load IMEM Bin File End\n");
	...
	UINT8 *pLscdq)
{
	...
	printk("In spca700xa_16mp start3acali=%x, size3acali=%d\n", start3acali, size3acali);
	printk("In spca700xa_16mp startLsc=%x, sizeLsc=%d\n", startLsc, sizeLsc);
	printk("In spca700xa_16mp startLscdq=%x, size=%d\n", startLscdq, sizeLscdq);
	if (idx == 0) {
...
int get_fw_version_in_bin(void)
{
	u8 *pbootBuf = NULL;
...
static int spca700xa_16mp_init_common(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct spca700xa_16mp_device *dev = to_spca700xa_16mp_sensor(sd);
	int ret = 0;
...
//ASUS_BSP---
	...
	printk("spca700xa_16mp read 0x1306 %x\n",retvalue_l&0xffff);
	//ASUS_BSP---, for calibration
	...
	if ( !IS_ERR_OR_NULL(fp) ){
        ...
        if(g_is_calibration){
	        status = EXISP_LoadCode(0x02, g_is_calibration, pbootBuf, pCalibOpt_g, p3acali_g, pLsc_g, pLscdq_g, sd); //Add for calibration
        }else{
                status = EXISP_LoadCode(0x00, g_is_calibration, pbootBuf, pCalibOpt_g, p3acali_g, pLsc_g, pLscdq_g, sd); //Add for calibration
        }
	//status = EXISP_LoadCode(0x01, pbootBuf, bootbin_size, sd);
	fw_update_status = ICATCH_FW_UPDATE_SUCCESS;
        if(g_is_calibration){
	        sensor_write_reg(client, 0x1306, 0x02); /* Front_calibration: 1 */
        }else{
                sensor_write_reg(client, 0x1306, 0x00); /* Front_calibration: 1 */
        }
	...
	//Wesley 20150114 ---- , For load from host
retry:
       return spca700xa_16mp_init_common(sd);
}
...
//ASUS_BSP+++
	...
	sensor_write_reg(client,0x714F,0x00);//Set VCM go to inf to reduce close AF module noise
	sensor_read_reg(client, 0x72A7, &retValue);
	while (retValue != 0x00) {
		uwTimeCnt++;
		if (uwTimeCnt > 1000) {
			printk("can't Wait VCM go to inf 1000 time.\n");
			break;
		}
	usleep_range(5000, 5100);
	sensor_read_reg(client, 0x72A7, &retValue);
	printk("Wait VCM go to inf.\n");
	}
...
static int spca700xa_16mp_try_res(u32 *w, u32 *h)
{
	int i;
	 ...
	 */
	for (i = 0; i < N_RES; i++) {
		if ((spca700xa_16mp_res[i].width >= *w) &&
		    (spca700xa_16mp_res[i].height >= *h))
			break;
	}
	...
	*w = spca700xa_16mp_res[i].width;
	*h = spca700xa_16mp_res[i].height;

	return 0;
}

static struct spca700xa_16mp_res_struct *spca700xa_16mp_to_res(u32 w, u32 h)
{
	int  index;
	...
	for (index = 0; index < N_RES; index++) {
		if ((spca700xa_16mp_res[index].width == w) &&
		    (spca700xa_16mp_res[index].height == h))
			break;
	}
	...
	return &spca700xa_16mp_res[index];
}

static int spca700xa_16mp_try_mbus_fmt(struct v4l2_subdev *sd,
				struct v4l2_mbus_framefmt *fmt)
{
	fmt->code = SPCA700XA_BAYER_ORDER;
	return spca700xa_16mp_try_res(&fmt->width, &fmt->height);
}

static int spca700xa_16mp_res2size(unsigned int res, int *h_size, int *v_size)
{
	...
	switch (res) {
	case SPCA700XA_16MP_RES_QCIF:
		hsize = SPCA700XA_16MP_RES_QCIF_SIZE_H;
		vsize = SPCA700XA_16MP_RES_QCIF_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_CIF:
		hsize = SPCA700XA_16MP_RES_CIF_SIZE_H;
		vsize = SPCA700XA_16MP_RES_CIF_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_QVGA:
		hsize = SPCA700XA_16MP_RES_QVGA_SIZE_H;
		vsize = SPCA700XA_16MP_RES_QVGA_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_VGA:
		hsize = SPCA700XA_16MP_RES_VGA_SIZE_H;
		vsize = SPCA700XA_16MP_RES_VGA_SIZE_V;
		break;
/*
	case SPCA700XA_16MP_RES_480P:
		hsize = SPCA700XA_16MP_RES_480P_SIZE_H;
		vsize = SPCA700XA_16MP_RES_480P_SIZE_V;
		break;
*/
	case SPCA700XA_16MP_RES_720P:
		hsize = SPCA700XA_16MP_RES_720P_SIZE_H;
		vsize = SPCA700XA_16MP_RES_720P_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_960P:
		hsize = SPCA700XA_16MP_RES_960P_SIZE_H;
		vsize = SPCA700XA_16MP_RES_960P_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_2M:
		hsize = SPCA700XA_16MP_RES_2M_SIZE_H;
		vsize = SPCA700XA_16MP_RES_2M_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_1080P:
		hsize = SPCA700XA_16MP_RES_1080P_SIZE_H;
		vsize = SPCA700XA_16MP_RES_1080P_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_5MP:
		hsize = SPCA700XA_16MP_RES_5MP_SIZE_H;
		vsize = SPCA700XA_16MP_RES_5MP_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_8MP:
		hsize = SPCA700XA_16MP_RES_8MP_SIZE_H;
		vsize = SPCA700XA_16MP_RES_8MP_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_13MP:
		hsize = SPCA700XA_16MP_RES_13MP_SIZE_H;
		vsize = SPCA700XA_16MP_RES_13MP_SIZE_V;
		break;
	case SPCA700XA_16MP_RES_16MP:
		hsize = SPCA700XA_16MP_RES_16MP_SIZE_H;
		vsize = SPCA700XA_16MP_RES_16MP_SIZE_V;
		break;
	default:
...
static int spca700xa_16mp_get_mbus_fmt(struct v4l2_subdev *sd,
				struct v4l2_mbus_framefmt *fmt)
{
	struct spca700xa_16mp_device *dev = to_spca700xa_16mp_sensor(sd);
	int width, height;
	...
	ret = spca700xa_16mp_res2size(dev->res, &width, &height);
	if (ret)
		return ret;
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct spca700xa_16mp_device *dev = to_spca700xa_16mp_sensor(sd);
	struct spca700xa_16mp_res_struct *res_index;
	...
	if (((mLowLightMode && mLowLightCapture && mZSL == true)
		|| mHDR) &&
		old_f.w == width && old_f.h == height) {
	...
	spca700xa_16mp_try_res(&width, &height);
	res_index = spca700xa_16mp_to_res(width, height);
	...
	if (mHDR) {
		printk("%s ===== HDR mode =====\n", __func__);
		sensor_write_reg(client, 0x7132, 0x00);
		...
		switch (res_index->res) {
			case SPCA700XA_16MP_RES_VGA:
				if (g_is_calibration) {
					sensor_write_reg(client, 0x7108, 0x0A);
					dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				} else {
					sensor_write_reg(client, 0x7108, 0x0B);
					dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				}
				break;
			case SPCA700XA_16MP_RES_720P:
				sensor_write_reg(client, 0x7108, 0x04);  //test pattern 0x28
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_960P:
				if (g_is_calibration){
					...
					sensor_write_reg(client,0x7108,0x00);
					dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				}
				break;
			case SPCA700XA_16MP_RES_2M:
				sensor_write_reg(client, 0x7108, 0x16);
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_1080P:
				sensor_write_reg(client, 0x7108, 0x02); //test pattern 0x27
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_5MP:
#if defined (CONFIG_Z370C) || defined (CONFIG_Z370CG)
				if (camera_id == MAIN_CAMERA_ID_TK)
					sensor_write_reg(client, 0x7108, 0x0A); //test pattern 0x2A
				else
#endif
					sensor_write_reg(client, 0x7108, 0xff); //test pattern 0x2A
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_8MP:
				sensor_write_reg(client, 0x7108, 0xff); //test pattern 0x24
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_13MP:
				sensor_write_reg(client, 0x7108, 0x08); //test pattern 0x2B
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			default:
		...
		sensor_write_reg(client, 0x710F, 0x00);

		if (mLowLightMode || mNightMode)
			sensor_write_reg(client, 0x7124, 0x00);
		else if (res_index->res <= SPCA700XA_16MP_RES_960P )
			sensor_write_reg(client, 0x7124, 0x1E); //fix preview sensor mode frame rate to 30fps for CTS
        else
            sensor_write_reg(client, 0x7124, 0x00);
		...
		switch (res_index->res) {
			case SPCA700XA_16MP_RES_VGA:
				if (g_is_calibration){
					sensor_write_reg(client, 0x7106, 0x0A);
					dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				} else {
					sensor_write_reg(client, 0x7106, 0x0B);
					dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				}
				break;
			case SPCA700XA_16MP_RES_720P:
				sensor_write_reg(client, 0x7106, 0x04);  //test pattern 0x28
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_960P:
				if (g_is_calibration){
					...
					sensor_write_reg(client,0x7106,0x00);
					dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				}
				break;
			case SPCA700XA_16MP_RES_2M:
				sensor_write_reg(client, 0x7106, 0x16);
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_1080P:
				sensor_write_reg(client, 0x7106, 0x02); //test pattern 0x27
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_5MP:
				sensor_write_reg(client, 0x7106, 0x0A); //test pattern 0x2A
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_8MP:
				sensor_write_reg(client, 0x7106, 0x01); //test pattern 0x24
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			case SPCA700XA_16MP_RES_13MP:
				sensor_write_reg(client, 0x7106, 0x08); //test pattern 0x2B
				dev_info(&client->dev, "%s: set for %s\n", __func__, res_index->desc);
				break;
			default:
				dev_err(&client->dev, "set resolution: %d failed!\n", res_index->res);
...
set_mode:
		if (mLowLightCapture) {
			printk("%s ===== ZSL mode for low light =====\n", __func__);
			sensor_write_reg(client, 0x7120, 0x02);
			mZSL = true;
			goto fmt_end;
		} else {
		...
		if (i == int_cnt) {
			pr_info("Change to %s mode fail\n", (mLowLightMode) ? "ZSL" : "Preview");
#if 1
...
fmt_end:
	...
	if ((fmt->code == V4L2_MBUS_FMT_SBGGR8_1X8)
		...
		sensor_write_reg(client, 0x7161, 0x00);
		...
		sensor_write_reg(client, 0x7160, 0x00);
		sensor_write_reg(client, 0x7161, 0x02);
	}
	// ASUS_BSP --- get raw data
...
static int spca700xa_16mp_g_focal(struct v4l2_subdev *sd, s32 *val)
{
	*val = (SPCA700XA_16MP_FOCAL_LENGTH_NUM << 16) | SPCA700XA_16MP_FOCAL_LENGTH_DEM;
	return 0;
}

static int spca700xa_16mp_g_fnumber(struct v4l2_subdev *sd, s32 *val)
{
	/* const f number for SPCA700XA_16MP */
	*val = (SPCA700XA_16MP_F_NUMBER_DEFAULT_NUM << 16) | SPCA700XA_16MP_F_NUMBER_DEM;
	return 0;
}

static int spca700xa_16mp_g_fnumber_range(struct v4l2_subdev *sd, s32 *val)
{
	*val = (SPCA700XA_16MP_F_NUMBER_DEFAULT_NUM << 24) |
		(SPCA700XA_16MP_F_NUMBER_DEM << 16) |
		(SPCA700XA_16MP_F_NUMBER_DEFAULT_NUM << 8) | SPCA700XA_16MP_F_NUMBER_DEM;
	return 0;
}
 ...
 * This returns EV.
 */
static int spca700xa_16mp_get_exposure_bias(struct v4l2_subdev *sd, s32 *value)
{
	*value = 0;
...
static struct spca700xa_16mp_control spca700xa_16mp_controls[] = {
	{
		.qc = {
		...
		.query = spca700xa_16mp_get_exposure_bias, //For EXIF value, need to combine with other parameter
	},
	{
		.qc = {
	...
	{
		.qc = {
			.id = V4L2_CID_LOW_LIGHT_CAPTURE,
			.type = V4L2_CTRL_TYPE_INTEGER,
			.name = "low light capture",
			.minimum = 0,
			.maximum = 1,
			.step = 1,
			.default_value = 0,
		},
		.tweak = sensor_s_lowlight_cap,
	},
	{
		.qc = {
			.id = V4L2_CID_GSENSOR ,
	...
	},
	{
		.qc = {
			...
			.type = V4L2_CTRL_TYPE_INTEGER,
			.name = "focal length",
			.minimum = SPCA700XA_16MP_FOCAL_LENGTH_DEFAULT,
			.maximum = SPCA700XA_16MP_FOCAL_LENGTH_DEFAULT,
			.step = 0x01,
			.default_value = SPCA700XA_16MP_FOCAL_LENGTH_DEFAULT,
			.flags = 0,
		},
		.query = spca700xa_16mp_g_focal,
	},
	{
		.qc = {
			...
			.type = V4L2_CTRL_TYPE_INTEGER,
			.name = "f-number",
			.minimum = SPCA700XA_16MP_F_NUMBER_DEFAULT,
			.maximum = SPCA700XA_16MP_F_NUMBER_DEFAULT,
			.step = 0x01,
			.default_value = SPCA700XA_16MP_F_NUMBER_DEFAULT,
			.flags = 0,
		},
		.query = spca700xa_16mp_g_fnumber,
	},
	{
		.qc = {
			...
			.type = V4L2_CTRL_TYPE_INTEGER,
			.name = "f-number range",
			.minimum = SPCA700XA_16MP_F_NUMBER_RANGE,
			.maximum =  SPCA700XA_16MP_F_NUMBER_RANGE,
			.step = 0x01,
			.default_value = SPCA700XA_16MP_F_NUMBER_RANGE,
			.flags = 0,
		},
		.query = spca700xa_16mp_g_fnumber_range,
	},
//ASUS_BSP++, add for calibration
	{
		.qc = {
...
//ASUS_BSP+++, for iCatch 3A information
        ...
        },
		{
				...
				.query = sensor_g_vblanking,
		},
//ASUS_BSP---, for iCatch 3A information
};

#define N_CONTROLS (ARRAY_SIZE(spca700xa_16mp_controls))

static struct spca700xa_16mp_control *spca700xa_16mp_find_control(__u32 id)
{
	int i;
	for (i = 0; i < N_CONTROLS; i++) {
		if (spca700xa_16mp_controls[i].qc.id == id)
			return &spca700xa_16mp_controls[i];
	}
	return NULL;
}
...
/*--------------------------------------------------------------------------*/
// ASUS BSP ----- for read otp data
// ASUS BSP +++ Vita, for read ov5670 otp data
static UINT16 SPCA7002_sensor_OV5670_i2c_read(struct i2c_client *client, UINT16 a_uwRegAddr, UINT8 a_ucDataBytes)
{
	UINT8 i;
	UINT16 ucRegVal;
	/*Start - I2C Read ID*/
	sensor_write_reg(client,0x923C,(a_ucDataBytes-1));  /* Sub address enable */
	sensor_write_reg(client,0x9200,0x03);  /* I2C Read mode	*/
	sensor_write_reg(client,0x9210,(a_uwRegAddr>>8)&0xFF);  /* Register addr MSB	*/
	sensor_write_reg(client,0x9212,(a_uwRegAddr)&0xFF);  /* Register addr LSB*/
	sensor_write_reg(client,0x9204,0x01);  /* Trigger I2C read	*/
	usleep_range(1000, 1100);
	if (a_ucDataBytes > 2)
	{
		pr_err("%s Warning: Overflow\n", __func__);
	}
	ucRegVal = 0;
	for(i = 0; i < a_ucDataBytes; i++)
	{
		UINT16 val;
		//ucRegVal += (I2CDataRead(0x9211+(i*2))<<((a_ucDataBytes-i-1)*8));	  /* MSB */
		sensor_read_reg(client, 0x9211+(i*2), &val);
		ucRegVal += (val<<((a_ucDataBytes-i-1)*8));	  /* MSB */
	}
	usleep_range(1000, 1100);
	/*End - I2C Read*/
	return ucRegVal;
...
static void SPCA7002_sensor_OV5670_i2c_write(struct i2c_client *client, UINT16 a_uwRegAddr, UINT16 a_ucRegVal, UINT8 a_ucDataBytes)
{	/*Start - I2C Write*/
	UINT8 i;

	sensor_write_reg(client,0x923C,(a_ucDataBytes-1));  /* Sub address enable */
	sensor_write_reg(client,0x9200,0x00);  /* I2C Write mode	*/
	sensor_write_reg(client,0x9210,(a_uwRegAddr>>8)&0xFF);  /* Register addr MSB	*/
	sensor_write_reg(client,0x9212,(a_uwRegAddr)&0xFF);  /* Register addr LSB*/
	for(i=0;i<a_ucDataBytes;i++)
	{
		//I2CDataWrite(0x9211+(i*2),((a_ucRegVal)>>((a_ucDataBytes-i-1)*8))&0xff);  /* Register value*/
		sensor_write_reg(client,0x9211+(i*2),((a_ucRegVal)>>((a_ucDataBytes-i-1)*8))&0xff);  /* Register value*/
	}
	sensor_write_reg(client,0x9204,0x01);  /* Trigger I2C write	*/
	udelay(100); /* 0.1ms */
	/*End - I2C Write*/
}

static void SPCA7002_sensor_OV5670_Power_On(struct i2c_client *client)
{
	...
	sensor_write_reg(client,0x9008, 0x00);  /* Need to check with vincent */
	sensor_write_reg(client,0x9009, 0x00);
	sensor_write_reg(client,0x900A, 0x00);
	sensor_write_reg(client,0x900B, 0x00);

	sensor_write_reg(client,0x9238, 0x30);
	sensor_write_reg(client,0x9240, 0x20);


}

static void SPCA7002_sensor_OV5670_Check_OTP_rd_status(struct i2c_client *client)
{
	UINT8 temp, count=0;

	temp = SPCA7002_sensor_OV5670_i2c_read(client, 0x3d81, 1);
...
static SPCA7002_sensor_OV5670_init(struct i2c_client *client)
{
	UINT32 i, temp;
	...
	temp = sizeof(cmd_init_code)/sizeof(cmd_init_code[0]);

	for(i=0; i<temp; i++){
		SPCA7002_sensor_OV5670_i2c_write(client,((cmd_init_code[i][0]<<8)+cmd_init_code[i][1]), cmd_init_code[i][2], 1);
	}
};

static void SPCA7002_sensor_OV5670_OTP_read(struct i2c_client *client, UINT16 a_uwReadStartOffset, UINT16 a_uwDataBytes, UINT8* a_ucDataBuf)
{

	UINT16 i;
	//UINT16 uwOTPDataAddr = 0x7000;

	for(i = 0; i < a_uwDataBytes; i++)
	{
		*((UINT8*)(a_ucDataBuf + i)) = (UINT8)(SPCA7002_sensor_OV5670_i2c_read(client, a_uwReadStartOffset + i, 1));
		//*((UINT8*)(a_ucDataBuf + i)) = (UINT8)(SPCA7002_sensor_OV5670_i2c_read(client, uwOTPDataAddr+a_uwReadStartOffset + i, 1));
		//*(UINT8*)(a_ucDataBuf+i) = (UINT8)(SPCA7002_sensor_OV5670_i2c_read(uwOTPDataAddr+a_uwReadStartOffset+i, 1));
		 pr_debug("otp_value: [%2d]0x%x\n", i,*((UINT8*)(a_ucDataBuf+i)));
	}

}

static int ov5670_otp_read(struct i2c_client *client)
{
	//int ret = 0;
	//int reg = 0;
	int i, otp_bytes = 0;
	//int otp_flag, addr, temp1;

	//unsigned int local_data;
	unsigned char read_value[32];
	int start_addr;
	int package;
	//struct ov5670_otp_struct *buf;

	printk("%s\n", __func__);
	//SPCA7002_sensor_OV5670_Power_On(client);
	//SPCA7002_sensor_OV5670_i2c_write(client, 0x0103, 0x01, 1); /* software reset */
	//msleep(20); /* wait 20ms */
	//SPCA7002_sensor_OV5670_init(client);
	SPCA7002_sensor_OV5670_i2c_write(client, 0x0100, 0x01, 1); /* sensor stream on */
	SPCA7002_sensor_OV5670_i2c_write(client, 0x3d81, 0x01, 1); /* trigger load OTP data */
	SPCA7002_sensor_OV5670_Check_OTP_rd_status(client);
	...
	for (package = 2; package >= 0; package--)
	{
		...
		SPCA7002_sensor_OV5670_OTP_read(client, start_addr, otp_bytes, &read_value);
...
// ASUS BSP +++ Vita, for read t4k35 otp data
static void SPCA7002_sensor_T4K35_poweron_set(struct i2c_client *client)
{
	sensor_write_reg(client, 0x1011,0x01); /*CPU reset*/
	/* Sensor Power ON */
	sensor_write_reg(client, 0x007a,0x0a); /*Set SPCA7002 PLL=432.25M when MCLK of SPCA7002 is 26M.*/
	sensor_write_reg(client, 0x008E,0x08); /*Set SPCA7002 f48M=48.02M when MCLK of SPCA7002 is 26M.*/
	sensor_write_reg(client, 0x0072,0x00); /*Set system clock source.*/
	usleep_range(2000, 2100); /* 2ms */
	sensor_write_reg(client, 0x0084,0x14); /*Set the sensor input clock divider.*/
	sensor_write_reg(client, 0x0034,0xFF); /*Turn on SPCA7002 all clock.*/
	sensor_write_reg(client, 0x9230,0x07); /*Set the I2C clock frequency.*/
	sensor_write_reg(client, 0x9033,0x00); /*Set front sensor reset pin to be low.*/
	sensor_write_reg(client, 0x9035,0x04); /*Set front sensor reset pin to be GPIO mode.*/
	sensor_write_reg(client, 0x9034,0x02); /*Set sensor input clock pin to be function mode.*/
	usleep_range(1000, 1100);/* 1ms */
	sensor_write_reg(client, 0x9032,0x32); /*Set T4K35 power down pin to be high and reset pin to be high. Set front sensor power down pin to be high.*/
	sensor_write_reg(client, 0x9034,0x32); /*Set front sensor power down and T4K35 power down, reset pin to be GPIO mode.*/
	sensor_write_reg(client, 0x9030,0x3F); /*Output enable*/
	sensor_write_reg(client, 0x9031,0x04); /*Output enable*/
	usleep_range(4000, 4100); /* 4ms */
}

static UINT8 SPCA7002_sensor_T4K35_i2c_read(struct i2c_client *client, UINT16 a_uwRegAddr)
{
	UINT8 ucRegVal;
	u16 retValue;
	/*Start - I2C Read ID*/
	...
	//ucRegVal = I2CDataRead(0x9211);	  /* MSB */
	ucRegVal = sensor_read_reg(client, 0x9211, &retValue);	  /* MSB */
	usleep_range(1000, 1100);   /* 1ms */
	/*End - I2C Read*/
	return retValue;
}

static void SPCA7002_sensor_T4K35_i2c_write(struct i2c_client *client, UINT16 a_uwRegAddr, UINT8 a_ucRegVal)
{	/*Start - I2C Write*/
	sensor_write_reg(client, 0x9238,0x30);  /* Sub address enable */
	sensor_write_reg(client, 0x9240,0x6C);  /* Slave address	*/
	sensor_write_reg(client, 0x9200,0x00);  /* I2C Write mode	*/
	sensor_write_reg(client, 0x9210,(a_uwRegAddr>>8)&0xFF);  /* Register addr MSB	*/
	sensor_write_reg(client, 0x9212,(a_uwRegAddr)&0xFF);  /* Register addr LSB*/
	sensor_write_reg(client, 0x9211,(a_ucRegVal)&0xFF);  /* Register value*/
	sensor_write_reg(client, 0x9204,0x01);  /* Trigger I2C write	*/
	usleep_range(5000, 5100);   /* 5ms */
	/*End - I2C Write*/
}

static UINT8 SPCA7002_sensor_T4K35_OTP_read(struct i2c_client *client, UINT8 a_ucPageNum, UINT8 a_ucByteNum)
{
	static UINT8 ucOTPPage=0xff;
	UINT8 ucOTPSizeByteCnt,ucOTPPageReConfigFlg;
	UINT8 ucAccessStateChk,ucErrorStateChk;
	UINT8 ucTrialCnt=0;
	UINT8 ucOTPReadVal=0xff;

	ucOTPSizeByteCnt = a_ucByteNum;

	if (ucOTPPage!=a_ucPageNum)
	{
	    ucOTPPage = a_ucPageNum;
	    ucOTPPageReConfigFlg = 1;
	}
	else
	{
	    ucOTPPageReConfigFlg = 0;
	}


	if (ucOTPPageReConfigFlg == 1)
	{
		/*Start to read T4K35 OTP*/
		SPCA7002_sensor_T4K35_i2c_write(client, 0x3500,0x01);/*Configure OTP read enable*/

		SPCA7002_sensor_T4K35_i2c_write(client, 0x3545,0x06);/*Configure input clock settings to be 24~27MHz.*/

		SPCA7002_sensor_T4K35_i2c_write(client, 0x3502,ucOTPPage);/*Configure OTP page*/
		SPCA7002_sensor_T4K35_i2c_write(client, 0x3547,0x04);/*Timeout check enable*/

		SPCA7002_sensor_T4K35_i2c_write(client, 0x3500,0x81);/*Start OTP access*/
		usleep_range(1000, 1100);
      		ucAccessStateChk = SPCA7002_sensor_T4K35_i2c_read(client, 0x3500);
		while (((ucAccessStateChk&0x80)>>7) == 1) /*Check Access state.*/
		{
	      		usleep_range(5000, 5100);
		      ucAccessStateChk = SPCA7002_sensor_T4K35_i2c_read(client, 0x3500);
	      		ucTrialCnt++;
			if (ucTrialCnt > 50)
			{
				printk("@#@ Warning!!! OTP timeout happened!");
				goto END;
			}
		}

		ucErrorStateChk = SPCA7002_sensor_T4K35_i2c_read(client, 0x3546);
		if ((((ucErrorStateChk)&0x8)>>3)==1)
		{
			printk("@#@ Warning!!! Error happened in OTP read!");
			goto END;
		}
	}

	ucOTPReadVal = (UINT8)(SPCA7002_sensor_T4K35_i2c_read(client, 0x3504+ucOTPSizeByteCnt));
	pr_debug("@#@ OTP[0%x]=0x%x!\n",0x3504+ucOTPSizeByteCnt,ucOTPReadVal);

	//printk("@#@ OTP data read done!\n");
...
static int t4k35_otp_read(struct i2c_client *client)
{
	int reg = 0;
	int i, otp_bytes = 0;
	...
	for (package = 2; package >= 0; package--)
	{
		for (i = 0; i < otp_bytes; i++)
		{
			read_value[i] = SPCA7002_sensor_T4K35_OTP_read(client, package, i);
			pr_debug("otp_value: [%2d]0x%x\n", i, read_value[i]);
		}
...
static int detect_rear_t4k35 (struct spca700xa_16mp_device *dev, struct i2c_client *client) {
	int ret = 0;
	...
	sensor_write_reg(client, 0x1011, 0x01); /*CPU reset*/
	/* Sensor Power ON */
	sensor_write_reg(client, 0x007a, 0x36); /*Set SPCA7002 PLL=503.75M when MCLK of SPCA7002 is 26M.*/
	sensor_write_reg(client, 0x008E, 0x09); /*Set SPCA7002 f48M=50.375M when MCLK of SPCA7002 is 26M.*/
	sensor_write_reg(client, 0x0072, 0x00); /*Set system clock source.*/
	usleep_range(2000, 2100); /* 2ms */
	sensor_write_reg(client, 0x0084, 0x14); /*Set the sensor input clock divider.*/
	sensor_write_reg(client, 0x0034, 0xFF); /*Turn on SPCA7002 all clock.*/
	...
	sensor_write_reg(client, 0x9034, 0x02); /*Set sensor input clock pin to be function mode.*/
	usleep_range(1000, 1100); /* 1ms */
	sensor_write_reg(client, 0x9032, 0x22); /*Set T4K35 power down pin to be low and reset pin to be high. Set front sensor power down pin to be high.*/
	sensor_write_reg(client, 0x9034, 0x32); /*Set front sensor power down and T4K35 power down, reset pin to be GPIO mode.*/
	sensor_write_reg(client, 0x9030, 0x3F); /*Output enable*/
	sensor_write_reg(client, 0x9031, 0x04); /*Output enable*/
	usleep_range(4000, 4100); /* 4ms */
	...
	sensor_write_reg(client, 0x9204, 0x01);	/* Trigger I2C read */
	usleep_range(3000, 3100);   /* 3ms */
	sensor_read_reg(client, 0x9211, &retvalue_h);	  /* MSB */
	usleep_range(1000, 1100);   /* 1ms */
	/* Read Sensor ID (LSB)*/
	sensor_write_reg(client, 0x9238, 0x30);	/* Sub address enable */
	sensor_write_reg(client, 0x9240, 0x6C);	/* Slave address	*/
	sensor_write_reg(client, 0x9200, 0x03);	/* I2C Read mode	*/
	sensor_write_reg(client, 0x9210, 0x00);	/* Register addr MSB	*/
	sensor_write_reg(client, 0x9212, 0x01);	/* Register addr LSB*/
	sensor_write_reg(client, 0x9204, 0x01);	/* Trigger I2C read */
	usleep_range(3000, 3100);  /* 2ms */
	sensor_read_reg(client, 0x9211, &retvalue_l);	  /* LSB */
	usleep_range(1000, 1100);   /* 1ms */

        sensorid = ((retvalue_h<<8)|(retvalue_l))&0xffff;
	printk("detect_rear_t4k35: read sensor id: 0x%04X",  sensorid);
        if (sensorid == 0x1481)
...
static int detect_rear_ov5670 (struct spca700xa_16mp_device *dev, struct i2c_client *client) {
	int ret = 0;
	u16 sensorid, retvalue_h, retvalue_l;

        // detect ov5670
        sensor_write_reg(client, 0x1011, 0x01);

        sensor_write_reg(client, 0x0084, 0x14);
        sensor_write_reg(client, 0x0034, 0xFF);
        sensor_write_reg(client, 0x9030, 0x3F);
        sensor_write_reg(client, 0x9031, 0x04);
        sensor_write_reg(client, 0x9034, 0xF2);
        sensor_write_reg(client, 0x9035, 0x04);
        ...
        sensor_write_reg(client, 0x9238, 0x30);
        sensor_write_reg(client, 0x9240, 0x20);
        sensor_write_reg(client, 0x9200, 0x03);
        sensor_write_reg(client, 0x9210, 0x30);
        sensor_write_reg(client, 0x9212, 0x0B);
        sensor_write_reg(client, 0x9204, 0x01);
        usleep_range(2000, 2100);
        sensor_read_reg(client, 0x9211, &retvalue_h);

        sensor_write_reg(client, 0x9210, 0x30);
        sensor_write_reg(client, 0x9212, 0x0C);
        sensor_write_reg(client, 0x9204, 0x01);
        usleep_range(2000, 2100);
        sensor_read_reg(client, 0x9211, &retvalue_l);

        sensorid = ((retvalue_h<<8)|(retvalue_l))&0xffff;
	printk("detect_rear_ov5670: read sensor id: 0x%04X",  sensorid);
        if (sensorid == 0x5670)
              pltfrm_camera_module_pr_info(&(dev->sd),
                      "successfully detected camera ID 0x%x\n",
                      sensorid);
...
static int spca700xa_16mp_detect(struct spca700xa_16mp_device *dev, struct i2c_client *client)
{
	...
	if(camera_id == MAIN_CAMERA_ID_OV) {
		ret = detect_rear_ov5670(dev, client);
		if(ret) {
			goto err;
		}
		// ASUS BSP ++, read ov5670 otp data
		sensor_name = "ov5670_iCatch";
		ov5670_otp_read(client);
		// ASUS BSP --
	} else {
		ret = detect_rear_t4k35(dev, client);
		if(ret) {
			goto err;
		}
		// ASUS BSP ++, read t4k35 otp data
		sensor_name = "t4k35_iCatch";
		t4k35_otp_read(client);
		// ASUS BSP --
	}
...
static int
spca700xa_16mp_s_config(struct v4l2_subdev *sd)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	struct inode *inode;
...
static int spca700xa_16mp_queryctrl(struct v4l2_subdev *sd, struct v4l2_queryctrl *qc)
{
	struct spca700xa_16mp_control *ctrl = spca700xa_16mp_find_control(qc->id);
        pr_debug("%s \n", __func__);
	if (ctrl == NULL)
		return -EINVAL;
	*qc = ctrl->qc;
...
static int spca700xa_16mp_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct spca700xa_16mp_control *octrl = spca700xa_16mp_find_control(ctrl->id);
	int ret;
        //printk("%s\n", __func__);
...
static int spca700xa_16mp_s_ctrl_old(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct spca700xa_16mp_control *octrl = spca700xa_16mp_find_control(ctrl->id);
	int ret;
        pr_debug("%s\n", __func__);
...
int spca700xa_16mp_s_ext_ctrls(struct v4l2_subdev *sd,struct v4l2_ext_controls *ctrls)
{
	int i;
	int ret = 0;
	pr_debug("%s\n", __func__);
	...
	for (i = 0; i < ctrls->count; i++) {
		...
		switch (ctrl->id) {
			case V4L2_CID_GSENSOR:
				sensor_s_gsensor_data(sd, ctrl->string);
				break;
			case V4L2_CID_FOCUS_AUTO:
				sensor_s_focus_window(sd,ctrl->string);
				break;
			case V4L2_CID_EXPOSURE_AREAS:
				sensor_s_exposure_window(sd, ctrl->string);
				break;
...
static int spca700xa_16mp_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret = 0;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	//struct spca700xa_16mp_device *dev = to_spca700xa_16mp_sensor(sd);
	...
	if (enable) {
		...
		sensor_write_reg(client, 0x7106, 0x00);
		sensor_write_reg(client, 0x7120, 0x00);
		sensor_write_reg(client, 0x7121, 0x01);
#endif
		ret = sensor_read_reg(client, 0x90CC, &value); //low byte of front sensor ID
		printk("%s value 0x90CC %x\n", __func__, value);
		ret = sensor_read_reg(client, 0x90CD, &value); //low byte of front sensor ID
		printk("%s value 0x90CD %x\n", __func__, value);
		ret = sensor_read_reg(client, 0x90CE, &value); //low byte of front sensor ID
		printk("%s value 0x90CE %x\n", __func__, value);
		ret = sensor_read_reg(client, 0x90CF, &value); //low byte of front sensor ID
		printk("%s value 0x90CF %x\n", __func__, value);

		if (mHDR) {
			sensor_write_reg(client, 0x7120, 0x01);
			pr_info("HDR: set to capture mode\n");
		...
		if (first_on == 1){
			...
			if (mLowLightMode && mLowLightCapture && skip_on) {
				printk("%s skip set 0x711B for low light/HDR capture\n", __func__);
				goto stream_end;
			} else if (mLowLightMode && mLowLightCapture && !skip_on) {
				sensor_write_reg(client, 0x711b, 0x72);
				skip_on = true;
				printk("%s set 0x711B = 0x72 for low light capture\n", __func__);
			} else if (mLowLightMode && !mLowLightCapture) {
				sensor_write_reg(client, 0x711b, 0x76);
				skip_on = false;
				printk("%s set 0x711B = 0x76 for low light preivew\n", __func__);
			} else if (mNightMode) {
				sensor_write_reg(client, 0x711b, 0x64);
				skip_on = false;
...
stream_end:
	...
	sensor_read_reg(client, 0x002C, &testval);
	printk("addr=0x002C, value=0x%x \n",testval);
	sensor_read_reg(client, 0x0082, &testval);
	printk("addr=0x0082, value=0x%x \n",testval);
	sensor_read_reg(client, 0x008C, &testval);
	printk("addr=0x008C, value=0x%x \n",testval);
	sensor_read_reg(client, 0x90D6, &testval);
	printk("addr=0x90D6, value=0x%x \n",testval);
	sensor_read_reg(client, 0x2108, &testval);
	printk("addr=0x2108, value=0x%x \n",testval);
	sensor_read_reg(client, 0x7049, &testval);
...
static int
spca700xa_16mp_enum_framesizes(struct v4l2_subdev *sd, struct v4l2_frmsizeenum *fsize)
{
	...
	fsize->discrete.width = spca700xa_16mp_res[index].width;
	fsize->discrete.height = spca700xa_16mp_res[index].height;

	return 0;
}

static int spca700xa_16mp_enum_frameintervals(struct v4l2_subdev *sd,
				       struct v4l2_frmivalenum *fival)
{
	...
	for (i = 0; i < N_RES; i++) {
		printk("res[%d]=%dx%d >= %dx%d\n", index, spca700xa_16mp_res[i].width, spca700xa_16mp_res[i].height, fival->width, fival->height);
		if ((spca700xa_16mp_res[i].width >= fival->width) &&
		    (spca700xa_16mp_res[i].height >= fival->height))
			break;
	}
	if (i == N_RES)
		i--;
	...
	fival->width = spca700xa_16mp_res[index].width;
	fival->height = spca700xa_16mp_res[index].height;
	fival->type = V4L2_FRMIVAL_TYPE_DISCRETE;
	fival->discrete.numerator = 1;
	fival->discrete.denominator = spca700xa_16mp_res[index].fps;
#if 1
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
	if (mLowLightMode) {
		if (camera_id == MAIN_CAMERA_ID_TK) {
			if (fival->width >= SPCA700XA_16MP_RES_VGA_SIZE_H ||
				fival->height >= SPCA700XA_16MP_RES_VGA_SIZE_V) {
				fival->width = SPCA700XA_16MP_RES_2M_SIZE_H;
				fival->height = SPCA700XA_16MP_RES_2M_SIZE_V;
				fival->discrete.denominator = 30;
			}
		} else {
			if (fival->width >= SPCA700XA_16MP_RES_VGA_SIZE_H ||
				fival->height >= SPCA700XA_16MP_RES_VGA_SIZE_V) {
				fival->width = SPCA700XA_16MP_RES_960P_SIZE_H;
				fival->height = SPCA700XA_16MP_RES_960P_SIZE_V;
				fival->discrete.denominator = 30;
			}
		}
	}
#else if defined(CONFIG_Z170CG) || defined(CONFIG_Z170C)
	if (mLowLightMode) {
		if (fival->width >= SPCA700XA_16MP_RES_VGA_SIZE_H ||
			fival->height >= SPCA700XA_16MP_RES_VGA_SIZE_V) {
			fival->width = SPCA700XA_16MP_RES_960P_SIZE_H;
...
static int spca700xa_16mp_enum_mbus_code(struct v4l2_subdev *sd,
...
static int spca700xa_16mp_enum_frame_size(struct v4l2_subdev *sd,
	struct v4l2_subdev_fh *fh,
	struct v4l2_subdev_frame_size_enum *fse)
{
	...
	fse->min_width = spca700xa_16mp_res[index].width;
	fse->min_height = spca700xa_16mp_res[index].height;
	fse->max_width = spca700xa_16mp_res[index].width;
	fse->max_height = spca700xa_16mp_res[index].height;

	return 0;
}

static struct v4l2_mbus_framefmt *
__spca700xa_16mp_get_pad_format(struct spca700xa_16mp_device *sensor,
...
static int
spca700xa_16mp_get_pad_format(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh,
		       struct v4l2_subdev_format *fmt)
{
	struct spca700xa_16mp_device *snr = to_spca700xa_16mp_sensor(sd);
	struct v4l2_mbus_framefmt *format =
			__spca700xa_16mp_get_pad_format(snr, fh, fmt->pad, fmt->which);
...
static int
spca700xa_16mp_set_pad_format(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh,
		       struct v4l2_subdev_format *fmt)
{
	struct spca700xa_16mp_device *snr = to_spca700xa_16mp_sensor(sd);
	struct v4l2_mbus_framefmt *format =
...
static int spca700xa_16mp_s_ctrl(struct v4l2_ctrl *ctrl)
{
	//struct spca700xa_16mp_device *snr = container_of(
	//	ctrl->handler, struct spca700xa_16mp_device, ctrl_handler);
        pr_debug("%s\n", __func__);
...
static int spca700xa_16mp_g_skip_frames(struct v4l2_subdev *sd, u32 *frames)
{
	int index;
	struct spca700xa_16mp_device *snr = to_spca700xa_16mp_sensor(sd);
	...
	for (index = 0; index < N_RES; index++) {
		if (spca700xa_16mp_res[index].res == snr->res)
			break;
	}
	...
	*frames = spca700xa_16mp_res[index].skip_frames;

	return 0;
}
#if 0
static int spca700xa_16mp_s_modes(struct v4l2_subdev *sd, u32 modes)
{
       //struct spca700xa_16mp_device *snr = to_spca700xa_16mp_sensor(sd);
       switch (modes) {
...
static int spca700xa_16mp_enum_mbus_fmt(struct v4l2_subdev *sd, unsigned int index,
...
static int
 spca700xa_16mp_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *param)
{
//	struct spca700xa_16mp_device *dev = to_spca700xa_16mp_sensor(sd);
...
int
spca700xa_16mp_g_frame_interval(struct v4l2_subdev *sd,
...
static int spca700xa_16mp_s_frame_interval(struct v4l2_subdev *sd,
			struct v4l2_subdev_frame_interval *interval)
{
	return 0;
}

static const struct v4l2_subdev_video_ops spca700xa_16mp_video_ops = {
	.s_stream = spca700xa_16mp_s_stream,
	.enum_framesizes = spca700xa_16mp_enum_framesizes,
	.enum_frameintervals = spca700xa_16mp_enum_frameintervals,
	.try_mbus_fmt = spca700xa_16mp_try_mbus_fmt,
	.s_mbus_fmt = spca700xa_16mp_set_mbus_fmt,
	.g_mbus_fmt = spca700xa_16mp_get_mbus_fmt,

	.enum_mbus_fmt = spca700xa_16mp_enum_mbus_fmt,
	.s_parm = spca700xa_16mp_s_parm,
	.g_frame_interval = spca700xa_16mp_g_frame_interval,
	.s_frame_interval =spca700xa_16mp_s_frame_interval,
};

static struct v4l2_subdev_sensor_ops spca700xa_16mp_sensor_ops = {
        .g_skip_frames = spca700xa_16mp_g_skip_frames,
//        .s_modes = spca700xa_16mp_s_modes,
};

static const struct v4l2_subdev_core_ops spca700xa_16mp_core_ops = {
	.queryctrl = spca700xa_16mp_queryctrl,
	.g_ctrl = spca700xa_16mp_g_ctrl,
	.s_ctrl = spca700xa_16mp_s_ctrl_old,
	.s_ext_ctrls = spca700xa_16mp_s_ext_ctrls,
	.s_power = spca700xa_16mp_s_power,
};

static const struct v4l2_subdev_pad_ops spca700xa_16mp_pad_ops = {
	.enum_mbus_code = spca700xa_16mp_enum_mbus_code,
	.enum_frame_size = spca700xa_16mp_enum_frame_size,
	.get_fmt = spca700xa_16mp_get_pad_format,
	.set_fmt = spca700xa_16mp_set_pad_format,
};

static const struct v4l2_subdev_ops spca700xa_16mp_ops = {
	.core = &spca700xa_16mp_core_ops,
	.video = &spca700xa_16mp_video_ops,
	.pad = &spca700xa_16mp_pad_ops,
	.sensor = &spca700xa_16mp_sensor_ops,
};

static const struct media_entity_operations spca700xa_16mp_entity_ops;


static int spca700xa_16mp_remove(struct i2c_client *client)
{
	struct spca700xa_16mp_device *dev;
	struct v4l2_subdev *sd = i2c_get_clientdata(client);

	dev = container_of(sd, struct spca700xa_16mp_device, sd);
#if 0
...
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
#define  GPIO_MAIN_CAM_Z370 56
#define  GPIO_SUB_CAM_Z370 65
...
static int getCamID_Z370(void)
{
	int ret = -1;
	int main_cam_id = get_gpio(GPIO_MAIN_CAM_Z370);
	switch (main_cam_id) {
		case 1: 
...
static int getSubCamID_Z370(void)
{
	int ret = -1;
	int sub_cam_id = get_gpio(GPIO_SUB_CAM_Z370);
	switch (sub_cam_id) {
		case 1: 
			return SUB_CAMERA_ID_2M;
			break;
		case 0:
		default:
			return SUB_CAMERA_ID_03M;
			break;
	}
	return ret;
}
...
static int getCamID_Z170(void)
{
	int ret = -1;
	int main_cam_id = get_gpio(GPIO_MAIN_CAM_Z170);
	switch (main_cam_id) {
		case 1: 
			return MAIN_CAMERA_ID_2M;
			break;
		case 0:
		default:
			return MAIN_CAMERA_ID_5M;
			break;
	}
	return ret;
}
...
static int spca700xa_16mp_probe(struct i2c_client *client,
		       const struct i2c_device_id *id)
{
	...
	//Add for ATD read camera status+++
	dev->sensor_i2c_attribute.attrs = spca700xa_16mp_attributes;
	...
	v4l2_i2c_subdev_init(&dev->sd, client, &spca700xa_16mp_ops);
#if 0
	if (client->dev.platform_data) {
		ret = spca700xa_16mp_s_config(&dev->sd, client->irq,
	...
	if (ret) {
		pltfrm_camera_module_pr_err(&(dev->sd),
			"failed with error %d\n", ret);
		spca700xa_16mp_remove(client);
		return ret;
	}

	pltfrm_camera_module_pr_info(&(dev->sd), "%s %d-%04x\n",
		SPCA700XA_16MP_DRIVER_NAME,
	...
	ret = spca700xa_16mp_detect(dev, client);
	if (ret) {
		pltfrm_camera_module_pr_err(&(dev->sd),
			"failed with error %d\n", ret);
		spca700xa_16mp_remove(client);
...
static struct of_device_id spca700xa_16mp_of_match[] = {
	{.compatible = "icatch," SPCA700XA_16MP_DRIVER_NAME "-v4l2-i2c-subdev",},
	{},
};

MODULE_DEVICE_TABLE(i2c, spca700xa_16mp_id);

static struct i2c_driver spca700xa_16mp_driver = {
        .driver = {
                .owner = THIS_MODULE,
                .name = SPCA700XA_16MP_DRIVER_NAME,
                .of_match_table = spca700xa_16mp_of_match//Clark add
        },
        .probe = spca700xa_16mp_probe,
        .remove = __exit_p(spca700xa_16mp_remove),
        .id_table = spca700xa_16mp_id,
};

static struct i2c_driver spca700xa_16mp_5m_driver = {
        .driver = {
                .owner = THIS_MODULE,
                .name = SPCA700XA_16MP_5M_DRIVER_NAME,
                .of_match_table = spca700xa_16mp_of_match//Clark add
        },
        .probe = spca700xa_16mp_probe,
        .remove = __exit_p(spca700xa_16mp_remove),
...
static struct i2c_driver spca700xa_16mp_8m_driver = {
        .driver = {
                .owner = THIS_MODULE,
                .name = SPCA700XA_16MP_8M_DRIVER_NAME,
                .of_match_table = spca700xa_16mp_of_match//Clark add
        },
        .probe = spca700xa_16mp_probe,
        .remove = __exit_p(spca700xa_16mp_remove),
...
static __init int init_spca700xa_16mp(void)
{
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
        camera_id = getCamID_Z370();
        if(camera_id == MAIN_CAMERA_ID_OV)
            return i2c_add_driver(&spca700xa_16mp_5m_driver);
        else
            return i2c_add_driver(&spca700xa_16mp_8m_driver);
#else
	return i2c_add_driver(&spca700xa_16mp_5m_driver);
#endif
}

static __exit void exit_spca700xa_16mp(void)
{
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
        camera_id = getCamID_Z370();
        if(camera_id == MAIN_CAMERA_ID_OV)
	    i2c_del_driver(&spca700xa_16mp_5m_driver);
        else
            i2c_del_driver(&spca700xa_16mp_8m_driver);
#else
        i2c_del_driver(&spca700xa_16mp_5m_driver);
#endif
}

//module_i2c_driver(spca700xa_16mp_driver);
module_init(init_spca700xa_16mp);//Clark mark
module_exit(exit_spca700xa_16mp);//Clark mark
