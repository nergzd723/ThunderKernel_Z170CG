...
#endif
static unsigned char *sensor_name = HM2051_NAME;
...
int ATD_hm2051_status = 0;  //Add for ATD read camera status
...
static struct attribute_group hm2051_attribute_group = {
	.attrs = hm2051_attributes
};
//Add for ATD read camera status---
...
static int hm2051_dbgfs_init_370(void)
{
	struct dentry *debugfs_dir;
...
static int hm2051_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on == 0)
        ...
        printk("DEBUG: %s, hm2051_global_setting\n", __func__);
...
#endif
        printk("DEBUG: %s, hm2051_res dev->fmt_idx: %d\n", __func__, dev->fmt_idx);
...
#endif
        ATD_hm2051_status = 1;
	dev_info(&client->dev, "detect hm2051 success\n");
	return 0;
}
...
static int hm2051_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	/*if (enable == 0) {
		printk("DEBUG: %s, stream off sleep 133 ms\n", __func__);
                msleep(133);
	}
	else {
...
#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
        printk("DEBUG: %s, support hm2051 for Z370CG BMMI test\n", __func__);
        hm2051_dbgfs_init_370();
#else
        hm2051_dbgfs_init();
#endif
			...
			const struct i2c_device_id *id)
{
	...
	if (!dev) {
        ...
        ret = sysfs_create_group(&client->dev.kobj, &hm2051_attribute_group);
        if (ret) {
