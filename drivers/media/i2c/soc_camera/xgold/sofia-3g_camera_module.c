...
#define OF_OV_GPIO_ICATCH_DVDD "icatch,pd-gpio_1v2"
#define OF_OV_GPIO_ICATCH_DVDD_SR "icatch,pd-gpio_1v2_sr"
#define OF_OV_GPIO_ICATCH_AVDD "icatch,pd-gpio_2v8"
...
const char *PLTFRM_CAMERA_MODULE_PIN_ICATCH_AVDD = OF_OV_GPIO_ICATCH_AVDD;
const char *PLTFRM_CAMERA_MODULE_PIN_ICATCH_DVDD = OF_OV_GPIO_ICATCH_DVDD;
...
int pltfrm_camera_module_set_pinctrl_state(
...
int pltfrm_camera_module_init_gpio(
	struct v4l2_subdev *sd)
{
	int ret = 0;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
int pltfrm_camera_module_release_gpio(
	struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct pltfrm_camera_module_data *pdata =
...
int get_hw_id(void) //OK
{
	int ret = -1;
	int tmp = 0;
        int hardware_id = 0;
	tmp = get_gpio(GPIO_HARDWARE_ID_2);
	...
	struct v4l2_subdev *sd)
{
	...
	char *gpio_name = PLTFRM_CAMERA_MODULE_PIN_ICATCH_DVDD;

#if defined(CONFIG_Z370CG) || defined(CONFIG_Z370C)
	if(get_hw_id() == SR) {
		gpio_name = OF_OV_GPIO_ICATCH_DVDD_SR;
	}
#endif
	...
	pdata->gpios[5].label = PLTFRM_CAMERA_MODULE_PIN_ICATCH_AVDD;
	pdata->gpios[5].pltfrm_gpio =
		of_get_named_gpio_flags(np, pdata->gpios[5].label, 0, NULL);
	pdata->gpios[5].active_low =
		of_property_read_bool(np, OF_OV_GPIO_ICATCH_AVDD "-is_active_low");

	pdata->gpios[6].label = PLTFRM_CAMERA_MODULE_PIN_ICATCH_DVDD;
//	pdata->gpios[6].pltfrm_gpio =
//		of_get_named_gpio_flags(np, pdata->gpios[6].label, 0, NULL);
	pdata->gpios[6].pltfrm_gpio =
		of_get_named_gpio_flags(np, gpio_name, 0, NULL);
	pdata->gpios[6].active_low =
		of_property_read_bool(np, OF_OV_GPIO_ICATCH_DVDD "-is_active_low");
