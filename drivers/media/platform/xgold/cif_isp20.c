	...
	struct cif_isp20_device *dev)
{
	...
	// +++ Hugh:  20150226 for iCatch will not merge into intel codebase, please don't remove
	if ((dev->mp_stream.updt_cfg == true)
		&& CIF_ISP20_PIX_FMT_IS_YUV(request_strm_fmt.frm_fmt.pix_fmt)
		&& CIF_ISP20_PIX_FMT_IS_RAW_BAYER(dev->config.mi_config.mp.output.pix_fmt))
	{
		request_strm_fmt.frm_fmt.pix_fmt = dev->config.mi_config.mp.output.pix_fmt;
	}
	// --- Hugh:  20150226
	...
	enum cif_isp20_inp inp)
{
	...
	if ((inp == CIF_ISP20_INP_DMA) || (inp == CIF_ISP20_INP_DMA_IE))
		...
		if (inp == CIF_ISP20_INP_CSI_0) {
			...
			//Wesley 20150213, for iCatch switch fail ++ will not merge into intel codebase, please don't remove
			if (!strcmp(cif_isp20_img_src_g_name(dev->img_src), "spca700xa_16mpf"))
				dev->config.mipi_config.input_sel = 0;
			else
				dev->config.mipi_config.input_sel = 1;
			//Wesley 20150213, for iCatch switch fail --
	...
	int val)
{
    ...
    case CIF_CUSTOM_IOCTL_REG_SET:
	case CIF_ISP20_CID_AUTO_FOCUS_START:
	case CIF_ISP20_CID_PAN_RELATIVE:
	case CIF_ISP20_CID_TILT_RELATIVE:
	case CIF_ISP20_CID_PAN_RESET:
	case CIF_ISP20_CID_TILT_RESET:
	case CIF_ISP20_CID_PAN_ABSOLUTE:
	case CIF_ISP20_CID_TILT_ABSOLUTE:
	case CIF_ISP20_CID_HDR:
	case CIF_ISP20_CID_AUTO_FOCUS_RANGE:
	case CIF_ISP20_CID_LOW_LIGHT_CAPTURE:
	case CIF_ISP20_CID_AUTO_FOCUS_STATUS:
	case CIF_ISP20_CID_METERING_MODE:
        case CIF_CUSTOM_CID_3A_LOCK:
