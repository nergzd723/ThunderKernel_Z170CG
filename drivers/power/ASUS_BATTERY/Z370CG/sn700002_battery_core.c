...
//Carlisle add for HWID judgement +++
static int HW_ID;
//Carlisle add for HWID judgement ---

int bq27621_unseal_block_data_memory(void);
int bq27621_set_cfg_update(void);
int bq27621_is_config_update_mode(void);
int bq27621_enable_block_data_memory_control(void);
int bq27621_access_state_subclass(int id);
int bq27621_write_block_offset_location(int offset);
int bq27621_read_checksum(int *csum);
int bq27621_read_block_data_memory_value(int offset, int *value);
int bq27621_write_block_data_memory_value(int offset, int value);
int bq27621_write_checksum(int csum);
int bq27621_set_soft_reset(void);
int bq27621_seal_block_data_memory(void);
...
[REG_MANUFACTURER_DATA] = sn200007_DATA(PRESENT, 0, 0, 65535),
	    ...
	    [REG_CURRENT] =
	    sn200007_DATA(CURRENT_NOW, 0x10, -32768, 32767),
...
int sn200007_smbus_read_data(int reg_offset, int byte, int *rt_value)
{
	...
	do {
		ret =
		    sn200007_read_i2c(sn200007_data[reg_offset].addr, rt_value,
				      0);
...
int sn200007_smbus_write_data(int reg_offset, int byte, unsigned int value)
{
	...
	do {
		if (byte) {
			ret =
			    i2c_smbus_write_byte_data(sn200007_device->client,
						      sn200007_data
						      [reg_offset].addr,
						      value & 0xFF);
		} else {
			ret =
			    i2c_smbus_write_word_data(sn200007_device->client,
						      sn200007_data
						      [reg_offset].addr,
						      value & 0xFFFF);
		}
...
static int sn200007_get_capacity(void)
{
	...
	do {
		sn200007_device->smbus_status =
		    sn200007_smbus_read_data(REG_CAPACITY, 0,
					     &sn200007_device->bat_capacity);
		if (sn200007_device->smbus_status < 0) {
			dev_err(&sn200007_device->client->dev,
				"%s: i2c read for %d failed sn200007_device->cap_err=%u\n",
				__func__, REG_CAPACITY, sn200007_device->cap_err);
			if (sn200007_device->cap_err > 5
			    || (sn200007_device->old_capacity == 0xFF)) {
				return -EINVAL;
			} else {
	...
	temp_capacity =
	    ((sn200007_device->bat_capacity >=
	      100) ? 100 : sn200007_device->bat_capacity);
	...
	sn200007_device->smbus_status =
	    sn200007_smbus_read_data_reg(0x0C, 0, &remainingcapacity);
	if (sn200007_device->smbus_status >= 0) {
...
static int sn200007_get_current(void)
{
	...
	sn200007_device->smbus_status =
	    sn200007_smbus_read_data_reg(sn200007_data[REG_CURRENT].addr, 0,
					 &rt_value);
	if (sn200007_device->smbus_status < 0) {
		dev_err(&sn200007_device->client->dev,
			"%s: i2c read for %d failed\n", __func__,
			sn200007_data[REG_CURRENT].addr);
	}
	if (rt_value & BIT15)
		rt_value |= 0xFFFF0000;	//neg. sign extend.
...
static int sn200007_get_temperature(void)
{
	...
	if(HW_ID <= 1){ //For ER PR8107 issue
		/* Gauge battery temp*/
		sn200007_device->smbus_status =
	    sn200007_smbus_read_data_reg(sn200007_data[REG_TEMPERATURE].addr, 0,
					 &rt_value);
		if (sn200007_device->smbus_status < 0) {
			dev_err(&sn200007_device->client->dev,
				"%s: i2c read for %d failed\n", __func__,
				sn200007_data[REG_TEMPERATURE].addr);
		}		
		...
		if(sn200007_device->old_temperature == GETTHERMALZONENULL){
			printk("[%s] Fail to read the thermal zone. please check get_thermal_zone_temperature function , Error Code : %d",__func__,sn200007_device->old_temperature);
		}
		if(sn200007_device->old_temperature < -300){	//Workaround to prevent thermal driver report a large negative value.
			sn200007_device->smbus_status =
			sn200007_smbus_read_data_reg(sn200007_data[REG_TEMPERATURE].addr, 0, &rt_value);
			if (sn200007_device->smbus_status < 0) {
				dev_err(&sn200007_device->client->dev,
...
static void batt_capacity_opti(void){
	...
	if(sn200007_device->old_capacity<=1){
     ...
     if( ( (G_USB_TYPE == AC_IN) || (G_USB_TYPE == USB_IN) || (G_PACK_AC_ONLINE == 1) ) && (sn200007_device->old_capacity == 100) ){ // CABLE IN & USOC = 100
		FC = true;
		set_FC_id_to_File(1);
		...
		if(sn200007_device->bat_current > 0){
			...
			if(FC == false){	
				...
				if(sn200007_device->old_capacity <=94){
					...
					if(PSOC >= 100){
						PSOC = 100;
						printk("[%s][WB] PSOC >=100 setup to 100%%\n",__func__);
					}else{
						PSOC +=1;
						printk("[%s][WB] PSOC < 100 , PSOC += 1 , PSOC = %d \n",__func__,PSOC);
					}
	...
	}else if(( (G_USB_TYPE == AC_IN) || (G_USB_TYPE == USB_IN) || (G_PACK_AC_ONLINE == 1) ) && (sn200007_device->old_capacity < 100)){ // CABLE IN & USOC < 100
		// Debug 
...
void sn200007_update_all(void)
{
	...
	// 2. voltage
	sn200007_device->smbus_status =
	    sn200007_smbus_read_data_reg(sn200007_data[REG_VOLTAGE].addr, 0,
					 &rt_value);
	if (sn200007_device->smbus_status < 0) {
		dev_err(&sn200007_device->client->dev,
			"%s: i2c read for %d failed\n", __func__,
			sn200007_data[REG_VOLTAGE].addr);
	}
	sn200007_device->bat_vol = rt_value;
	...
	// 5. battery capacity optimization
    batt_capacity_opti(); 
...
static void sn200007_stress_test_poll(struct work_struct *work)
{
	...
	ret =
	    sn200007_smbus_read_data_reg(sn200007_data[REG_CAPACITY].addr, 0,
					 &percentage);
	if (ret < 0) {
		BAT_ERR
		    ("------------> sn200007 gauge i2c fail !!!!! <--------------------\n");
	} else {
		BAT_NOTICE
		    ("------------> stress test percentage  = %d , polling = %d sec <--------------------\n",
		     percentage, stress_test_poll_mode / HZ);
	}

	if (!smb345_has_charger_error())
		BAT_NOTICE
		    ("------------> smb345 charger status successful , polling = %d sec <--------------------\n",
		     stress_test_poll_mode / HZ);
	else
		BAT_ERR
		    ("------------> smb345 charger status fail !!!!! <--------------------\n");

	queue_delayed_work(sn200007_stress_test_work_queue,
			   &sn200007_stress_test_poll_work,
...
static void battery_status_poll(struct work_struct *work)
{
	...
	struct sn200007_device_info *batt_dev =
	    container_of(work, struct sn200007_device_info,
	...
	if( ( (G_USB_TYPE == AC_IN) || (G_USB_TYPE == USB_IN) ) && (sn200007_device->bat_current > 0)  ){
		...
		if (PSOC >= 50
			&& PSOC <= 100)
			polling_time =
				(90 * HZ) < polling_time ? (90 * HZ) : polling_time;
		else if (PSOC >= 20
			&& PSOC <= 49)
			polling_time =
				(60 * HZ) < polling_time ? (60 * HZ) : polling_time;
		else if (PSOC >= 5
			&& PSOC <= 19)
			polling_time =
				(30 * HZ) < polling_time ? (30 * HZ) : polling_time;
		else if (PSOC >= 0
			&& PSOC <= 4)
			polling_time =
				(30 * HZ) < polling_time ? (30 * HZ) : polling_time;
			...
			BAT_NOTICE
				("*** Battery percentage is out of the legal range (percentage < 0 or percentage > 100) ***\n");
			polling_time = 5 * HZ;
		}
	}
	
	printk
	    ("<BATT> battery info (P:%d %%(%d %%)(%d %%), V:%d mV, C:%d mA, T:%d.%d P: %d secs S: %s)\n",
		 PSOC,
	     sn200007_device->old_capacity, sn200007_device->bat_capacity,
	     sn200007_device->bat_vol, sn200007_device->bat_current,
	     sn200007_device->old_temperature / 10,
	     sn200007_device->old_temperature % 10, polling_time / HZ,
	     status_text[sn200007_device->bat_status]);

	/* Schedule next polling */
	queue_delayed_work(battery_poll_work_queue, &batt_dev->status_poll_work,
			   polling_time);
}
...
static void low_low_battery_check(struct work_struct *work)
{
	...
	queue_delayed_work(battery_poll_work_queue,
			   &sn200007_device->status_poll_work, 0.1 * HZ);
	msleep(2000);
...
static int sn200007_get_health(enum power_supply_property psp,
			       union power_supply_propval *val)
{
				 ...
				 union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = sn200007_device->bat_current;
		break;
...
static int proc_sn200007_test_info_dump_open(struct inode *inode,
					     struct file *file)
{
...
static int sn200007_register_upilogger_proc_fs(void)
{
	...
	entry =
	    proc_create("bq27520_test_info_dump", 0664, NULL,
			&proc_sn200007_test_info_dump_ops);
	if (!entry) {
...
//	if (checkBatteryInvalidStatus())
		queue_delayed_work(battery_poll_work_queue,
				   &sn200007_device->status_poll_work, 0 * HZ);
/*
	else
		queue_delayed_work(battery_poll_work_queue,
...
/**
 * Firmware Update for TI bq27621
 */
...
int bq27520_write_i2c(struct i2c_client *client, u8 reg, int value,
		      int b_single)
{
	struct i2c_msg msg;
...
#ifdef ASUS_ENG_BUILD
		dev_err(&client->dev,
			"I2C write to 0x%02X error with code: %d\n", reg, -EIO);
#endif
		return -EIO;
	}
...
int bq27520_read_i2c(struct i2c_client *client, u8 reg, int *rt_value,
		     int b_single)
{
	struct i2c_msg msg[1];
	unsigned char data[2];
...
int bq27520_cmp_i2c(int reg_off, int value)
{
	...
	while (retry--) {
		ret =
		    bq27520_read_i2c(sn200007_device->client, reg_off, &val, 1);
		if (ret < 0)
			continue;
...
int bq27520_asus_battery_dev_read_fw_cfg_version(void)
{
	...
	if (ret) {
		dev_err(sn200007_device->client,
...
int bq27621_asus_battery_dev_read_fw_cfg_version(void)
{
	int DV = 0;
	int ret = 0, temp = 0, count = 0;

	/* 1. UNSEAL it by sending the appropriate keys to Control( ) (0x00 and 0x01) */
	ret = bq27621_unseal_block_data_memory();
	if (ret < 0) {
	 ...
	 */
	count = 0;
	while (bq27621_is_config_update_mode() != 1) {
		count++;
		msleep(500);
	 ...
	 *    memory control
	 */
	ret = bq27621_enable_block_data_memory_control();
	if (ret < 0) {
		dev_err(sn200007_device->client,
	 ...
	 */
	ret = bq27621_access_state_subclass(0x40);
	if (ret < 0) {
		dev_err(sn200007_device->client,
			"Access state subclass error %d.\n", ret);
		return ret;
	}
	 ...
	 */
	ret = bq27621_write_block_offset_location(3);
	if (ret < 0) {
		dev_err(sn200007_device->client,
			"Write block offset location error %d.\n", ret);
		return ret;
	}

	/* 8. Read DF Version byte at 0x43 (offset = 3). */
	ret = bq27621_read_block_data_memory_value(3, &DV);
	if (ret < 0) {
		dev_err(sn200007_device->client, "Read DF Version error %d.\n",
			ret);
		return ret;
	}
	pr_info("%s: DF Version is %x\n", __func__, DV);

	/*
	 * 12. Exit CONFIG UPDATE mode by sending SOFT_RESET subcommand,
	 *     Control(0x0042)
	 */
	ret = bq27621_set_soft_reset();
	if (ret < 0) {
		dev_err(sn200007_device->client,
			"Exit CONFIG UPDATE mode error %d.\n", ret);
		return ret;
	}
	 ...
	 */
	count = 0;
	while (bq27621_is_config_update_mode() != 0) {
		count++;
		msleep(500);
	...
	ret = bq27621_seal_block_data_memory();
	if (ret < 0) {
		dev_err(sn200007_device->client,
...
int bq27621_asus_battery_dev_read_fw_cfg_version_lite(void)
{
	int DV = 0;
	int ret = 0, temp = 0, count = 0;
	int retry_counter = 3;
...
int update_fw(struct update_op *p_start, struct update_op *p_end)
{
	...
	while (p_start <= p_end) {
		...
		case OP_ROM_WRITE:
			op_execute_ret =
			    bq27520_rom_mode_write_i2c(p_start->off,
						       p_start->arg, 1);
			if (op_execute_ret < 0) {
				BAT_DBG_E("   FAIL\n");
				return ret;
		...
		case OP_ROM_CMP:
			op_execute_ret =
			    bq27520_rom_mode_cmp(p_start->off, p_start->arg);
		...
		case OP_I2C_WRITE:
			msleep(7);
			op_execute_ret =
			    bq27520_write_i2c(sn200007_device->client,
...
int update_from_normal_mode(bool forceupdate)
{
	...
	fw_cfg_version = bq27621_asus_battery_dev_read_fw_cfg_version_lite();
	if (fw_cfg_version < 0) {
		BAT_DBG_E
		    ("%s: read battery firmware config version failed, ret = %d\n",
		     __func__, fw_cfg_version);
		return fw_cfg_version;
	}

	fw_gauge_id = bq27621_asus_battery_dev_read_gauge_id();
	if (fw_gauge_id < 0) {
		BAT_DBG_E("%s: read battery firmware gauge id failed, ret = %d\n", __func__, fw_gauge_id);
		goto Done;
	}
	...
	if (!forceupdate) {
		if (fw_cfg_version == latest_fw_cfg_version && fw_gauge_id == latest_fw_gauge_id) {
			BAT_NOTICE
			    ("========= No need to flash battery cell data due to that both data flash version and gauge id are equal =========");
			goto Done;
		}
	}

	smb345_charging_toggle(JEITA, false);	//Stop charging to flash fw.
	msleep(5000);
...
static ssize_t batt_switch_name(struct switch_dev *sdev,char *buf){
	...
	return sprintf(buf,"%d-%d-%x\n",g_battery_id,bq27621_asus_battery_dev_read_fw_cfg_version_lite(),
...
/**
 * End of Firmware Update for TI bq27621
 */

/* 1. Send the appropriate keys to Control( ) (0x00 and 0x01) */
int bq27621_unseal_block_data_memory(void)
{
	int ret = 0;
	...
	if (ret < 0) {
		BAT_DBG_E
		    ("%s: Send the appropriate keys to Control( ) failed, %d\n",
		     __func__, ret);
		return ret;
	}
...
int bq27621_set_cfg_update(void)
{
	...
	if (ret < 0) {
		BAT_DBG_E("%s: Send SET_CFGUPDATE subcommand failed, %d\n",
			  __func__, ret);
		return ret;
	}
...
int bq27621_enable_block_data_memory_control(void)
{
	...
	if (ret < 0) {
		BAT_DBG_E("%s: Enable block data memory control failed, %d\n",
			  __func__, ret);
		return ret;
	}
 ...
 /* 5.
  * Write 0x40 using the DataClass( ) command (0x3E) to access the State
  * subclass (64 decimal, 0x40 hex) containing the DF Version parameter.
  */
int bq27621_access_state_subclass(int id)
{
	...
	ret = bq27520_write_i2c(sn200007_device->client, 0x3E, id, 1);
	if (ret < 0) {
		BAT_DBG_E("%s: Access the State subclass failed, %d\n",
			  __func__, ret);
		return ret;
	}
...
int bq27621_write_block_offset_location(int offset)
{
	...
	else if (32 <= offset && offset <= 41)
		ret = bq27520_write_i2c(sn200007_device->client, 0x3F, 0x01, 1);
	else {
		BAT_DBG_E("%s: Invalid block offset location %d\n,", __func__,
			  offset);
		return -EINVAL;
	}

	if (ret < 0) {
		BAT_DBG_E("%s: Write the block offset location failed, %d\n",
			  __func__, ret);
		return ret;
	}
...
int bq27621_read_checksum(int *csum)
{
	...
	ret = bq27520_read_i2c(sn200007_device->client, 0x60, csum, 1);
	if (ret < 0) {
		BAT_DBG_E("%s: Read the 1-byte checksum failed, %d\n", __func__,
			  ret);
		return ret;
	}
...
/* 8. Read block data memory value 1-byte */
int bq27621_read_block_data_memory_value(int offset, int *value)
{
	int ret = 0;
	int addr;
	...
	ret = bq27520_read_i2c(sn200007_device->client, addr, value, 1);
	if (ret < 0) {
		BAT_DBG_E("%s: Read block data memory value failed, %d\n",
			  __func__, ret);
		return ret;
	}

	BAT_DBG("%s: addr is 0x%x, value is 0x%x\n", __func__, addr, *value);

	return ret;
}

/* 9. Write block data memory value 1-byte */
int bq27621_write_block_data_memory_value(int offset, int value)
{
	int ret = 0;
	int addr;
	...
	ret = bq27520_write_i2c(sn200007_device->client, addr, value, 1);
	if (ret < 0) {
		BAT_DBG_E("%s: Write block data memory value failed, %d\n",
			  __func__, ret);
		return ret;
	}
...
/* 11. Write the 1-byte checksum using the BlockDataChecksum( ) command (0x60). */
int bq27621_write_checksum(int csum)
{
	int ret = 0;
	...
	if (ret < 0) {
		BAT_DBG_E("%s: Write the 1-byte checksum failed, %d\n",
			  __func__, ret);
		return ret;
	}
...
/* 12.
 * Exit CONFIG UPDATE mode by sending SOFT_RESET subcommand, Control(0x0042)
 */
int bq27621_set_soft_reset(void)
{
	int ret = 0;
	...
	if (ret < 0) {
		BAT_DBG_E("%s: Sending SOFT_RESET subcommand failed, %d\n",
			  __func__, ret);
		return ret;
	}
...
/* 14. Return to SEALED mode by sending the Control(0x0020) command. */
int bq27621_seal_block_data_memory(void)
{
	int ret = 0;
	...
	if (ret < 0) {
		BAT_DBG_E("%s: Send the Control(0x0020) command failed, %d\n",
			  __func__, ret);
		return ret;
	}
...
static int create_sn200007_firmware_proc_fs(void)
{
	...
	entry =
	    proc_create("driver/gaugeIC_FW", 0664, NULL,
			&proc_sn200007_firmware_ops);
	if (!entry) {
...
static int proc_sn200007_firmware_write_2(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	cancel_delayed_work(&sn200007_device->status_poll_work);
	queue_delayed_work(battery_poll_work_queue,
		&sn200007_device->status_poll_work, 30*HZ);
...
static int sn200007_probe(struct i2c_client *client,
			  const struct i2c_device_id *id)
{
	int ret, i = 0, rt_value;

	BAT_NOTICE
	    ("========================= (sn200007_probe) client->addr= %02x\n",
	     client->addr);
...
//	main_update_flow();		[Carlisle]Updating from user-space
	...
	INIT_DELAYED_WORK(&sn200007_device->status_poll_work,
			  battery_status_poll);
	INIT_DELAYED_WORK(&sn200007_device->low_low_bat_work,
	...
	wake_lock_init(&sn200007_device->low_battery_wake_lock,
		       WAKE_LOCK_SUSPEND, "low_battery_detection");
	wake_lock_init(&sn200007_device->cable_event_wake_lock,
	...
	if (ret)
		dev_err(&client->dev,
			"sn200007_probe: unable to create the sysfs\n");

	/* stress test */
	sn200007_stress_test_work_queue =
	    create_singlethread_workqueue("i2c_battery_wq");
	if (!sn200007_stress_test_work_queue) {
		printk
		    ("battery sn200007 : unable to create i2c stress test workqueue\n");
	}
	INIT_DELAYED_WORK(&sn200007_stress_test_poll_work,
			  sn200007_stress_test_poll);
	...
	if (ret)
		dev_err(&client->dev,
			"Cannot register sn200007 miscdev (err = %d)\n", ret);
	...
	if (ret) {
		BAT_ERR
		    ("Unable to create sn200007_register_upilogger_proc_fs\n");
	}
#endif
	...
	queue_delayed_work(battery_poll_work_queue,
			   &sn200007_device->status_poll_work, 5 * HZ);
	/* Update gauge fw */
	queue_delayed_work(sn200007_update_work_queue, &sn200007_device->update_gauge_work, 15*HZ);
	...
	printk(KERN_INFO "Hardware_id is %d \n", HW_ID);
