...
//#define UG31XX_BATTERY_ID_SELECTION                 ///< Select GGB according to battery ID
...
#ifdef  UG31XX_BATTERY_ID_SELECTION
typedef struct upi_rid_st {
  char  target[16];
  int   upper_bound;      ///< [AT-PM] : Unit in 0.1kOhm ; 05/08/2015
  int   lower_bound;      ///< [AT-PM] : Unit in 0.1kOhm ; 05/08/2015
  char *table;
} upi_rid_type;

static upi_rid_type upi_ug31xx_rid_table[] =
{
  { "10kOhm",   80,   120,  FactoryGGBXFile_A023Cl, },
  { "51kOhm",   410,  610,  FactoryGGBXFile_A0249l, },
  { "100kOhm",  800,  1200, FactoryGGBXFile_A025Jl, },
  { "",         0,    0,    FactoryGGBXFile_A023Cl, },      ///< [AT-PM] : Use A023Cl as default ; 06/18/2015
};
#endif  ///< end of UG31XX_BATTERY_ID_SELECTION
...
extern bool G_UPI_INIT;

#define UG31XX_COVER_TYPE_POWER_BANK    (1)
#define UG31XX_COVER_TYPE_AUDIO_SLEEVE  (2)
...
static bool is_charging_full(void)
{
  ...
  if (ret == POWER_SUPPLY_STATUS_FULL) {
    GLOGD("[%s]: Charging staus is POWER_SUPPLY_STATUS_FULL\n", __func__);
    return (true);
  } else {
    GLOGD("[%s]: Charging staus is %d\n", __func__, ret);
    return (false);
  }
}
...
static void upi_ug31xx_reset_capacity(void)
{
  GWORD wSysMode;
  GBYTE buf[2];
...
{
  upi_ug31xx->curr_cable_status = gpio_get_value(66) == 0 ? UPI_UG31XX_CABLE_IN : UPI_UG31XX_CABLE_OUT;
...
//  UPI_MEMCPY(FactoryGGBXFile, FactoryGGBXFile_A027Co, sizeof(GlobalDFVarType));     ///< CB81
  ...
  if(get_cover_type() == UG31XX_COVER_TYPE_POWER_BANK)
  {
    UPI_MEMCPY(FactoryGGBXFile, FactoryGGBXFile_A0249l, sizeof(GlobalDFVarType));     ///< CB71
  }
  if(get_cover_type() == UG31XX_COVER_TYPE_AUDIO_SLEEVE)
  {
    UPI_MEMCPY(FactoryGGBXFile, FactoryGGBXFile_A0262h, sizeof(GlobalDFVarType));     ///< CA71 and CA81
  }
...
static void upi_ug31xx_main_task(struct work_struct *work)
{
  ...
  else if(wSysMode & SYS_MODE_MEAS_STABLE)
  {
    #ifdef  UG31XX_BATTERY_ID_SELECTION
    if(get_cover_type() == UG31XX_COVER_TYPE_POWER_BANK)
    {
      /// [AT-PM] : Read RID ; 05/08/2015
      SmbReadCommand(SBS_RID);
      wRid = (GWORD)(*(SBSCmd->pbData + 1));
      wRid = (wRid << 8) | (*(SBSCmd->pbData));
      GLOGE("[%s]: RID = %d for AUDIO SLEEVE\n", __func__, wRid);

      /// [AT-PM] : Select cell table according to RID ; 05/08/2015
      idx = 0;
      while(1)
      {
        if((upi_ug31xx_rid_table[idx].upper_bound == 0) &&
           (upi_ug31xx_rid_table[idx].lower_bound == 0))
        {
          break;
        }

        if((wRid < upi_ug31xx_rid_table[idx].upper_bound) &&
           (wRid > upi_ug31xx_rid_table[idx].lower_bound))
        {
          break;
        }
      ...
      UPI_MEMCPY(FactoryGGBXFile, upi_ug31xx_rid_table[idx].table, sizeof(GlobalDFVarType));

      /// [AT-PM] : Issue command for reloading cell table ; 05/08/2015
      buf[0] = RELOAD_GGB_MFA % 256;
      buf[1] = RELOAD_GGB_MFA / 256;
      SmbWriteCommand(SBS_MFA, &buf[0], 2);
    }
    #endif  ///< end of UG31XX_BATTERY_ID_SELECTION

    schedule_delayed_work(&upi_ug31xx->info_update_work, msecs_to_jiffies(TIME_RESET_NEXT_ROUND));
    GLOGE("[%s]: Wait for next normal round\n", __func__);
  }
  else
  {
    schedule_delayed_work(&upi_ug31xx->info_update_work, msecs_to_jiffies(TIME_RESET_NEXT_ROUND));
...
{
  GLOGD("[%s]: gauge_attached = %d (1 = true)\n", __func__, gauge_attached);
...
/* ASUS cover test +++ */
static int proc_ug31xx_i2c(struct seq_file *m, void *v)
{
  GBYTE buf[2];
  GWORD wData;
  #ifdef  UG31XX_ATTACH_DETECTION
  ...
  buf[0] = REG16_CHARGE % 256;
  buf[1] = REG16_CHARGE / 256;
  SmbWriteCommand(SBS_REG_ADDR, &buf[0], 2);

  SBSCmd->errNo = NO_ERR_PASS;
  *(SBSCmd->pbData)     = 0x55;
  *(SBSCmd->pbData + 1) = 0xAA;
  SmbReadCommand(SBS_REG_DATA);
  wData = (GWORD)(*(SBSCmd->pbData + 1));
  wData = (wData << 8) | (*SBSCmd->pbData);
  GLOGE("[%s]: Value = %04x (%d)\n", __func__, wData, SBSCmd->errNo);
...
static int proc_ug31xx_i2c_ops_open(struct inode *inode, struct file *file)
{
  return (single_open(file, proc_ug31xx_i2c, NULL));
}

static const struct file_operations proc_ug31xx_i2c_ops = {
  .owner = THIS_MODULE,
  .open = proc_ug31xx_i2c_ops_open,
  .read = seq_read,
  .write = ug31xx_proc_write,
  .llseek = seq_lseek,
  .release = single_release,
...
static int ug31xx_create_proc(void)
{
  ...
  ent = proc_create("driver/CoverGaugeIC_Status", 0664, NULL, &proc_ug31xx_i2c_ops);
  if(!ent)
  {
    GLOGE("create /driver/CoverGaugeIC_Status\n");
  }
  return (0);
}
...
static int upi_ug31xx_battery_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
  ...
  switch (psp)
  {
    case POWER_SUPPLY_PROP_PRESENT:
      ...
      {
        if (gauge_attached == 1 && G_UPI_INIT == true)
          val->intval = 1;
        else
          val->intval = 0;
      }
    ...
    case POWER_SUPPLY_PROP_STATUS:
      ...
      else
      {
        ...
        GLOGD("[%s]: POWER_SUPPLY_PROP_STATUS: POWER_SUPPLY_STATUS_UNKNOWN\n", __func__);
      }
      break;
    ...
    case POWER_SUPPLY_PROP_CAPACITY:
      ...
      /// notify smart cover driver of cover battery capacity
      /// to check if the usb driver should be disconnected.
      if (G_UPI_INIT == true) {
        set_cover_usb(val->intval);
      }
    ...
    case POWER_SUPPLY_PROP_PACK_TYPE:
      if (get_cover_type() == POWER_BANK)
        val->intval = 1;
      else if (get_cover_type() == AUDIO_SLEEVE)
        val->intval = 2;
      else if (get_cover_type() == NFC_WC)
        val->intval = 3;
      else if (get_cover_type() == NFC)
        val->intval = 4;
      else if (get_cover_type() == WC)
        val->intval = 5;
      else
        val->intval = 0;
      GLOGD("[%s]: POWER_SUPPLY_PROP_PACK_TYPE: %d\n", __func__, val->intval);
      break;
...
{
  GBYTE  buf[2];
  int cover_otg_flag = 0;
  int usb_type = 0;

  cover_otg_flag = (g_cover_otg_flag == NULL) ? 0 : g_cover_otg_flag;
  GLOGD("<BATT> g_cover_otg_flag is %d, cover_otg_flag is %d\n", g_cover_otg_flag, cover_otg_flag);

  usb_type = (G_USB_TYPE == NULL) ? 0 : G_USB_TYPE;
  GLOGD("<BATT> G_USB_TYPE is %d, usb_type is %d\n", G_USB_TYPE, usb_type);

  upi_ug31xx->prev_cable_status = upi_ug31xx->curr_cable_status;
  if(gpio_get_value(66) == 0 && cover_otg_flag == 0 && (usb_type == AC_IN || usb_type == USB_IN))
  {
  ...
  GLOGD("<BATT> ++++++++++++++++++++ cable status changed %d -> %d (%d) ++++++++++++++++++++\n",
...
static void upi_ug31xx_power_supply_change(struct work_struct *work)
{
  ...
  GLOGD("[%s]: power supply changed.\n", __func__);
}

void force_upi_ug31xx_power_supply_change(void)
{
  #ifdef  UG31XX_REGISTER_POWER_SUPPLY
  power_supply_changed(&upi_ug31xx_supply);
  #endif  ///< end of UG31XX_REGISTER_POWER_SUPPLY
}
EXPORT_SYMBOL(force_upi_ug31xx_power_supply_change);

void force_upi_ug31xx_battery_external_power_changed(void)
{
  #ifdef  UG31XX_REGISTER_POWER_SUPPLY
  upi_ug31xx_battery_external_power_changed(&upi_ug31xx_supply);
  #endif  ///< end of UG31XX_REGISTER_POWER_SUPPLY
}
EXPORT_SYMBOL(force_upi_ug31xx_battery_external_power_changed);
...
static int ug31xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  cover_detect_register(AUDIO_SLEEVE, "ug31xx_battery:upi_ug31xx_attach", upi_ug31xx_attach);
  cover_detect_register(POWER_BANK, "ug31xx_battery:upi_ug31xx_attach", upi_ug31xx_attach);
...
static struct of_device_id ug31xx_match_table[] = {
  { .compatible = "intel,ug31xx-fg"},
  { },
};
...
static struct i2c_driver ug31xx_i2c_driver = {
  .driver    = {
    ...
    .of_match_table = ug31xx_match_table,
  },
...
static const struct idi_device_id idi_ids[] = {
  {
   ...
   .subdevice = IDI_SUBDEVICE_ID_ASUS_BAT,
   },

  { /* end: all zeroes */ },
};

static int ug31xx_idi_probe(struct idi_peripheral_device *ididev,
...
static int __exit ug31xx_idi_remove(struct idi_peripheral_device *ididev)
{
  pr_info("%s\n", __func__);
...
static struct idi_peripheral_driver ug31xx_idi_driver = {
  .driver = {
       .owner = THIS_MODULE,
       .name = "ug31xx_idi",
       .pm = NULL,
  ...
  .p_type = IDI_BAT2,
  .id_table = idi_ids,
  .probe = ug31xx_idi_probe,
  .remove = ug31xx_idi_remove,
};

static int __init ug31xx_battery_init(void)
{
  int ret;

  ret = idi_register_peripheral_driver(&ug31xx_idi_driver);
  if (ret) {
...
module_init(ug31xx_battery_init);
//late_initcall(ug31xx_battery_init);

static void __exit ug31xx_battery_exit(void)
{
  i2c_del_driver(&ug31xx_i2c_driver);
}
module_exit(ug31xx_battery_exit);
