...
// Webber Jeita ++++++++++++++++++++++++
static bool vflt_4_11 = false;
// Webber Jeita ------------------------

// Webber +++ fix can't enter COS mode in power on/off stress teset
static bool shutdown_do_not_set_vbus = false;
// Webber --- 
...
#define STAT_D                    0x3e
#define STAT_D_APSD_STATUS        BIT(4)
#define STAT_E                    0x3f
...
/* Webber use for read Vbus signal*/
#define PMU_C0_FS_REG 0xE6501038
#define PMU_C0_FS_CHGDET BIT(8)
/**/
...
int smb345_soc_control_jeita(void) {
    ...
    /* set hard hot limit = 70 deg.c, write 0bh[5:4]="11" */
    ...
    /* set soft hot limit = 59 deg.c, write 0bh[1:0]="11" */
    ret = smb345_masked_write(smb345_dev->client,
        HARD_SOFT_LIMIT_CELL_TEMP_MONITOR_REG,
        BIT(1)|BIT(0),
        BIT(1)|BIT(0));
...
int smb345_charger_control_jeita(void) {
    ...
    ret = smb345_set_writable(smb345_dev, true);
    if (ret < 0) {
        pr_err("fail to allow violate register can be written\n");
        return ret;
    }
    ...
    /* set soft hot limit = 47 deg.c, write 0bh[1:0]= "01" */
    ret = smb345_masked_write(smb345_dev->client,
			HARD_SOFT_LIMIT_CELL_TEMP_MONITOR_REG,
			BIT(1) | BIT(0),
			BIT(0));
    ...
    if (ret) {
        pr_err("fail to set soft hot limie ret=%d\n", ret);
        return ret;
    }
    ...
    /* set recharge voltage = Vflt-100mV, write 01h[3:2]="01" */
    ret = smb345_masked_write(smb345_dev->client,
			0x01,
			BIT(2)|BIT(3),
			BIT(2));
    if (ret) {
        pr_err("fail to set Recharge Voltage, ret=%d\n", ret);
        return ret;
    }
...
static int smb358_pre_config(void) {
    ...
    ret = smb345_masked_write(smb345_dev->client,
        ...
        SMB358_FAST_CHG_CURRENT_VALUE_1500mA);
    if (ret < 0) {
        dev_err(&smb345_dev->client->dev, "%s: fail to set fast charge current!\n", __func__);
        goto fail;
    }
    ...
    ret = smb345_masked_write(smb345_dev->client,
            ...
            BIT(7));
    if (ret < 0) {
        dev_err(&smb345_dev->client->dev, "%s: fail to set cold soft limit current!\n", __func__);
        goto fail;
    }
    ...
    if (ret < 0) {
        dev_err(&smb345_dev->client->dev, "%s: fail to set termination current!\n", __func__);
        goto fail;
    }
    ...
    if (ret < 0) {
        dev_err(&smb345_dev->client->dev, "%s: fail to set charger disable!\n", __func__);
        goto fail;
    }
    ...
    if (ret < 0) {
        dev_err(&smb345_dev->client->dev, "%s: fail to set charger voltage!\n", __func__);
        goto fail;
    }
    ...
    ret = smb345_charging_toggle(JEITA, true);
    if (ret < 0) {
        dev_err(&smb345_dev->client->dev, "%s: fail to set charger enable!\n", __func__);
        goto fail;
    }
...
static void smb3xx_config_max_current(int usb_state) {
    ...
    if (usb_state == AC_IN) {
        aicl_result = get_aicl_results();
        if (aicl_result >= 1000) {
            dev_err(&smb345_dev->client->dev, "%s: now input current setting is larger than 1200 mA, no need set again!!\n", __func__);
            return;
        }
...
int do_battery_jeita(int batt_tempr, int batt_volt,int temp_zone_last) {
   ...
   } else if (batt_tempr >= (0 + need_add_0) && batt_tempr < (150 + need_add_0_10)) {
      // 0 < Temp < 15
      battery_temp_zone = TEMP_0_10;
   } else if (batt_tempr >= (150 + need_add_0_10) && batt_tempr < (500 - need_add_50_55)) {
      // 15 < Temp < 50
      battery_temp_zone = TEMP_10_50;
...
// Webber Jeita +++++++++++++++++++++++++++++++++++++
   if(vflt_4_11 == false){  /*  vfloat = 4.35v   */
   	 if (batt_volt >= 4250) {
...
static irqreturn_t smb345_detect_irq_handler(int irq, void *data)
{
	...
	queue_delayed_work(system_nrt_wq, &smb->detect_irq_work, msecs_to_jiffies(500));
...
static int smb345_i2c_read_reg(struct i2c_client *client, u8 reg_addr, u8 *data)
{
...
//	*status = (regval & PMU_CONFIG_CHGDET_M) ? VBUS_OFF : VBUS_ON; /* FIXME: it should be reverse */
	...
	/*Do not use, because it will result in an error of judgment*/
	//ret = smb345_i2c_read_reg(chrgr->client, STAT_D, &regval);
	//status2 = (regval & STAT_D_APSD_STATUS) ? VBUS_ON : VBUS_OFF;

	/* read PMIC C0_FS_Register REG 0xE6501038 bit 8 (CHGDET) as vbus signal*/
	ret = idi_client_ioread(smb345_dev->ididev,PMU_C0_FS_REG , &regval);
	printk("[%s][WB] read Reg 0x%08X  : 0x%08x\n",__func__,PMU_C0_FS_REG ,regval);
	*status = (regval & PMU_C0_FS_CHGDET) ? VBUS_OFF : VBUS_ON;

	if(g_usb_state == ENABLE_5V){
		*status = VBUS_OFF;
		printk("[%s][WB] CURRENT OTG plug in ,so VBUS set to %d\n",__func__,(*status));
	}
...
static void smb345_interrupt_irq_worker(struct work_struct *work)
{
	...
	u8 data;
	//int vbus_prev;
	struct smb345_charger *smb = container_of(work, struct smb345_charger, interrupt_irq_work);
	...
	if(vbus_prev != smb->vbus){
		...
		atomic_notifier_call_chain(&smb->otg_handle->notifier, USB_EVENT_VBUS, &smb->vbus);
	}
#endif
...
static void smb345_detect_irq_worker(struct work_struct *work)
{
	...
	u8 data, val;
	int vbus_prev;
	//struct smb345_charger *smb = container_of(work, struct smb345_charger, detect_irq_work);
	struct smb345_charger *smb = &chrgr_data;

	printk("%s\n", __func__);

	if(shutdown_do_not_set_vbus == false){ // normal case
		printk("[%s][WB] in normal case , can set vbus singal\n",__func__);
		...
		if(vbus_prev != smb345_dev->vbus){
			printk("%s(), vbus changed, notify usb\n", __func__);
			atomic_notifier_call_chain(&smb345_dev->otg_handle->notifier, USB_EVENT_VBUS, &smb345_dev->vbus);
		}
		up(&smb345_dev->prop_lock);
	}else{ /*fix can't enter COS mode after shutdown*/ 
		printk("[%s][WB] in shutdown case , do not set vbus singal\n",__func__);
	}
...
static int smb345_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	smb->pins_inactive = pinctrl_lookup_state(smb->pinctrl,
						"inactive");
...
*/
	...
	ret = request_irq(smb->chgint_irq,
			smb345_detect_irq_handler, IRQF_SHARED, "smb345_charger", smb);
	if (ret != 0) {
	...
	ret = request_irq(smb->chgdet_irq,
			smb345_detect_irq_handler, IRQF_NO_SUSPEND, "smb345_charger", smb);
	if (ret != 0) {
		pr_err("Failed to register @PMU for GHGINT irq! ret=%d", ret);
		return ret;
	}
...
static int smb345_shutdown(struct i2c_client *client) {
    ...
    /*fix can't enter COS mode after shutdown*/
    shutdown_do_not_set_vbus = true;
...
static void smb345_complete(struct device *dev) {
    ...
    /*for charging os power spec behavior , long press to enter normal os*/
    if (smb345_dev) {
	      if (entry_mode == 4)
