...
static DEVICE_ATTR(audio_codec_status, S_IRUGO, codec_show, NULL);
...
int ret_headset_status = 0;
static ssize_t headset_show(struct device *dev, struct device_attribute *attr,
			   char *buf);
static ssize_t headset_set(struct device *dev, struct device_attribute *attr,
         const char *buf, size_t count);
static DEVICE_ATTR(headset_status, S_IWUSR | S_IRUGO, headset_show, headset_set);
...
EXPORT_SYMBOL(ret_headset_status);

extern void xgold_jack_headset_set(int state);
static ssize_t headset_set(struct device *dev, struct device_attribute *attr,
         const char *buf, size_t count)
{
	afe_debug("headset_set buf=%s", buf);
	afe_debug("headset_set count=%d\n", count);

	if (*(buf) == '0') {
	  afe_debug("headset_set input 0");
	  xgold_jack_headset_set(0);
	} else if (*(buf) == '1') {
	  afe_debug("headset_set input 1");
	  xgold_jack_headset_set(1);
	} else if (*(buf) == '2') {
	  afe_debug("headset_set input 2");
	  xgold_jack_headset_set(2);
	}
...
static int agold_afe_get_spkgpio_info(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_info *uinfo)
{
...
static int agold_afe_get_spkgpio_val(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *uinfo)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct agold_afe_data *agold_afe = snd_soc_codec_get_drvdata(codec);
	mutex_lock(&codec->mutex);
	uinfo->value.integer.value[0] = agold_afe->spk_status;
	mutex_unlock(&codec->mutex);
	return 0;
}

static int agold_afe_set_spkgpio_val(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *uinfo)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct agold_afe_data *agold_afe = snd_soc_codec_get_drvdata(codec);
	mutex_lock(&codec->mutex);
	agold_afe->set_spk_gpio(agold_afe->codec->dev, uinfo->value.integer.value[0]);
	mutex_unlock(&codec->mutex);
	return 0;
}
...
static const struct snd_kcontrol_new agold_afe_snd_controls[] = {
	...
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "EXT SPK AMP GPIO",
		.info = agold_afe_get_spkgpio_info,
		.get = agold_afe_get_spkgpio_val,
		.put = agold_afe_set_spkgpio_val
	},
#ifdef CONFIG_SND_SOC_AGOLD_620
...
skip_pinctrl:
	...
	if (!ret) {
		...
		agold_afe->spk_status = 0;
		gpio_direction_output(agold_afe->spk_gpio, 0);
	} else {
...
static int __init agold_afe_modinit(void)
{
  ...
  if (entry_mode != 1) {
    afe_debug("return because entry_mode!=1");
    return -ENODEV;
  }
