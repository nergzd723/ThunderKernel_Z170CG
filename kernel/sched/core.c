...
need_resched:
	...
	if (unlikely(prev->on_rq && (preempt_count() & PREEMPT_ACTIVE)
				&& foreground_pid == prev->pid)) {
		next = prev;
	} else {
...
/*
 * sched_set_foreground - set foreground app
 * @pid:
 *
 * sched_set_foreground disable the preempt schedule for
 * forground app
 */
SYSCALL_DEFINE2(sched_set_foreground, pid_t, pid, int, enable)
{
	if (pid > 0) {
		if (enable)
			foreground_pid = pid;
