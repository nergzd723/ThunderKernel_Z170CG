...
int get_lcm_id(void) //OK
{
	...
	//printk("Get the lcm ID \n");
        int tmp_lcm=0;
        //tmp_lcm=get_gpio(GPIO_LCM_1);
//	tmp = get_gpio(GPIO_LCM_1);
//	printk("Get the lcm ID : GPIO 46(LCM ID 2) :%d \n",tmp);
//	lcm_id =(tmp<<1);	
	tmp = get_gpio(GPIO_LCM_0);
	printk("Get the lcm ID : GPIO 62(LCM ID 1) :%d \n",tmp);
	lcm_id = tmp;
	switch (lcm_id) {
		case 0:
			return CPT;
...
int total_info_func(void)	
{	
//int pcbid_status; 	
int get_hw_rev(void);	
int get_project_id(void);	
int get_lcm_id(void);	
int get_sub_cam_id(void);
int get_main_cam_id(void);	
int get_rf_sku_id(void);	

pcbid_status=hardware_id| project_id <<3|lcm_id <<6|sub_cam_id <<8|main_cam_id <<9|rf_sku_id<<10;	
    printk("pcbid_status := %x\n", pcbid_status,pcbid_status);	
    return pcbid_status;	
}
...
void PCBID_add_dvfs(void)
{
	...
	//printk("Debugnode:%s\n",__FUNCTION__);
        int total_info = total_info_func();
        printk("total_info := %x \n", total_info,total_info);
        rul = debugfs_create_x32("pcbid_status", 0644, debugfs_root, &pcbid_status);
        if (!rul)
                goto Fail;
